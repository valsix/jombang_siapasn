...
class Pagination{
	...
	var $prevLink		= '‹ ';
	var $lastLink		= '» ';
	//var $fullTagOpen	= '<div class="pagination"><span class="menampilkan">';
	var $fullTagOpen	= '<section class="halaman"><nav role="navigation">';
	var $fullTagClose	= '</nav></section>';
	var $firstTagOpen	= '';
	var $firstTagClose	= '&nbsp;';
	var $lastTagOpen	= '&nbsp;';
	var $lastTagClose	= '';
	...
	function createLinks(){ 
		...
		if ($numPages == 1){
			if ($this->showCount){
				$info = $this->fullTagOpen.'<span class="menampilkan">Menampilkan : ' . $this->totalRows.$this->fullTagClose;
				return $info;
			}else{
				return '';
			}
		...
		if ($this->showCount){
		   ...
		   $info = '<span class="menampilkan">Menampilkan ' . ( $currentOffset + 1 ) . ' ke ' ;
		   ...
		   $info .= ' dari ' . $this->totalRows . '</span> <span class="cd-pagination">';
