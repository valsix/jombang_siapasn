...
class globalfilepegawai
{
	...
	function persyaratanusulan($arrparam)
	{
		...
		else
		{
			...
			$set->selectByParamsUsulanGroupNew(array(), -1, -1, $statement);
		}
		...
		$arrreturn["NAMA_PEGAWAI"]= $set->getField("NAMA_PEGAWAI");
		$arrreturn["NAMA_LENGKAP"]= $set->getField("NAMA_LENGKAP");
		...
		while($set->nextRow())
		{
			...
			if(!empty($reqPegawaiId))
			{
				...
				if(empty($arrkondisicheck))
				{
					...
					$arrdata["PEGAWAI_NAMA"]= str_replace("'", "", $set->getField("NAMA_PEGAWAI"));
					$arrdata["PEGAWAI_NAMA_LENGKAP"]= str_replace("'", "", $set->getField("NAMA_LENGKAP"));
