...
class penghargaan_json extends CI_Controller {
	...
	function add()
	{
		...
		$reqPenghargaanPredikatId= $this->input->post("reqPenghargaanPredikatId");
		...
		$set->setField('PENGHARGAAN_PREDIKAT_ID', ValToNullDB($reqPenghargaanPredikatId));
		...
		$set->setField("REF_PENGHARGAAN_ID", $reqRefPenghargaanId);
		$set->setField("NAMA_DETIL", setQuote($reqNamaDetil,1));
	...
	function delete()
	{
		...
		$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
		$set->setField("LAST_DATE", "NOW()");
		$set->setField("PENGHARGAAN_ID", $reqId);
		
		if($reqMode == "penghargaan_0")
		{
	...
	{	
		$this->load->model('PenghargaanLog');

		$set = new PenghargaanLog();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PENGHARGAAN_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PENGHARGAAN_ID");
		...
		$arrayWhere = array("PENGHARGAAN_ID" => $riwayatId);
