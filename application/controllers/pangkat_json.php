...
class pangkat_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("NAMA", "KODE", "LINK_URL_INFO", "PANGKAT_ID");
		$aColumnsAlias = array("NAMA", "KODE", "LINK_URL_INFO", "PANGKAT_ID");
	...
	function combokenaikanpangkat()
	{
		...
		if($reqMode == "upt")
		{
			$statementpengaturan= " AND EXISTS
			(
				SELECT 1
				FROM PERSURATAN.SURAT_MASUK_UPT X WHERE SURAT_MASUK_UPT_ID = ".$reqRowId."
				AND A.PENGATURAN_KENAIKAN_PANGKAT_ID = X.PENGATURAN_KENAIKAN_PANGKAT_ID
			)";
		}
		elseif($reqMode == "bkd")
		{
			...
			(
				SELECT 1
				FROM PERSURATAN.SURAT_MASUK_BKD X WHERE SURAT_MASUK_BKD_ID = ".$reqRowId."
				AND A.PENGATURAN_KENAIKAN_PANGKAT_ID = X.PENGATURAN_KENAIKAN_PANGKAT_ID
			)";
		}
		...
		$set->selectByParams(array(), -1, -1, $statementpengaturan);
		$set->firstRow();
		$reqPengaturanTiga= dateToPageCheck($set->getField("TANGGAL_BATAS_AKHIR_USUL_NON_TIGA_BAWAH"));
		$reqPengaturanTiga= strtotime($reqPengaturanTiga);
		// echo $reqPengaturanTiga;exit();

		$reqPengaturanEmpat= dateToPageCheck($set->getField("TANGGAL_BATAS_AKHIR_USUL_NON_EMPAT_ATAS"));
		$reqPengaturanEmpat= strtotime($reqPengaturanEmpat);
		// echo $reqPengaturanEmpat;exit();

		// echo $set->query;exit();
		unset($set);

		$set = new Pangkat();
		// $statement= " AND A.PANGKAT_ID >= ".$reqId;
		$statement= " AND A.PANGKAT_ID > ".$reqId;

		if($tanggalsekarang > $reqPengaturanTiga)
			$statement.= " AND A.PANGKAT_ID > 34";
			// echo "lebih tiga";

		if($tanggalsekarang > $reqPengaturanEmpat)
			$statement.= " AND A.PANGKAT_ID < 41";
		// 	echo "lebih empat";
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id'] = $set->getField("PANGKAT_ID");
			$arr_json[$i]['text'] = $set->getField("KODE");	
			$i++;
		}
		echo json_encode($arr_json);
	}
