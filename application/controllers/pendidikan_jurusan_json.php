...
class pendidikan_jurusan_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("PENDIDIAKN_NAMA", "NAMA", "LINK_URL_INFO", "PENDIDIKAN_JURUSAN_ID");
		$aColumnsAlias = array("PENDIDIAKN_NAMA", "NAMA", "LINK_URL_INFO", "PENDIDIKAN_JURUSAN_ID");
	...
	function add()
	{
		...
		{
			$set->setField('PENDIDIKAN_JURUSAN_ID', $reqId); 
			$set->setField("LAST_USER", $this->LOGIN_USER);
			$set->setField("USER_LOGIN_ID", $this->LOGIN_ID);
			$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
			$set->setField("LAST_DATE", "NOW()");
	...
	function delete()
	{
		...
		if($reqMode == "pendidikan_jurusan_0")
		{
		...
		elseif($reqMode == "pendidikan_jurusan_1")
		{
	...
	function combo() 
	{
		...
		while($set->nextRow())
		{
			$infoid= $set->getField("PENDIDIKAN_JURUSAN_ID");
			$arr_json[$i]['id'] = $infoid;
			$arr_json[$i]['label'] = $set->getField("NAMA");
			$arr_json[$i]['desc'] = $set->getField("NAMA");

			/*$inforumpunid= $inforumpun= "";
			$statement= " AND A.INFOMODE = 'pendidikan_jurusan' AND A.PERMEN_ID = 1 AND A.NILAI > 0 AND A.INFOID = ".$infoid;
			$setdetil= new PendidikanJurusan();
			$setdetil->selectparamsrumpunnilai(array(), -1,-1, $statement);
			// echo $setdetil->query;exit;
			while ($setdetil->nextRow())
			{
				if(empty($inforumpunid))
				{
					$inforumpunid= $setdetil->getField("RUMPUN_ID");
					$inforumpun= $setdetil->getField("KETERANGAN");
				}
				else
				{
					$inforumpunid.= ",".$setdetil->getField("RUMPUN_ID");
			...
			$vrumpunid= $set->getField("RUMPUN_ID");
			$inforumpunid= $inforumpun= "";

			if(!empty($vrumpunid))
			{
				$statementdetil= " AND A.PERMEN_ID = 1 AND A.RUMPUN_ID IN (".$vrumpunid.")";
				$setdetil= new Rumpun();
			...
			$arr_json[$i]['rumpun_id']= $inforumpunid;
			$arr_json[$i]['rumpun_nama']= $inforumpun;
	...
	function log() 
	{
		$this->load->model('PendidikanJurusanLog');

		$set = new PendidikanJurusanLog();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PENDIDIKAN_JURUSAN_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PENDIDIKAN_JURUSAN_ID");
