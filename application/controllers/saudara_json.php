...
class saudara_json extends CI_Controller {
	...
	function delete()
	{
		...
		$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
		$set->setField("LAST_DATE", "NOW()");
		$set->setField("SAUDARA_ID", $reqId);
		
		if($reqMode == "saudara_0")
		{
	...
	{	
		...
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "SAUDARA_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "SAUDARA_ID");
		...
		$arrayWhere = array("SAUDARA_ID" => $riwayatId);
