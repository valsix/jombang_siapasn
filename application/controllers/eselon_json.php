...
class eselon_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("NAMA", "PANGKAT_MINIMAL_NAMA", "PANGKAT_MAKSIMAL_NAMA", "LINK_URL_INFO", "ESELON_ID");
		$aColumnsAlias = array("NAMA", "PANGKAT_MINIMAL_NAMA", "PANGKAT_MAKSIMAL_NAMA", "LINK_URL_INFO", "ESELON_ID");
		...
		$set->selectByParamsMonitoring(array(), $dsplyRange, $dsplyStart, $searchJson);     		
	...
	function add()
	{
		...
		if($tempSimpan==1)
		{
			for($i=0;$i<count($reqTunjangan);$i++)
			{
				$set_data = new EselonDetil();
				$set_data->setField('ESELON_ID', $reqId);
				$set_data->setField('TUNJANGAN', ValToNullDB(dotToNo($reqTunjangan[$i])));
				...
				{
					$set_data->setField('ESELON_DETIL_ID', $reqRowDetilId[$i]);
					$set_data->update();
				}
			}
	...
	function log() 
	{
		...
		$columnsShow = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "ESELON_ID");
