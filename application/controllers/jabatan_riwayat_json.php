...
class jabatan_riwayat_json extends CI_Controller {
	...
	function add()
	{
		...
		$reqLamaMenjabat= $this->input->post("reqLamaMenjabat");
		$reqNilaiRekam= $this->input->post("reqNilaiRekam");
		$reqRumpunJabatan= $this->input->post("reqRumpunJabatan");
		$reqNoSkSertifikasi= $this->input->post("reqNoSkSertifikasi");
		$reqTglSkSertifikasi= $this->input->post("reqTglSkSertifikasi");
		$reqTglBerlaku= $this->input->post("reqTglBerlaku");
		$reqTglExpired= $this->input->post("reqTglExpired");
		$reqTmtSpmt= $this->input->post("reqTmtSpmt");
		$reqBidangJabatanTerkaitId= $this->input->post("reqBidangJabatanTerkaitId");
		...
		$set->setField('BIDANG_JABATAN_TERKAIT_ID', ValToNullDB($reqBidangJabatanTerkaitId));
		...
		$set->setField('STATUS_SK_DASAR_JABATAN', ValToNullDB($reqSkDasarJabatan));
		$set->setField('TMT_SELESAI_JABATAN', dateToDBCheck($reqTanggalSelesai));
		$set->setField('LAMA_JABATAN', ValToNullDB($reqLamaMenjabat));
		$set->setField('NILAI_REKAM_JEJAK', ValToNullDB($reqNilaiRekam));
		$set->setField('RUMPUN_ID', ValToNullDB($reqRumpunJabatan));
		$set->setField('SERTIFIKASI_NO_SK', $reqNoSkSertifikasi);
		$set->setField('SERTIFIKASI_TGL_SK', dateToDBCheck($reqTglSkSertifikasi));
		$set->setField('SERTIFIKASI_TGL_BERLAKU', dateToDBCheck($reqTglBerlaku));
		$set->setField('SERTIFIKASI_TGL_EXPIRED', dateToDBCheck($reqTglExpired));

		$set->setField('EOFFICE_JABATAN_ID', ValToNullDB($reqEofficeJabatanId));
		$set->setField('EOFFICE_JABATAN_NAMA', $reqEofficeJabatanNama);
		$set->setField('EOFFICE_SATUAN_KERJA_ID', ValToNullDB($reqEofficeSatkerId));
		$set->setField('EOFFICE_SATUAN_KERJA_NAMA', $reqEofficeSatkerNama);
		$set->setField('TMT_SPMT', dateToDBCheck($reqTmtSpmt));
	...
	function jabatandiklatstruktural() 
	{
		...
		$statement= " AND A.TIPE_PEGAWAI_ID = 11 AND (COALESCE(NULLIF(A.STATUS, ''), NULL) IS NULL OR A.STATUS = '2')";

		$arrdatainfo=[];
		if(empty($reqId) || empty($reqTanggal))
		{
			$statement= " AND 1=2";
		}
		else
		{
			$reqTanggal= dateToPageCheck($reqTanggal);

			$statement.= " AND A.PEGAWAI_ID = ".$reqId."
			AND TO_DATE(TO_CHAR(COALESCE(A.TMT_SELESAI_JABATAN, NOW()), 'YYYY-MM-DD'), 'YYYY-MM-DD') >= TO_DATE('".$reqTanggal."', 'YYYY-MM-DD')
			AND TO_DATE(TO_CHAR(A.TMT_JABATAN, 'YYYY-MM-DD'), 'YYYY-MM-DD') <= TO_DATE('".$reqTanggal."', 'YYYY-MM-DD')
			";
			$set= new JabatanRiwayat();
			$set->selectdata(array(), -1, -1, $statement);
			// echo $set->query;exit;
			$set->firstRow();
			$infoid= $set->getField("JABATAN_RIWAYAT_ID");

			if(!empty($infoid))
			{
				$arrdata= [];
				$arrdata["infoid"]= $infoid;
				$arrdata["infonama"]= $set->getField("NAMA");
				$arrdata["infoeselonnama"]= $set->getField("ESELON_NAMA");
				$arrdata["infosatkernama"]= $set->getField("SATKER_NAMA");
				$arrdata["inforumpunid"]= $set->getField("RUMPUN_ID");
				$arrdata["inforumpunnama"]= $set->getField("RUMPUN_NAMA");
