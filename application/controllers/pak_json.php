...
class pak_json extends CI_Controller {
	...
	function add()
	{
		...
		$reqCheckPakAwal2= $this->input->post('reqCheckPakAwal');
		if(empty($reqCheckPakAwal2))
		{
			echo "xxx-".'Belum ada pilihan Pertama / Integrasi / Konversi ';
			exit;
		}
		$reqJabatanFtId= $this->input->post('reqJabatanFtId');
		if($reqCheckPakAwal2 !='1' && empty($reqJabatanFtId)){
			echo "xxx-".'Belum ada pilihan Nama Jabatan Fungsional Yang Diusulkan ';
			exit;
		}
		...
		// $reqCheckPakIntegrasi = $this->input->post('reqCheckPakIntegrasi');
		// $reqCheckPakKonversi = $this->input->post('reqCheckPakKonversi');
		// $reqCheckPakAwal= $this->input->post('reqCheckPakAwal');	
		$reqCheckPakIntegrasi =  $reqCheckPakKonversi=$reqCheckPakAwal='';

		if($reqCheckPakAwal2=='1'){
			$reqCheckPakAwal='1';
		}else if($reqCheckPakAwal2=='2'){
			$reqCheckPakIntegrasi ='1';
		}else if($reqCheckPakAwal2=='3'){
			$reqCheckPakKonversi='1';
		}
		...
		$set->setField('BULAN_MULAI', ValToNullDB(getMonth($reqTglMulai)));	
		$set->setField('BULAN_SELESAI', ValToNullDB(getMonth($reqTglSelesai)));	
		$set->setField('TAHUN_MULAI', ValToNullDB(getYear2($reqTglMulai)));	
		$set->setField('TAHUN_SELESAI', ValToNullDB(getYear2($reqTglSelesai)));	

		$set->setField('PAK_INTERGRASI', $reqCheckPakIntegrasi);
		$set->setField('PAK_KONVERSI', $reqCheckPakKonversi);
	...
	{	
		...
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PAK_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PAK_ID");
		...
		$arrayWhere = array("PAK_ID" => $riwayatId);
	...
	function combokenaikanpangkat()
	{
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id']= $set->getField("PAK_ID");
			$arr_json[$i]['text']= $set->getField("NO_SK");
			$arr_json[$i]['tanggal']= dateToPageCheck($set->getField('TANGGAL_MULAI'));
			$arr_json[$i]['total']= $set->getField("TOTAL_KREDIT");
			$i++;
		}
