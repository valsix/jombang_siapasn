...
class pendidikan_riwayat_json extends CI_Controller {
	...
	function add()
	{
		...
		$set->setField('STATUS_SK_DASAR_PENGAKUAN', ValToNullDB($reqSkDasarPengakuan));
		$set->setField('CANTUM_GELAR_TANGGAL', dateToDBCheck($reqCantumGelarTgl));
		$set->setField('CANTUM_GELAR_NO_SK', $reqCantumGelarNoSk);
		$set->setField('DASAR_PANGKAT_RIWAYAT_ID', ValToNullDB($reqDasarPangkatRiwayatId));
		$set->setField('NILAI_REKAM_JEJAK', ValToNullDB($reqNilaiKualifikasi));
		if(empty($reqRumpunJabatan))
			$reqRumpunJabatan= "NULL";
		$set->setField('RUMPUN_ID', $reqRumpunJabatan);
	...
	function cari_pegawai_usulan()
	{
		...
		// khusus pns saja
		$statement.= " AND A.STATUS_PEGAWAI_ID IN (2)";
		...
		(
						SELECT PEGAWAI_ID
						FROM persuratan.SURAT_MASUK_PEGAWAI WHERE JENIS_ID = 1
						AND SATUAN_KERJA_PEGAWAI_USULAN_ID IN (".$reqSatuanKerjaId.")
						AND (STATUS_BERKAS NOT IN (10) OR STATUS_BERKAS IS NULL OR (STATUS_BERKAS = 10 AND (STATUS_SURAT_KELUAR NOT IN (3) OR STATUS_SURAT_KELUAR IS NULL )))
		)
	...
	function cari_pegawai_karsu_usulan()
	{
		...
		/*  PEMBATASAN PENCARIAN HANYA NIP YANG BELUM PERNAH USUL
		$statement.= "
		AND A.PEGAWAI_ID NOT IN 
		(

			SELECT PEGAWAI_ID 
			FROM persuratan.SURAT_MASUK_PEGAWAI WHERE JENIS_ID = 3
			AND SATUAN_KERJA_PEGAWAI_USULAN_ID IN (".$reqSatuanKerjaId.")
			AND (STATUS_BERKAS NOT IN (10) OR STATUS_BERKAS IS NULL OR (STATUS_BERKAS = 10 AND (STATUS_SURAT_KELUAR NOT IN (3) OR STATUS_SURAT_KELUAR IS NULL )))
	...
	function cari_pegawai_pensiun_usulan()
	{
		...
		(
			SELECT PEGAWAI_ID
			FROM persuratan.SURAT_MASUK_PEGAWAI WHERE JENIS_ID = ".$reqJenis."
			AND SATUAN_KERJA_PEGAWAI_USULAN_ID IN (".$reqSatuanKerjaId.")
			AND (STATUS_BERKAS NOT IN (10) OR STATUS_BERKAS IS NULL OR (STATUS_BERKAS = 10 AND (STATUS_SURAT_KELUAR NOT IN (3) OR STATUS_SURAT_KELUAR IS NULL )))
		";

		// if($reqKategori == "meninggal")
		// $arrkategori= array("meninggal", "pensiundini", "pensiunudzur");
		$arrkhususkategori= array("dini", "udzur");
		$arrkategori= array("meninggal");
		if(in_array($reqKategori, $arrkategori))
		{
			$statement.= " AND KATEGORI = '".$reqKategori."'";
		}
		...
		while($set->nextRow())
		{
			...
			$arr_json[$i]['pensiuntmttahun'] = getYear($set->getField("PENSIUN_TMT"));
	...
	function cari_pegawai_kenaikan_pangkat_reguler()
	{
		...
		// STATUS_BERKAS NOT IN (10) OR 
		$statement.= "
		AND A.PEGAWAI_ID NOT IN
		(
			SELECT PEGAWAI_ID
			FROM persuratan.SURAT_MASUK_PEGAWAI WHERE JENIS_ID = 10
			AND SATUAN_KERJA_PEGAWAI_USULAN_ID IN (".$reqSatuanKerjaId.")
			AND 
			(
				STATUS_BERKAS IS NULL 
			)
		)";
		// OR 
		// (
		// 	STATUS_BERKAS = 10 AND 
		// 	(STATUS_SURAT_KELUAR NOT IN (3) OR STATUS_SURAT_KELUAR IS NULL )
		// )
		...
		if($reqKpJenis == "kpreguler" || $reqKpJenis == "kppi")
			$statement.= " AND JR.JENIS_JABATAN_ID = '2'";
		elseif($reqKpJenis == "kpbtugas" || $reqKpJenis == "kpstugas")
			$statement.= " AND JR.JENIS_JABATAN_ID IN ('2', '3')";
		...
		while($set->nextRow())
		{
			...
			$arr_json[$i]['jabatantambahanid'] = $set->getField("JABATAN_TAMBAHAN_ID");
	...
	function combokenaikanpangkat()
	{
		...
		$reqId= $this->input->get("reqId");
		$reqStatusPendidikan= $this->input->get("reqStatusPendidikan");

		if(empty($reqStatusPendidikan))
			$reqStatusPendidikan= 3;

		$set = new PendidikanRiwayat();
		$statement= " AND A.STATUS_PENDIDIKAN = '".$reqStatusPendidikan."' AND A.PEGAWAI_ID = ".$reqId;
	...
	function auto() 
	{
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id'] = $set->getField("PENDIDIKAN_ID");
			$arr_json[$i]['label'] = $set->getField("NAMA");
			$arr_json[$i]['desc'] = $set->getField("NAMA");
			$arr_json[$i]['nilai_rumpun'] = $set->getField("NILAI_RUMPUN");
			$i++;
		}
		echo json_encode($arr_json);		
