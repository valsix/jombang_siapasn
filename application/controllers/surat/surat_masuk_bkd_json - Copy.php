...
class surat_masuk_bkd_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumnsAlias = array("NOMOR", "TANGGAL", "TOTAL_PEGAWAI_PROGRES_KEMBALI", "SATUAN_KERJA_ASAL_NAMA", "POSISI_TERAKHIR", "AKSI", "SURAT_MASUK_BKD_ID");
	...
	function json_surat() 
	{
		...
		{
			$searchJson = "  AND (UPPER(A.NOMOR) LIKE '%".strtoupper($tempSearch)."%' OR UPPER(A.PERIHAL) LIKE '%".strtoupper($tempSearch)."%'
	...
	function set_info_pegawai_data()
	{
		...
		$tempInfo='
			<div class="row">
				...
				<div class="col s9 m11">'.$reqNomor.'</div>
			</div>
			<div class="row">
