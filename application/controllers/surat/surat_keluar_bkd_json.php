...
class surat_keluar_bkd_json extends CI_Controller {
	...
	function getinfototal() 
	{
		...
		$reqSatuanKerjaId= $this->input->get("reqSatuanKerjaId");
		$reqTahun= $this->input->get("reqTahun");
		$reqStatusSuratMasuk= $this->input->get("reqStatusSuratMasuk");

		$this->load->library('globalfilepegawai');
		$vfpeg= new globalfilepegawai();

		$arrparamsatuankerja= ["reqSatuanKerjaId"=>$reqSatuanKerjaId];
		$statementparamsatuankerja= $vfpeg->paramsatuankerja($arrparamsatuankerja);
		// echo $statementparamsatuankerja;exit;
		$statement.= $statementparamsatuankerja;

		if(!empty($reqJenis))
			$statement.= " AND SMP.JENIS_ID = ".$reqJenis;

		/*if(!empty($reqStatusSuratMasuk))
			$statement.= " AND SMP.INFO_STATUS_KIRIM = ".$reqStatusSuratMasuk;*/

		if(!empty($reqTahun))
		{
			$vwhere= "TO_CHAR(COALESCE(SMU.TANGGAL, SMB.TANGGAL), 'YYYY')";
			if($reqJenis == 10)
				$vwhere= "COALESCE(SMU.PENGATURAN_KENAIKAN_PANGKAT_ID, SMB.PENGATURAN_KENAIKAN_PANGKAT_ID)";
			
			$statement.= " AND ".$vwhere." = '".$reqTahun."'";
		}

		$set= new SuratMasukPegawai();
		$set->selectByParamsInfoTotal(array(), -1,-1, $statement);
		$set->firstRow();
		// echo $set->query;exit();
		$infojumlahturunstatus= $set->getField("INFOJUMLAHTURUNSTATUS");
		$infojumlahkirim= $set->getField("INFOJUMLAHKIRIM");
		$infojumlahterima= $set->getField("INFOJUMLAHTERIMA");
		$infojumlahterverifikasi= $set->getField("INFOJUMLAHTERVERIFIKASI");
		
		$arrData= array("infojumlahturunstatus"=>$infojumlahturunstatus, "infojumlahkirim"=>$infojumlahkirim, "infojumlahterima"=>$infojumlahterima, "infojumlahterverifikasi"=>$infojumlahterverifikasi);
	...
	function json_pegawai()
	{
		...
		$aColumns = array("NIP_BARU", "NAMA_LENGKAP", "SATUAN_KERJA_INDUK", "PROSES_STATUS", "PROSES_TANGGAL", "SURAT_MASUK_PEGAWAI_ID", "SURAT_MASUK_UPT_ID", "SURAT_MASUK_BKD_ID", "INFO_LINK", "INFO_LINK_DETIL");
		...
		$arrparamjenis= ["reqJenis"=>$reqJenis];
		$arrlinkdetil= $vfpeg->paramlinkdetil($arrparamjenis);
		// print_r($arrlinkdetil);exit;
		$vlinkdetil= $arrlinkdetil["linkdetil"];
		...
		if(!empty($reqStatusSuratMasuk))
		{
			if($reqStatusSuratMasuk == -1)
				$statement.= " AND SMP.INFO_STATUS_KIRIM = 0";
			else
				$statement.= " AND SMP.INFO_STATUS_KIRIM = ".$reqStatusSuratMasuk;
		}
		...
		{
			$tempStatusKirimTeknis= $set->getField("STATUS_KIRIM_TEKNIS");
			$infostatuskembali= $set->getField("STATUS_KEMBALI");
			$infosuratmasukpegawaiid= $set->getField("SURAT_MASUK_PEGAWAI_ID");
			$infosuratmasukuptid= $set->getField("SURAT_MASUK_UPT_ID");
			$infosuratmasukbkdid= $set->getField("SURAT_MASUK_BKD_ID");
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				...
				else if ($aColumns[$i] == "INFO_LINK")
				{
					$infolinkdetil= $vlinkdetil;
					if(!empty($infolinkdetil))
					{
						$infolinkdetil= $vlinkdetil."?reqJenis=".$reqJenis;
						if($vlinkdetil == "surat_masuk_upt_add")
							$infolinkdetil.= "&reqId=".$infosuratmasukuptid;
						else
							$infolinkdetil.= "&reqId=".$infosuratmasukbkdid;
					}
					$row[] = $infolinkdetil;
				}
				else if ($aColumns[$i] == "INFO_LINK_DETIL")
				{
					$arrparamjenis= ["reqJenis"=>$reqJenis, "vlinkdetil"=>$vlinkdetil, "infostatuskembali"=>$infostatuskembali];
					$arrparamjenis= $vfpeg->paramlinksubdetil($arrparamjenis);

					if(!empty($arrparamjenis))
					{
						$arrparamjenis.= ";".$infosuratmasukpegawaiid;
						$arrparamjenis= ["reqdata"=>$arrparamjenis, "reqkunci"=>$enkripdekripkunci];
						$arrparamjenis= $vfpeg->enkripdata($arrparamjenis);
					}
					$row[] = $arrparamjenis;
				}
				else
	...
	function surat_keluar_kgb_nomor()
	{
		...
		$set->setField('IS_SEDIAAN', ValToNullDB($reqIsSediaan));
	...
	function add()
	{
		...
		$reqPerihal= $this->input->post("reqPerihal");
		$reqIsManual= $this->input->post("reqIsManual");
		$reqIsSediaan= $this->input->post("reqIsSediaan");
	...
	function json_surat_sediaan()
	{
		...
		$aColumns = array("NOMOR_AGENDA", "TANGGAL_TERIMA", "TANGGAL_SURAT", "NOMOR_SURAT", "PERIHAL", "AKSI", "STATUS_KIRIM", "SATUAN_KERJA_ID_AKSI", "SURAT_KELUAR_BKD_SEDIAAN_ID", "SURAT_KELUAR_BKD_ID");
		$aColumnsAlias = $aColumns;
		...
		// $searchJson = "  AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')";
		$allRecord = $set->getCountByParamsSediaan(array(), $statement);
		// echo $allRecord;

		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $set->getCountByParamsSediaan(array(), $statement.$searchJson);
		// echo $sOrder;exit();
		$set->selectByParamsSediaan(array(), $dsplyRange, $dsplyStart, $statement.$searchJson, $sOrder);
		// echo $set->query;exit;
		/* Output */
		$output = array(
			"sEcho" => intval($_GET['sEcho']),
		...
		while($set->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				$infokolom= $aColumns[$i];
				if($infokolom == "TANGGAL_TERIMA" || $infokolom == "TANGGAL_SURAT")
					$row[] = getFormattedDate($set->getField($infokolom));
				else if($aColumns[$i] == "AKSI")
				{
					// ASAL_SURAT,
					$temp= "";
					$temp= $set->getField("INFO_STATUS");
					$row[] = $temp;
	...
	function statussediaankirim()
	{
		...
		$set->setField("SURAT_KELUAR_BKD_ID", $reqId);
		// $set->setField("PERIHAL", setQuote($tempPerihalInfo));
		$set->setField("STATUS_KIRIM", ValToNullDB($reqStatusBerkas));
		$set->setField("LAST_LEVEL", $this->LOGIN_LEVEL);
		$set->setField("LAST_USER", $this->LOGIN_USER);
		$set->setField("USER_LOGIN_ID", $this->LOGIN_ID);
		$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
	...
	function statusterima()
	{
		...
		$set->setField("SURAT_KELUAR_BKD_SEDIAAN_ID", $reqRowId);
		$set->setField("LAST_LEVEL", $this->LOGIN_LEVEL);
		$set->setField("LAST_USER", $this->LOGIN_USER);
		$set->setField("USER_LOGIN_ID", $this->LOGIN_ID);
		$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
		
		if($set->updatesediaanterimaberkas())
		{
			$arrJson["PESAN"] = "Terima berkas, data berhasil di simpan.";
		}
		else
		{
			// echo $set->query;exit();
			$arrJson["PESAN"] = "Terima berkas, data gagal di simpan.";
		}
		echo json_encode($arrJson);
	}

	function addteruskan()
	{
		$this->load->model('persuratan/SuratKeluarBkd');
		...
		$reqStatusDisposisi= $this->input->post("reqStatusDisposisi");
		$reqIsi= $this->input->post("reqIsi");
		$reqTanggalDisposisi= $this->input->post("reqTanggalDisposisi");
		$reqSatuanKerjaTujuanId= $this->input->post("reqSatuanKerjaTujuanId");
		$reqSatuanKerjaTujuanNama= $this->input->post("reqSatuanKerjaTujuanNama");

		$set->setField("ISI", setQuote($reqIsi));
		$set->setField("SATUAN_KERJA_TUJUAN_ID", ValToNullDB($reqSatuanKerjaTujuanId));
		$set->setField("SATUAN_KERJA_TUJUAN_NAMA", $reqSatuanKerjaTujuanNama);
		$set->setField("TANGGAL_DISPOSISI", dateToDBCheck($reqTanggalDisposisi));
		$set->setField("TERDISPOSISI", $reqStatusDisposisi);
		...
		if($simpan == "1")
		{
			$inforeturn= "-Data berhasil";

			if($reqStatusDisposisi == "2")
				$inforeturn.= "di revisi";
			else
				$inforeturn.= "di teruskan";
