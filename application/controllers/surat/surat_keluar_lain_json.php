...
class surat_keluar_lain_json extends CI_Controller {
	...
	function json_surat()
	{
		$this->load->model('persuratan/SuratKeluarLain');

		$set = new SuratKeluarLain();
		...
		$aColumns = array("NOMOR", "TANGGAL", "PERIHAL", "SATUAN_KERJA_LENGKAP", "AKSI", "PERIODE_BULAN", "PERIODE_TAHUN", "SURAT_MASUK_PEGAWAI_ID", "JENIS_ID", "SURAT_KELUAR_BKD_ID");
		$aColumnsAlias = array("NOMOR", "TANGGAL", "PERIHAL", "SATUAN_KERJA_LENGKAP", "AKSI", "PERIODE_BULAN", "PERIODE_TAHUN", "SURAT_MASUK_PEGAWAI_ID", "JENIS_ID", "SURAT_KELUAR_BKD_ID");
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				...
				else if($aColumns[$i] == "AKSI")
				{
					...
					else
					{
						...
						{
						$row[] = $temp.'<a href="javascript:void(0)" onclick="hapusdata(\''.$set->getField("SURAT_MASUK_PEGAWAI_ID").'\')" class="round waves-effect waves-light red white-text" title="Hapus" ><i class="mdi-action-delete"></i></a>';
						}
						else
						$row[] = '';
					}
	...
	function json_surat_keluar_kgb()
	{
		...
		while($set->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				...
				{
					if($reqStatusPilih == "1")
					$row[] = '<a href="javascript:void(0)" onclick="pilihdata(\''.$set->getField("SURAT_KELUAR_BKD_ID").'\')" class="round waves-effect waves-light red white-text" title="Pilih" >Pilih Data</a>';
					else
					$row[] = $set->getField($aColumns[$i]);
				}
				else if($aColumns[$i] == "AKSI")
	...
	function statuskirim()
	{
		...
		$set->setField("SURAT_KELUAR_BKD_ID", $reqId);
		$set->setField("PERIHAL", $tempPerihalInfo);
