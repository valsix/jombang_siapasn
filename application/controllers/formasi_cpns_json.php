...
class formasi_cpns_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("NAMA", "LINK_URL_INFO", "FORMASI_CPNS_ID");
		$aColumnsAlias = array("NAMA", "LINK_URL_INFO", "FORMASI_CPNS_ID");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == " ORDER BY FORMASI_CPNS_ID asc " )
			{
				/*
				...
				*/
				$sOrder = " ORDER BY FORMASI_CPNS_ID ASC ";
	...
	function add()
	{
		...
		{
			$set->setField('FORMASI_CPNS_ID', $reqId); 
			$set->setField("LAST_USER", $this->LOGIN_USER);
			$set->setField("USER_LOGIN_ID", $this->LOGIN_ID);
			$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
			$set->setField("LAST_DATE", "NOW()");
	...
	function combo() 
	{
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id'] = $set->getField("FORMASI_CPNS_ID");
	...
	function log() 
	{
		$this->load->model('FormasiCpnsLog');

		$set = new FormasiCpnsLog();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "FORMASI_CPNS_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "FORMASI_CPNS_ID");
