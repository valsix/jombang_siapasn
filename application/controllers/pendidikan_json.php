...
class pendidikan_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("NAMA", "PANGKAT_MINIMAL", "PANGKAT_MAKSIMAL", "NO_URUT", "LINK_URL_INFO", "PENDIDIKAN_ID");
		$aColumnsAlias = array("NAMA", "PANGKAT_MINIMAL", "PANGKAT_MAKSIMAL", "NO_URUT", "LINK_URL_INFO", "PENDIDIKAN_ID");
		...
		$sOrder= " ORDER BY A.PENDIDIKAN_ID ASC ";
	...
	function add()
	{
		...
		$reqPangkatMinimal= $this->input->post("reqPangkatMinimal");
		$reqPangkatMaksimal= $this->input->post("reqPangkatMaksimal");
		$reqNoUrut= $this->input->post("reqNoUrut");
		
		$set->setField('NAMA', $reqNama);
		$set->setField('PANGKAT_MAKSIMAL', $reqPangkatMaksimal);
		$set->setField('PANGKAT_MINIMAL', $reqPangkatMinimal);
		$set->setField('NO_URUT', $reqNoUrut);
	...
	function option() 
	{
		...
		$reqPendidikanMinimalId= $this->input->get('reqPendidikanMinimalId');
		$set = new Pendidikan();
		
		if($reqPendidikanMinimalId == ""){}
		else
		$statement= " AND A.PENDIDIKAN_ID >= ".$reqPendidikanMinimalId;
		...
		while($set->nextRow())
		{
			$arrID[$i] = $set->getField("PENDIDIKAN_ID");
	...
	function log() 
	{	
		$this->load->model('PendidikanLog');

		$set = new PendidikanLog();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PENDIDIKAN_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "PENDIDIKAN_ID");
