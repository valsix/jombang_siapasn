...
class golongan_pppk_gaji_json extends CI_Controller {
	...
	function json() 
	{
		$this->load->model('GolonganPppkGaji');

		$set = new GolonganPppkGaji();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("MASA_KERJA", "KODE", "NAMA", "LINK_URL_INFO", "GOLONGAN_PPPK_GAJI_ID");
		$aColumnsAlias = array("MASA_KERJA", "KODE", "NAMA", "LINK_URL_INFO", "GOLONGAN_PPPK_GAJI_ID");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY NAMA desc" )
			{
				...
				*/
				$sOrder = " ORDER BY GOLONGAN_PPPK_GAJI_ID ASC ";
	...
	{
		$this->load->model('GolonganPppkGaji');
		$this->load->model('GolonganPppkGajiDetil');
		...
		{
			$set->setField('GOLONGAN_PPPK_GAJI_ID', $reqId); 
			if($set->update())
			{				
				$tempSimpan=1;
			}
		...
		if($tempSimpan==1)
		{
			...
			{
				$set_data = new GolonganPppkGajiDetil();
				$set_data->setField('GOLONGAN_PPPK_GAJI_ID', $reqId);
				...
				{
					$set_data->setField('GOLONGAN_PPPK_GAJI_DETIL_ID', $reqRowDetilId[$i]);
					$set_data->update();
				}
			}
	...
	function combo() 
	{
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id'] = $set->getField("GOLONGAN_PPPK_GAJI_ID");
	...
	function gajipokok() 
	{
		...
		$reqGolonganPppkId= $this->input->get("reqGolonganPppkId");
		$reqMasaKerja= $this->input->get("reqMasaKerja");
		$reqTglSk= $this->input->get("reqTglSk");
		
		$tempPeriode= str_replace("-","",$reqTglSk);
		$tempGolonganPppkGaji= $set->getCountByParamsGajiBerlaku($tempPeriode, $reqMasaKerja, $reqGolonganPppkId);
		if($tempGolonganPppkGaji == "")
		$tempGolonganPppkGaji= 0;
		//echo $set->query;exit;
		echo $tempGolonganPppkGaji;
	}	

	function log() 
	{	
		$this->load->model('GolonganPppkGajiLog');

		$set = new GolonganPppkGajiLog();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "GOLONGAN_PPPK_GAJI_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "GOLONGAN_PPPK_GAJI_ID");
