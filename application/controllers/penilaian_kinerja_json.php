...
class penilaian_kinerja_json extends CI_Controller {
	...
	function comboNilaiKuadran(){
		...
		$nilai_kuadran->selectByParams(array("A.KODE"=>$reqId));
	...
	function bentukQr(){
		...
		$qrcodegenerators->generateQr($reqdata);	
	}
	...
	function add()
	{
		...
		// CHECK UNIX KESAMAAN TAHUN , PERIODE, TRIWULAN -- TIDAK BOLEH ADA DUPLICATE
		$statement= " AND A.TAHUN = ".ValToNullDB($reqTahun)." AND A.TRIWULAN=".ValToNullDB($reqTriwulan);
		$statementLevel= " AND (COALESCE(NULLIF(A.STATUS, ''), NULL) IS NULL OR A.STATUS = '2')";
		$set->selectByParams(array("A.PEGAWAI_ID"=>$reqId),-1,-1,$statement.$statementLevel);
		$arrPenilaian = $set->rowResult;
		$arrPenilaian = array_change_key_case_recursive_upper($arrPenilaian);
		$totalRecords = $set->rowCount;
		$vReqPenilaiSkpId= in_array_column($reqRowId, "PENILAIAN_SKP_DETIL_ID", $arrPenilaian);
		$indexdata= $vReqPenilaiSkpId[0];
        $arrResult = $arrPenilaian[$indexdata];
        $reqPenilaiSkpId = $arrResult['PENILAIAN_SKP_DETIL_ID'];
		$reqStatusFaield = false;
		if($totalRecords > 0   ){
			$reqStatusFaield = true;
			if($reqPenilaiSkpId ==$reqRowId){
				$reqStatusFaield = false;
			}
		}
		// TRIWULAN TIBAK BOLEH KOSONG
		if(empty($reqTriwulan)){
			$reqStatusFaield = true;
		}
		...
		{
			$set->setField('PENILAIAN_SKP_DETIL_ID', $reqRowId); 
			if($set->update())
			{
				$infosimpan= "1";
			}
	...
	function delete()
	{
		...
		$set->setField("LAST_LEVEL", $this->LOGIN_LEVEL);
		$set->setField("LAST_USER", $this->LOGIN_USER);
		$set->setField("USER_LOGIN_ID", ValToNullDB($this->LOGIN_ID));
		$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
		$set->setField("LAST_DATE", "NOW()");
		$set->setField("PENILAIAN_SKP_DETIL_ID", $reqId);
