...
class jenis_pengadaan_json extends CI_Controller {
	...
	function json() 
	{
		$this->load->model('JenisPengadaan');

		$set = new JenisPengadaan();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("NAMA", "LINK_URL_INFO", "JENIS_PENGADAAN_ID");
		$aColumnsAlias = array("NAMA", "LINK_URL_INFO", "JENIS_PENGADAAN_ID");
	...
	function add()
	{
		...
		{
			$set->setField('JENIS_PENGADAAN_ID', $reqId); 
			$set->setField("LAST_USER",  $this->LOGIN_USER);
			$set->setField("USER_LOGIN_ID", $this->LOGIN_ID);
			$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
			$set->setField("LAST_DATE", "NOW()");
	...
	function delete()
	{
		...
		elseif($reqMode == "jenis_pengadaan_1")
		{
	...
	function combo() 
	{
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id'] = $set->getField("JENIS_PEGAWAI_ID");
	...
	function log() 
	{
		$this->load->model('JenisPengadaanLog');

		$set = new JenisPengadaanLog();

		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);

		$columnsShow = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "JENIS_PENGADAAN_ID");
		...
		$aColumnsAlias = $columnsShow;
			...
			if ( isset( $_GET['iSortCol_0'] ) )
			{
				...
				if ( trim($sOrder) == "ORDER BY INFO_LOG desc" )
				{
					...
					* another row will not be displayed at all.
					*/
					$sOrder = " ORDER BY LAST_DATE desc ";
