...
class jenis_pegawai_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("NAMA", "LINK_URL_INFO", "JENIS_PEGAWAI_ID");
		$aColumnsAlias = array("NAMA", "LINK_URL_INFO", "JENIS_PEGAWAI_ID");
	...
	function add()
	{
		...
		{
			$set->setField('JENIS_PEGAWAI_ID', $reqId); 
			$set->setField("LAST_USER",  $this->LOGIN_USER);
			$set->setField("USER_LOGIN_ID", $this->LOGIN_ID);
			$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
			$set->setField("LAST_DATE", "NOW()");
	...
	function delete()
	{
		...
		elseif($reqMode == "jenis_pegawai_1")
		{
	...
	function log() 
	{
		$this->load->model('JenisPegawaiLog');

		$set = new JenisPegawaiLog();

		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);

		$columnsShow = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "JENIS_PEGAWAI_ID");
