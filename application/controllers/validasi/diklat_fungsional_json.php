...
class diklat_fungsional_json extends CI_Controller {
	...
	function add()
	{
		$this->load->model('validasi/DiklatFungsional');
	...
	function delete()
	{
		...
		$set->setField("USER_LOGIN_PEGAWAI_ID", ValToNullDB($this->LOGIN_PEGAWAI_ID));
		$set->setField("LAST_DATE", "NOW()");
		$set->setField("DIKLAT_FUNGSIONAL_ID", $reqId);
		
		if($reqMode == "diklat_fungsional_0")
		{
		...
		elseif($reqMode == "diklat_fungsional_1")
		{
	...
	{	
		$this->load->model('DiklatFungsionalLog');

		$set = new DiklatFungsionalLog();
		
		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);
		
		
		$aColumns = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "DIKLAT_FUNGSIONAL_ID");
		$aColumnsAlias = array("INFO_LOG", "LAST_USER", "LAST_DATE", "STATUS_NAMA", "DIKLAT_FUNGSIONAL_ID");
		...
		$arrayWhere = array("DIKLAT_FUNGSIONAL_ID" => $riwayatId);
