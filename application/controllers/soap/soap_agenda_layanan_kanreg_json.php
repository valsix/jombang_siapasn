...
class soap_agenda_layanan_kanreg_json extends CI_Controller {
	...
	function json() 
	{
		$this->load->model('soap/SoapAgenda');

		$set = new SoapAgenda();
		$reqBidang= $this->input->get("reqBidang");

		// $set->selectByParamsInfo();
		// // $set->selectByParamsDetilInfo("EF19467");
		// echo $set->selectCount()."<br/>";
		// while($set->nextRow())
		// {
		// 	// echo $set->getField("NOAGENDA");
		// 	echo $set->getField("NIP");
		// }
		// exit();
		...
		$aColumns = array("NOAGENDA", "NOUSUL", "TGLPPT", "PEL", "NOAGENDA");
		...
		$set->selectByParamsInfo($reqBidang);
		$allRecord = $set->selectCount($reqBidang);
		// echo $allRecord;
	...
	function json_detil() 
	{
		...
		$reqAgenda= $this->input->get("reqAgenda");

		$aColumns = array("NIP", "STATUS", "KET");
		$aColumnsAlias = $aColumns;
		...
		$set->selectByParamsDetilInfo($reqAgenda);
		$allRecord = $set->selectCount();
		// echo $allRecord;
