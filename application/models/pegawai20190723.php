	...
	function selectByParamsCetakPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order=' ORDER BY A.PEGAWAI_ID ASC')
	{
		...
		SELECT
			...
			ELSE
				...
				END
			END TMT_PENSIUN,
			(CASE WHEN A.PENDIDIKAN_RIWAYAT_ID IS NULL THEN NULL ELSE F.PENDIDIKAN_NAMA END) || (CASE WHEN COALESCE(NULLIF(F.PENDIDIKAN_JURUSAN_NAMA,'') , NULL ) IS NULL THEN NULL ELSE ' - ' || F.PENDIDIKAN_JURUSAN_NAMA END) PENDIDIKAN,
