  ...
  ***/
  ...
  class SuratKeluarBkd extends Entity{ 
    ...
    **/
	...
	function update()
	{
		...
		SET
			...
			, IS_SEDIAAN= ".$this->getField("IS_SEDIAAN")."
    ...
    function updatesediaanterimaberkas()
	{
		$str = "		
		...
		SET
			...
			, USER_LOGIN_ID= ".$this->getField("USER_LOGIN_ID")."
			, USER_LOGIN_PEGAWAI_ID= ".$this->getField("USER_LOGIN_PEGAWAI_ID")."
		WHERE SURAT_KELUAR_BKD_SEDIAAN_ID = ".$this->getField("SURAT_KELUAR_BKD_SEDIAAN_ID")."
		";
		$this->query = $str;
		// echo $str;exit;
		return $this->execQuery($str);
    ...
    function updatesediaanteruskan()
	{
		$str = "		
		UPDATE PERSURATAN.SURAT_KELUAR_BKD_SEDIAAN
		SET
    ...
    **/ 
    ...
    function selectByParamsUptDinasKpTahun($statement='',$order="ORDER BY B.TANGGAL_PERIODE DESC")
	{
		$str = "
		SELECT A.PENGATURAN_KENAIKAN_PANGKAT_ID, B.TANGGAL_PERIODE
		FROM
		(
			SELECT
			COALESCE(SMU.PENGATURAN_KENAIKAN_PANGKAT_ID, SMB.PENGATURAN_KENAIKAN_PANGKAT_ID) PENGATURAN_KENAIKAN_PANGKAT_ID
			, SMP.JENIS_ID
			FROM persuratan.SURAT_MASUK_PEGAWAI SMP
			LEFT JOIN persuratan.SURAT_MASUK_UPT SMU ON SMP.SURAT_MASUK_UPT_ID = SMU.SURAT_MASUK_UPT_ID
			LEFT JOIN persuratan.SURAT_MASUK_BKD SMB ON SMP.SURAT_MASUK_BKD_ID = SMB.SURAT_MASUK_BKD_ID
			WHERE 1=1
			GROUP BY COALESCE(SMU.PENGATURAN_KENAIKAN_PANGKAT_ID, SMB.PENGATURAN_KENAIKAN_PANGKAT_ID), SMP.JENIS_ID
		) A
		INNER JOIN PENGATURAN_KENAIKAN_PANGKAT B ON A.PENGATURAN_KENAIKAN_PANGKAT_ID = B.PENGATURAN_KENAIKAN_PANGKAT_ID
		WHERE 1=1
		"; 
	...
	{
		/*A.SURAT_KELUAR_BKD_ID, A.JENIS_ID, A.SURAT_MASUK_BKD_ID, A.SURAT_MASUK_UPT_ID, 
		A.SURAT_MASUK_PEGAWAI_ID, A.PERIODE
		, A.NOMOR_AWAL, A.NOMOR_URUT, A.KLASIFIKASI_ID,
		A.TANGGAL, A.KEPADA, A.PERIHAL, A.IS_MANUAL, A.SATUAN_KERJA_TUJUAN_ID, 
		A.SATUAN_KERJA_TUJUAN_NAMA, A.SATUAN_KERJA_TEKNIS_ID, A.SATUAN_KERJA_TERIMA_SURAT_ID, 
		...
		*/
		...
		SELECT
			A.*
			, SUBSTR(A.PERIODE,1,2) PERIODE_BULAN, SUBSTR(A.PERIODE,3,4) PERIODE_TAHUN
			...
			, B.KODE KLASIFIKASI_KODE, B.NAMA KLASIFIKASI_NAMA
		...
		(
			...
			WHERE 1 = 1
		) TTD ON TTD.TANDA_TANGAN_BKD_ID = (SELECT AMBIL_TANDA_TANGAN_BKD_TGL(COALESCE(A.TANGGAL, CURRENT_DATE)))
		WHERE 1=1
		"; 
    ...
    function selectByParamsSediaan($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order=' ORDER BY A.TANGGAL DESC')
	{
		$str = "
		SELECT
			A.SURAT_KELUAR_BKD_ID, B1.SURAT_KELUAR_BKD_SEDIAAN_ID, A.STATUS_KIRIM, B1.TERBACA, B1.TERDISPOSISI, B1.TANGGAL_DISPOSISI
			, B1.ISI, B1.SATUAN_KERJA_ASAL_NAMA, A.SATUAN_KERJA_TUJUAN_ID SATUAN_KERJA_TUJUAN_AKHIR_ID
			, B2.SATUAN_KERJA_SEDIAAN SATUAN_KERJA_TUJUAN_ID, AMBIL_SATKER_NAMA(B2.SATUAN_KERJA_SEDIAAN) SATUAN_KERJA_TUJUAN_NAMA
			, A.NOMOR_AWAL NOMOR_AGENDA, A.TANGGAL TANGGAL_SURAT, A.NOMOR NOMOR_SURAT, A.PERIHAL
			, AMBIL_SATKER_NAMA(A.SATUAN_KERJA_TERIMA_SURAT_ID) ASAL_SURAT, B1.TANGGAL TANGGAL_TERIMA
			,
			CASE
			WHEN A.STATUS_KIRIM = '1' THEN
				CASE
				WHEN B1.TERBACA IS NULL THEN
				'Di kirim ke ' || B1.SATUAN_KERJA_ASAL_NAMA
				WHEN B1.TERDISPOSISI = 2 THEN
				'Revisi oleh ' || B1.SATUAN_KERJA_ASAL_NAMA
				WHEN B1.TERDISPOSISI = 3 THEN
				'Surat dapat diambil di ' || B1.SATUAN_KERJA_ASAL_NAMA
				WHEN B1.TERBACA = '1' THEN
				'Di terima ' || B1.SATUAN_KERJA_ASAL_NAMA
				ELSE
				''
				END
			ELSE 'Belum terkirim ke ' || B1.SATUAN_KERJA_ASAL_NAMA END INFO_STATUS
			, CASE WHEN A.STATUS_KIRIM = '1' THEN B1.SATUAN_KERJA_ASAL_ID ELSE NULL END SATUAN_KERJA_ID_AKSI
			--, CASE WHEN A.STATUS_KIRIM = '1' AND A.SATUAN_KERJA_TUJUAN_ID != B1.SATUAN_KERJA_ASAL_ID THEN B1.SATUAN_KERJA_ASAL_ID ELSE NULL END SATUAN_KERJA_ID_AKSI
		FROM persuratan.SURAT_KELUAR_BKD A
		LEFT JOIN
		(
			SELECT
			A.SURAT_KELUAR_BKD_SEDIAAN_ID, A.SURAT_KELUAR_BKD_ID, B.TANGGAL, B.TERDISPOSISI, B.ISI
			, B.TERBACA, B.TERBALAS, B.TANGGAL_DISPOSISI, B.SATUAN_KERJA_ASAL_ID, B.SATUAN_KERJA_ASAL_NAMA
			FROM persuratan.SURAT_KELUAR_BKD_SEDIAAN_POSISI A
			INNER JOIN persuratan.SURAT_KELUAR_BKD_SEDIAAN B ON A.SURAT_KELUAR_BKD_SEDIAAN_ID = B.SURAT_KELUAR_BKD_SEDIAAN_ID
		) B1 ON A.SURAT_KELUAR_BKD_ID = B1.SURAT_KELUAR_BKD_ID
		LEFT JOIN SATUAN_KERJA B2 ON B1.SATUAN_KERJA_ASAL_ID = B2.SATUAN_KERJA_ID
		WHERE 1=1
		"; 
    ...
    function getCountByParamsSediaan($paramsArray=array(), $statement='')
	{
		$str = "
		SELECT COUNT(1) AS ROWCOUNT
		FROM persuratan.SURAT_KELUAR_BKD A
		...
		(
			SELECT
			A.SURAT_KELUAR_BKD_SEDIAAN_ID, A.SURAT_KELUAR_BKD_ID, B.TANGGAL
			, B.TERBACA, B.TERBALAS, B.TANGGAL_DISPOSISI, B.SATUAN_KERJA_ASAL_ID, B.SATUAN_KERJA_ASAL_NAMA
			FROM persuratan.SURAT_KELUAR_BKD_SEDIAAN_POSISI A
			INNER JOIN persuratan.SURAT_KELUAR_BKD_SEDIAAN B ON A.SURAT_KELUAR_BKD_SEDIAAN_ID = B.SURAT_KELUAR_BKD_SEDIAAN_ID
		) B1 ON A.SURAT_KELUAR_BKD_ID = B1.SURAT_KELUAR_BKD_ID
