  ...
  ***/
  ...
  class SuratMasukPegawai extends Entity{ 
		...
		$this->setField("SURAT_MASUK_PEGAWAI_ID", $this->getNextId("SURAT_MASUK_PEGAWAI_ID","PERSURATAN.SURAT_MASUK_PEGAWAI")); 
     	...
     	$str = "
			INSERT INTO PERSURATAN.SURAT_MASUK_PEGAWAI (
				...
				, KATEGORI, KETERANGAN_PENSIUN, TMT_PENSIUN
		...
		$this->setField("SURAT_MASUK_PEGAWAI_ID", $this->getNextId("SURAT_MASUK_PEGAWAI_ID","PERSURATAN.SURAT_MASUK_PEGAWAI"));
     	...
     	$str = "
			INSERT INTO PERSURATAN.SURAT_MASUK_PEGAWAI (
				...
				, KP_PAK_LAMA_STATUS
				, KP_STATUS_PENDIDIKAN_RIWAYAT_CANTUM_GELAR, KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS
				, LAST_USER, LAST_DATE, LAST_LEVEL, USER_LOGIN_ID, USER_LOGIN_PEGAWAI_ID
				) 
			VALUES (
				 ".$this->getField("SURAT_MASUK_PEGAWAI_ID").",
				 ...
				 ".$this->getField("KP_STATUS_PENDIDIKAN_RIWAYAT_CANTUM_GELAR").",
				 ".$this->getField("KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS").",
				 '".$this->getField("LAST_USER")."',
				 ".$this->getField("LAST_DATE").",
				 ".$this->getField("LAST_LEVEL").",
				 ".$this->getField("USER_LOGIN_ID").",
	...
	function insertKarsuUsulan()
	{
		$this->setField("SURAT_MASUK_KARSU_ID", $this->getNextId("SURAT_MASUK_KARSU_ID","persuratan.SURAT_MASUK_KARSU"));
     	...
     	(
	     	...
	     	, PEGAWAI_ID, SUAMI_ISTRI_ID, SUAMI_ISTRI_PISAH_ID, NIKAH_PERTAMA_PNS_STATUS
	     	, NIKAH_PERTAMA_PNS_STATUS_SI, NIKAH_PERTAMA_PNS_TANGGAL
	     	, NIKAH_PERTAMA_PASANGAN_STATUS, NIKAH_PERTAMA_PASANGAN_STATUS_SI
	     	, NIKAH_PERTAMA_PASANGAN_TANGGAL, JENIS_KESALAHAN, TERTULIS, SEHARUSNYA
	     	, NO_SURAT_KEHILANGAN, TANGGAL_SURAT_KEHILANGAN, KETERANGAN
	     	, LAST_USER, LAST_DATE, LAST_LEVEL
     	) 
     	VALUES
     	(
    ...
    **/ 
    ...
    function selectByParamsAnak($pegawaiid="", $kategori="", $statement='',$order=' ORDER BY CAST(A.ANAK_USIA AS NUMERIC) DESC')
	{
		...
		(
			SELECT
				ROW_NUMBER () OVER (ORDER BY AMBIL_UMUR_TAHUN(A.TANGGAL_LAHIR, P.TMT) DESC) NOMOR
				, AMBIL_UMUR_TAHUN(A.TANGGAL_LAHIR, P.TMT) ANAK_USIA
				, P.TMT, B.NAMA SUAMI_ISTRI_NAMA, PN.NAMA PENDIDIKAN_NAMA, COALESCE(JK.NAMA, 'Belum diisi') JENIS_KAWIN_NAMA
				, CASE B.STATUS_S_I WHEN '1' THEN 'Nikah' WHEN '2' THEN 'Cerai' WHEN '2' THEN 'Meninggal' ELSE 'Belum di set' END SUAMI_ISTRI_STATUS_NAMA
				, CASE A.STATUS_KELUARGA WHEN '1' THEN 'Kandung' WHEN '2' THEN 'Tiri' ELSE 'Angkat' END ANAK_STATUS_NAMA
				, CASE A.STATUS_AKTIF WHEN '1' THEN 'Hidup' WHEN '2' THEN 'Wafat' ELSE '' END ANAK_STATUS_HIDUP
				, CASE A.STATUS_BEKERJA WHEN '1' THEN 'Sudah Bekerja' ELSE 'Belum Bekerja' END ANAK_STATUS_BEKERJA
				, CASE A.STATUS_LULUS WHEN '1' THEN 'Sudah Lulus' ELSE 'Belum Lulus' END ANAK_STATUS_LULUS
				, A.*
			FROM 
			(
			...
			LEFT JOIN SUAMI_ISTRI B ON A.SUAMI_ISTRI_ID = B.SUAMI_ISTRI_ID
			LEFT JOIN PENDIDIKAN PN ON A.PENDIDIKAN_ID = PN.PENDIDIKAN_ID
			LEFT JOIN sapk.jenis_kawin JK ON JK.ID = A.JENIS_KAWIN_ID
	...
	function selectByParamsInfoTotal($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order='')
	{
		$str = "
		SELECT 
			COALESCE(SUM(CASE WHEN INFO_STATUS_KIRIM = 1 THEN 1 ELSE 0 END),0) INFOJUMLAHTURUNSTATUS
			, COALESCE(SUM(CASE WHEN INFO_STATUS_KIRIM = 2 THEN 1 ELSE 0 END),0) INFOJUMLAHKIRIM
			, COALESCE(SUM(CASE WHEN INFO_STATUS_KIRIM = 3 THEN 1 ELSE 0 END),0) INFOJUMLAHTERIMA
			, COALESCE(SUM(CASE WHEN INFO_STATUS_KIRIM = 4 THEN 1 ELSE 0 END),0) INFOJUMLAHTERVERIFIKASI
		FROM PEGAWAI A
		INNER JOIN
		(
			SELECT
			CASE
			--untuk status kembali
			WHEN STATUS_KEMBALI = 1 AND STATUS_BERKAS != 99 THEN 1
			--untuk kirim
			WHEN STATUS_BERKAS = 2 OR STATUS_BERKAS = 5 THEN 2
			--posisi di bkdsm
			WHEN STATUS_BERKAS = 9 OR (STATUS_BERKAS = 10 AND STATUS_VERIFIKASI IS NULL) THEN 3
			--kalau terverifikasi
			WHEN STATUS_VERIFIKASI = 1 AND STATUS_BERKAS != 99 THEN 4
			--pembatalan
			WHEN STATUS_BERKAS = 99 THEN 9
			ELSE 0 END INFO_STATUS_KIRIM
			, *
			FROM persuratan.SURAT_MASUK_PEGAWAI
		) SMP ON A.PEGAWAI_ID = SMP.PEGAWAI_ID
	...
	function selectByParamsUsulanPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order=' ORDER BY SMP.SURAT_MASUK_PEGAWAI_ID ASC')
	{
		...
		SELECT A.PEGAWAI_ID, A.SATUAN_KERJA_ID, A.NIP_BARU
			...
			, (CASE WHEN COALESCE(NULLIF(A.GELAR_DEPAN,'') , NULL ) IS NULL THEN '' ELSE A.GELAR_DEPAN || ' ' END) || A.NAMA || (CASE WHEN COALESCE(NULLIF(A.GELAR_BELAKANG,'') , NULL ) IS NULL THEN '' ELSE '' || A.GELAR_BELAKANG END) NAMA_LENGKAP
			, A.NAMA NAMA_SAJA
			, SMB.NOMOR NOMOR_USUL_BKDPP, L.LAST_DATE PROSES_TANGGAL
			, CASE WHEN SMP.STATUS_BERKAS = 99 THEN L.INFO_PROSES || L.LAST_USER ELSE L.INFO_PROSES END PROSES_STATUS
			, SMP.STATUS_BERKAS
			, SMP.STATUS_KEMBALI, SMP.STATUS_PERNAH_TURUN
			, SKR.NOMOR NOMOR_SURAT_KELUAR, SMP.STATUS_SURAT_KELUAR
			, SMP.USULAN_SURAT_URUT
			, SMP.STATUS_E_FILE_BKD, CASE WHEN SMP.STATUS_E_FILE_BKD = 1 THEN 'Valid' ELSE 'Belum Valid' END INFO_E_FILE_BKD
			, SMP.STATUS_E_FILE_UPT, CASE WHEN SMP.STATUS_E_FILE_UPT = 1 THEN 'Valid' ELSE 'Belum Valid' END INFO_E_FILE_UPT
		FROM PEGAWAI A
		INNER JOIN
		(
			SELECT
	...
	function selectByParamsMonitoring($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order=' ORDER BY A.PEGAWAI_ID ASC')
	{
		...
		SELECT
			...
			WHEN SMP.JENIS_ID = 7 THEN
				...
				ELSE 
					CASE
					WHEN TO_CHAR(SMB.TANGGAL, 'MM') = '01' THEN CAST(CAST(TO_CHAR(SMB.TANGGAL, 'YYYY') AS NUMERIC) - 0 AS TEXT)
					WHEN TO_CHAR(SMU.TANGGAL, 'MM') = '01' THEN CAST(CAST(TO_CHAR(SMU.TANGGAL, 'YYYY') AS NUMERIC) - 0 AS TEXT)
					ELSE
			...
			WHEN SMP.JENIS_ID = 10 THEN
				CASE WHEN SMP.SURAT_MASUK_BKD_ID IS NOT NULL THEN CAST(CAST(TO_CHAR(SMB.TANGGAL_PERIODE_KP, 'YYYY') AS NUMERIC) - 0 AS TEXT)
				ELSE CAST(CAST(TO_CHAR(SMU.TANGGAL_PERIODE_KP, 'YYYY') AS NUMERIC) - 0 AS TEXT)
				END
			WHEN SMP.JENIS_ID = 1 THEN
				CASE WHEN SMP.SURAT_MASUK_BKD_ID IS NOT NULL THEN CAST(CAST(TO_CHAR(SMB.TANGGAL, 'YYYY') AS NUMERIC) - 1 AS TEXT)
			...
			, SMP.USULAN_SURAT_ID, US.STATUS_KIRIM STATUS_KIRIM_USULAN, US.JENIS_ID STATUS_JENIS_USULAN
			, SMP.KATEGORI
			, CASE
			WHEN SMP.KATEGORI = 'bup' AND SMP.JENIS_ID = 7 THEN 'Pensiun BUP'
			WHEN SMP.KATEGORI = 'meninggal' AND SMP.JENIS_ID = 7 THEN 'Pensiun Janda/Duda'
			WHEN SMP.KATEGORI = 'dini' AND SMP.JENIS_ID = 7 THEN 'Pensiun APS(Dini)'
			WHEN SMP.KATEGORI = 'udzur' AND SMP.JENIS_ID = 7 THEN 'Pensiun Udzur'
			END KATEGORI_NAMA
			, CASE SMP.KP_JENIS
			...
			, SMP.KP_STATUS_SURAT_TANDA_LULUS, SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS
			, SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_CANTUM_GELAR
			, SMP.KP_PAK_LAMA_STATUS, SMP.KP_PAK_LAMA_ID, SMP.KP_PAK_BARU_ID
			, SMP.KP_STATUS_SERTIFIKAT_KEASLIAN, SMP.KP_STATUS_SERTIFIKAT_PENDIDIK
			, CASE
    ...
    function selectByParamsMonitoringKpUpt($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order=' ORDER BY A.PEGAWAI_ID ASC')
	{
		$str = "
				SELECT A.PEGAWAI_ID, CASE A.STATUS WHEN '1' THEN 'Tidak Aktif' ELSE 'Aktif' END STATUS_NAMA, A.SATUAN_KERJA_ID,
					...
					, AMBIL_SATKER_NAMA_DETIL(SMP.SATUAN_KERJA_PEGAWAI_USULAN_ID) SATUAN_KERJA_PEGAWAI_USULAN_NAMA
					, CASE KPG_RIW.JENIS_KARPEG WHEN 1 THEN 'Baru' WHEN 2 THEN 'Revisi' WHEN 3 THEN 'Kehilangan' END JENIS_KARPEG_NAMA
					, SMP.USULAN_SURAT_ID, US.STATUS_KIRIM STATUS_KIRIM_USULAN, SMP.KATEGORI
					, CASE
					WHEN SMP.KATEGORI = 'bup' AND SMP.JENIS_ID = 7 THEN 'Pensiun BUP'
					WHEN SMP.KATEGORI = 'meninggal' AND SMP.JENIS_ID = 7 THEN 'Pensiun Janda/Duda'
					WHEN SMP.KATEGORI = 'dini' AND SMP.JENIS_ID = 7 THEN 'Pensiun APS(Dini)'
	...
	function selectByParamsMonitoringPensiun($paramsArray=array(),$limit=-1,$from=-1, $kategori="", $statement='',$order=' ORDER BY A.PEGAWAI_ID ASC')
	{
		...
		$str = "
				SELECT A.PEGAWAI_ID, CASE A.STATUS WHEN '1' THEN 'Tidak Aktif' ELSE 'Aktif' END STATUS_NAMA, A.SATUAN_KERJA_ID,
					...
					, P.PANGKAT_KODE, P.PANGKAT_TMT, P.PANGKAT_TH, P.PANGKAT_BL
					, COALESCE(SMP.TMT_PENSIUN, P.PENSIUN_TMT) PENSIUN_TMT
					, CASE WHEN SMP.TMT_PENSIUN IS NOT NULL THEN
					--CASE WHEN (COALESCE(CAST(P.PANGKAT_BL AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 2) AS NUMERIC),0)) % 12 = 0
					--THEN 
					--(
					--	COALESCE(CAST(P.PANGKAT_TH AS NUMERIC),0) 
					--	+ COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 1) AS NUMERIC),0)
					--	--+ FLOOR((COALESCE(CAST(P.PANGKAT_BL AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 2) AS NUMERIC),0)) / 12)
					--)
					--ELSE
					--(COALESCE(CAST(P.PANGKAT_TH AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 1) AS NUMERIC),0))
					--END
					(COALESCE(CAST(P.PANGKAT_TH AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 1) AS NUMERIC),0))
					ELSE P.PENSIUN_TH END
					+
					CASE WHEN SMP.TMT_PENSIUN IS NOT NULL AND (COALESCE(CAST(P.PANGKAT_BL AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 2) AS NUMERIC),0)) >= 12 THEN 1 ELSE 0 END
					PENSIUN_TH
					, CASE WHEN P.PENSIUN_TMT IS NOT NULL THEN
					CASE WHEN (COALESCE(CAST(P.PANGKAT_BL AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 2) AS NUMERIC),0)) % 12 = 0 
					THEN 0
					WHEN (COALESCE(CAST(P.PANGKAT_BL AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 2) AS NUMERIC),0)) > 12 THEN
					(COALESCE(CAST(P.PANGKAT_BL AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 2) AS NUMERIC),0)) - 12
					ELSE
					(COALESCE(CAST(P.PANGKAT_BL AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(P.PANGKAT_TMT, SMP.TMT_PENSIUN), ' - ', 2) AS NUMERIC),0))
					END
					ELSE P.PENSIUN_BL END PENSIUN_BL
					--, COALESCE(SMP.TMT_PENSIUN, P.PENSIUN_TMT) PENSIUN_TMT
				...
				(
					SELECT
						...
						--THEN
						--(
						--	COALESCE(CAST(PR.MASA_KERJA_TAHUN AS NUMERIC),0) 
						--	+ COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(PR.TMT_PANGKAT, P.TMT), ' - ', 1) AS NUMERIC),0)
						--	--+ FLOOR((COALESCE(CAST(PR.MASA_KERJA_BULAN AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(PR.TMT_PANGKAT, P.TMT), ' - ', 2) AS NUMERIC),0)) / 12)
						--)
						--ELSE
						...
						(COALESCE(CAST(PR.MASA_KERJA_TAHUN AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(PR.TMT_PANGKAT, P.TMT), ' - ', 1) AS NUMERIC),0))
						+
						CASE WHEN (COALESCE(CAST(PR.MASA_KERJA_BULAN AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(PR.TMT_PANGKAT, P.TMT), ' - ', 2) AS NUMERIC),0)) >= 12 THEN 1 END
						PENSIUN_TH
						, CASE WHEN (COALESCE(CAST(PR.MASA_KERJA_BULAN AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(PR.TMT_PANGKAT, P.TMT), ' - ', 2) AS NUMERIC),0)) % 12 = 0 
						THEN 0
						WHEN (COALESCE(CAST(PR.MASA_KERJA_BULAN AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(PR.TMT_PANGKAT, P.TMT), ' - ', 2) AS NUMERIC),0)) > 12 THEN
						(COALESCE(CAST(PR.MASA_KERJA_BULAN AS NUMERIC),0) + COALESCE(CAST(SPLIT_PART(AMBIL_UMUR_DUK(PR.TMT_PANGKAT, P.TMT), ' - ', 2) AS NUMERIC),0)) - 12
						ELSE
					...
					if(!empty($kategori))
					{
						$str .= "AND REPLACE(P.JENIS, 'pensiun', '') = '".$kategori."'";
					}
				$str .= ") P ON A.PEGAWAI_ID = P.PEGAWAI_ID
				LEFT JOIN persuratan.SURAT_MASUK_BKD SMB ON SMP.SURAT_MASUK_BKD_ID = SMB.SURAT_MASUK_BKD_ID
				LEFT JOIN
				(
					SELECT A.PANGKAT_RIWAYAT_ID, B.KODE, A.TMT_PANGKAT
    ...
    function selectByParamsMonitoringKenaikanPangkat($paramsArray=array(),$limit=-1,$from=-1, $kategori="", $statement='',$order=' ORDER BY A.PEGAWAI_ID ASC')
	{
		...
		SELECT
			...
			WHEN SMP.KP_PANGKAT_ID = 41 AND SMP.KP_DIKLAT_ID = 1 THEN '2'
			ELSE '' END KP_STATUS_STRUKTURAL_ID
			, SMP.KP_DIKLAT_ID, SMP.KP_DIKLAT_STRUKTURAL_ID
			, DS.NAMA KP_DIKLAT_STRUKTURAL_NAMA, DS.TANGGAL_STTPP KP_DIKLAT_STRUKTURAL_TANGGAL
			, CASE SMP.KP_JENIS
			WHEN 'kpreguler' THEN 'KP Reguler'
			WHEN 'kpstruktural' THEN 'KP Pilihan (Jabatan Struktural)'
			WHEN 'kpjft' THEN 'KP Pilihan (Jabatan Fungsional Tertentu)'
			...
			, STL.NO_STLUD KP_NO_STLUD, STL.TANGGAL_STLUD KP_TANGGAL_STLUD
			, PENR1.PENDIDIKAN_JURUSAN_NAMA KP_PENDIDIKAN_JURUSAN_NAMA, SPLIT_PART(PF.PATH, '/', 3) KP_PATH_NAMA
			...
			, P_LAMA.TANGGAL_MULAI KP_PAK_LAMA_TANGGAL, P_LAMA.TOTAL_KREDIT KP_PAK_LAMA_NILAI
			, P_BARU.TANGGAL_MULAI KP_PAK_BARU_TANGGAL, P_BARU.TOTAL_KREDIT KP_PAK_BARU_NILAI
			...
			,
			CASE
			WHEN SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_CANTUM_GELAR = 1 THEN 'Ya'
			WHEN SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_CANTUM_GELAR = 2 THEN 'Tidak'
			ELSE '' END KP_STATUS_PENDIDIKAN_RIWAYAT_CANTUM_GELAR_NAMA
			, SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS
			,
			CASE
			WHEN SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS = 1 THEN 'Ijin belajar / tugas belajar mandiri'
			WHEN SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS = 2 THEN 'Tugas Belajar'
			WHEN SMP.KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS = 3 THEN 'Tanpa Ijin Belajar'
			ELSE '' END KP_STATUS_PENDIDIKAN_RIWAYAT_IJIN_TUGAS_NAMA
		FROM PEGAWAI A
		INNER JOIN persuratan.SURAT_MASUK_PEGAWAI SMP ON A.PEGAWAI_ID = SMP.PEGAWAI_ID
		LEFT JOIN persuratan.SURAT_MASUK_UPT SMU ON SMP.SURAT_MASUK_UPT_ID = SMU.SURAT_MASUK_UPT_ID
		LEFT JOIN persuratan.SURAT_MASUK_BKD SMB ON SMP.SURAT_MASUK_BKD_ID = SMB.SURAT_MASUK_BKD_ID
		...
		) DS ON DS.DIKLAT_STRUKTURAL_ID = SMP.KP_DIKLAT_STRUKTURAL_ID
    ...
    function updatestatusefile()
	{
		$str = "		
		UPDATE PERSURATAN.SURAT_MASUK_PEGAWAI
		SET
			".$this->getField("FIELD")."= ".$this->getField("FIELD_NILAI")."
		WHERE ".$this->getField("FIELD_WHERE")." = ".$this->getField("FIELD_WHERE_NILAI")."
    ...
    function updatestatusuptbkdefile()
	{
		$str = "		
		UPDATE PERSURATAN.SURAT_MASUK_PEGAWAI
		SET
			STATUS_E_FILE_UPT= ".$this->getField("FIELD_NILAI")."
			, STATUS_E_FILE_BKD= ".$this->getField("FIELD_NILAI")."
		WHERE ".$this->getField("FIELD_WHERE")." = ".$this->getField("FIELD_WHERE_NILAI")."
		AND PEGAWAI_ID = ".$this->getField("PEGAWAI_ID")."
		"; 
		$this->query = $str;
