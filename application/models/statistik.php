...
***************************************************************************************************** */
  ...
  ***/
  ...
  class Statistik extends Entity{ 
    ...
    **/
	...
	function selectByParamsTipePegawai($statement='', $sOrder="ORDER BY A.TIPE_PEGAWAI_ID")
	{
		...
		(
			...
			FROM PEGAWAI A
			INNER JOIN SATUAN_KERJA C ON A.SATUAN_KERJA_ID = C.SATUAN_KERJA_ID
			WHERE STATUS_PEGAWAI_ID IN (1,2,6)
			".$statement."
		) A
		WHERE 1=1
   ...
   //tambahan buat dashboard rendy

    function selectByParamsPensiun($statement='', $sOrder="ORDER BY TMT_BULAT")
	{
		//query 
		$str = "
				select tmt_bulat, sum (jumlah) AS jumlah
				from
				(
					SELECT
						DATE_TRUNC( 'month', tmt ) :: DATE AS tmt_bulat,
						b.satuan_kerja_id,
						COUNT ( DATE_TRUNC( 'month', tmt ) :: DATE ) AS jumlah 
					FROM
						pegawai_status a
						INNER JOIN pegawai b ON a.pegawai_id = b.pegawai_id 
					WHERE
						a.status_pegawai_id IN ( 3, 4, 5 ) 
						AND DATE_TRUNC( 'month', tmt ) >= DATE_TRUNC(
							'year',
						NOW()) 
						AND a.tmt <= ( DATE_TRUNC( 'month', NOW()) + INTERVAL '1 MONTH' ) 
					GROUP BY tmt_bulat, b.satuan_kerja_id
					union all
					SELECT
						tmt_bulat
						, a.satuan_kerja_id
						, COUNT ( DATE_TRUNC( 'MONTH', tmt_bulat ) :: DATE ) AS jumlah 
					FROM
						(
						SELECT
						CASE
							...
							WHEN A
								.TANGGAL_LAHIR IS NULL THEN
								NULL ELSE
								CASE
										
										WHEN TO_CHAR( A.TANGGAL_LAHIR, 'MM' ) = '12' THEN
										TO_DATE(
											CONCAT (
												'01-01-',
												CAST (
													CAST ( TO_CHAR( A.TANGGAL_LAHIR, 'YYYY' ) AS NUMERIC ) + 1 + COALESCE ( JAB_RIW.USIA_BUP, 0 ) AS TEXT 
												)),
											'DD-MM-YYYY' 
											) ELSE TO_DATE(
											CONCAT (
												'01-',
												CAST ( TO_CHAR( A.TANGGAL_LAHIR, 'MM' ) AS NUMERIC ) + 1,
												'-',
											CAST ( CAST ( TO_CHAR( A.TANGGAL_LAHIR, 'YYYY' ) AS NUMERIC ) + COALESCE ( JAB_RIW.USIA_BUP, 0 ) AS TEXT )),
											'DD-MM-YYYY' 
										) 
									END 
									END tmt_bulat
					...
					WHERE
					STATUS_PEGAWAI_ID IN ( 1, 2 ) 
					) A 
					WHERE
					1 = 1 
					AND DATE_TRUNC( 'month', tmt_bulat ) >= (
					DATE_TRUNC( 'month', NOW() + INTERVAL '1 MONTH' )) 
					AND tmt_bulat <= (( DATE_TRUNC( 'year', NOW()) + INTERVAL '2 YEAR' ) - INTERVAL '1 MONTH' ) 
					GROUP BY tmt_bulat, a.satuan_kerja_id
				) A
				INNER JOIN SATUAN_KERJA C ON A.SATUAN_KERJA_ID = C.SATUAN_KERJA_ID
				WHERE 
				".$statement."
