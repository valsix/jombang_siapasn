  ...
  class Skcpns extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		...
		$this->setField("SK_CPNS_ID", $this->getNextId("SK_CPNS_ID","SK_CPNS"));
     	$str = "
			INSERT INTO SK_CPNS (
				SK_CPNS_ID, PEGAWAI_ID, PEJABAT_PENETAP_ID, PEJABAT_PENETAP, PANGKAT_ID, NO_NOTA, TANGGAL_NOTA, NO_SK, 
				TANGGAL_SK, TMT_CPNS, TANGGAL_TUGAS, NO_STTPP, TANGGAL_STTPP, NAMA_PENETAP, NIP_PENETAP, MASA_KERJA_TAHUN, 
				MASA_KERJA_BULAN, GAJI_POKOK, NO_PERSETUJUAN_NIP, TANGGAL_PERSETUJUAN_NIP, PENDIDIKAN_RIWAYAT_ID, 
    ...
    function update()
	{
		...
		$str = "		
				UPDATE SK_CPNS
				SET    
				  	SK_CPNS_ID= ".$this->getField("SK_CPNS_ID").",
    ...
    **/ 
    ...
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order=' ORDER BY A.SK_CPNS_ID ASC')
	{
		$str = "
				SELECT
					A.SK_CPNS_ID, A.PEGAWAI_ID, A.PEJABAT_PENETAP_ID, A.PEJABAT_PENETAP, A.PANGKAT_ID, A.NO_NOTA, A.TANGGAL_NOTA, A.NO_SK, 
					A.TANGGAL_SK, A.TMT_CPNS, A.TANGGAL_TUGAS, A.NO_STTPP, A.TANGGAL_STTPP, A.NAMA_PENETAP, A.NIP_PENETAP, A.MASA_KERJA_TAHUN, 
					A.MASA_KERJA_BULAN, A.GAJI_POKOK,A. NO_PERSETUJUAN_NIP, A.TANGGAL_PERSETUJUAN_NIP, A.PENDIDIKAN_RIWAYAT_ID, 
    ...
    function selectByParamsBak($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order=' ORDER BY A.SK_CPNS_ID ASC')
	{
		$str = "
				SELECT
