  ...
  ***/
  ...
  class Dynaport extends Entity{ 
    ...
    **/
	...
	function selectByParamsMonitoring($paramsArray=array(),$limit=-1,$from=-1, $statement='', $order=' ORDER BY A.PEGAWAI_ID ASC', $aColumnsView= "")
	{
		...
		$str = " SELECT A.PEGAWAI_ID";
		for($i=0; $i < count($aColumnsView); $i++)
		{
			$str .= ", ".$aColumnsView[$i];
		}
		...
		LEFT JOIN
		(
			SELECT A.PANGKAT_RIWAYAT_ID, B.KODE, A.TMT_PANGKAT, A.NO_SK
			FROM PANGKAT_RIWAYAT A
			LEFT JOIN PANGKAT B ON A.PANGKAT_ID = B.PANGKAT_ID
		) PANG_RIW ON A.PANGKAT_RIWAYAT_ID = PANG_RIW.PANGKAT_RIWAYAT_ID
		LEFT JOIN
