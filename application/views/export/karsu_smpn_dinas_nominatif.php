...
if($reqStatusBkdUptId == "1")
{
  ...
  $set= new SuratMasukUpt();
  $set->selectByParamsCetakPengantarSatuOrang(array(), -1, -1, $statement);
  // $set->selectByParamsCetakPengantarSatuOrang(array(), 8, 0, $statement);
  
  // $set->selectByParamsCetakPengantarSatuOrang(array(), 13, 0, $statement);
  // $set->selectByParamsCetakPengantarSatuOrang(array(), 10, 0, $statement);
  // $set->selectByParamsCetakPengantarSatuOrang(array(), 9, 0, $statement);
...
while($set->nextRow())
{
  ...
  // echo $tempJenisKarpeg;exit();
  $tempKeterangan= "";
  if($tempJenisKarpeg == "1")
  $tempKeterangan= "BARU";
  elseif($tempJenisKarpeg == "2")
  $tempKeterangan= "REVISI<br/>terdapat kesalahan pada ".$set->getField("JENIS_KESALAHAN")." tertulis ".$set->getField("TERTULIS")." seharusnya yang benar tertulis ".$set->getField("SEHARUSNYA");
...
<table border="0" cellspacing="0" cellpadding="0" width="1171">
  <tr>
    ...
    <tr>
      <td colspan="4" style="text-align:center"><br/>DAFTAR NAMA PERMOHONAN KARIS / KARSU<br/><br/><br/></td>
    </tr>
</table>
<div class="area-rekap-gaji">
  ...
  <div class="row col-6 float-r" style="float:right; margin-right:-250px; margin-top:0px;">
      ...
      <div style="margin-bottom:100px !important">
        ...
        ?>
        <span style="">KABUPATEN JOMBANG</span>
        <!-- <span style="">KABUPATEN JOMBANG<?=$tempModBatas?>-<?=$tempMod?>-<?=$tempHeader?></span> -->
      </div>
      <?
      }
      ?>
