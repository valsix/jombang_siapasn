...
$(function(){
	...
	$('#ff').form({
		url:'surat/surat_masuk_pegawai_json/add_pensiun_dinas',
		onSubmit:function(){
		...
		success:function(data){
			...
			else
			{
				mbox.alert(infodata, {open_speed: 500}, interval = window.setInterval(function() 
				{
					...
					mbox.close();
					document.location.href= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_pensiun/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId="+rowid;
				}, 1000));
	...
	$('input[id^="reqNipBaru"]').autocomplete({
		source:function(request, response){
			...
			{
				urlAjax= "pendidikan_riwayat_json/cari_pegawai_pensiun_usulan?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqKategori=<?=$reqKategori?>&reqMode=1";
			}
		...
		{ 
			...
			if (id.indexOf('reqNipBaru') !== -1)
			{
				...
				vurl= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_pensiun_anak/?reqId=<?=$reqId?>&reqKategori=<?=$reqKategori?>&reqPegawaiId="+ui.item.id+"&reqPensiunTmtTahun="+pensiuntmttahun;
				$.ajax({
					'url': vurl
					, beforeSend: function () {
						$(".preloader-wrapper").show();
...
<script type="text/javascript">
  $(document).ready(function() {
				   ...
				   buttons: [
					   {
						   ...
						   callback: function() {
								...
								function(data){
									...
									{
										clearInterval(interval);
										document.location.href= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_pensiun/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>";
									}, 1000));
									$(".mbox > .right-align").css({"display": "none"});
									//$(".right-align").hide();
								});
	...
	$("#reqselanjutnya").click(function() { 
    	document.location.href= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_lookup_verfikasi_pensiun/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>";
    });
	...
	$("#kembali").click(function() {
		kembali= "<?=$kembali?>";
		if(kembali == "")
			infolinkurl= "surat_masuk_bkd_add_pegawai";
		else
			infolinkurl= kembali;
