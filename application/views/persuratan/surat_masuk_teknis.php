...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
			...
			//"columnDefs": [{ className: 'never', targets: [ 7 ] }],
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/surat_masuk_bkd_json/json_teknis?reqJenis=<?=$reqJenis?>",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
        ...
        $('#btnAdd').on('click', function () {
        	newWindow = window.open("app/loadUrl/persuratan/surat_masuk_teknis_add?reqJenis=<?=$reqJenis?>", 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
        ...
        $('#btnEdit').on('click', function () {
        	if(anSelectedData == "")
        		return false;				
        	newWindow = window.open("app/loadUrl/persuratan/surat_masuk_teknis_add?reqJenis=<?=$reqJenis?>&reqId="+anSelectedId, 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
			tempindextab= parseInt(tempindextab) + 1;
		...
		$("#btnCari").on("click", function () {
			...
			oTable.fnReloadAjax("surat/surat_masuk_bkd_json/json_teknis?reqJenis=<?=$reqJenis?>&sSearch="+reqCariFilter);
		});
        		...
        		if (r){
        			$.getJSON("surat/surat_masuk_bkd_json/delete/?reqRowId="+anSelectedId,
        				function(data){
        					...
        					oTable.fnReloadAjax("surat/surat_masuk_bkd_json/json_teknis?reqJenis=<?=$reqJenis?>");
        				});
