...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/usulan_surat_json/json?reqJenis=<?=$reqJenis?>&reqKirim=<?=$reqKirim?>",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
        ...
        $('#btnAdd').on('click', function () {
        	newWindow = window.open("app/loadUrl/persuratan/usulan_surat_add?reqJenis=<?=$reqJenis?>&reqKirim=<?=$reqKirim?>", 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
        ...
        $('#btnEdit').on('click', function () {
        	if(anSelectedData == "")
        		return false;				
        	newWindow = window.open("app/loadUrl/persuratan/usulan_surat_add?reqJenis=<?=$reqJenis?>&reqKirim=<?=$reqKirim?>&reqId="+anSelectedId, 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
			tempindextab= parseInt(tempindextab) + 1;
		...
		$("#btnCari").on("click", function () {
			...
			oTable.fnReloadAjax("surat/usulan_surat_json/json?reqJenis=<?=$reqJenis?>&reqKirim=<?=$reqKirim?>");
		});
...
function hapusdata(id)
{
	...
	$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
		if (r){
			$.getJSON("surat/usulan_surat_json/deletesurat/?reqId="+id,
				function(data){
					$.messager.alert('Info', data.PESAN, 'info');
					setCariInfo();
				});
						...
						<table id="example" class="display mdl-data-table dt-responsive" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<th style="text-align:center">Tanggal Usul</th>
