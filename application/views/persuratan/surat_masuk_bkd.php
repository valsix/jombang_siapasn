...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/surat_masuk_bkd_json/json?reqJenis=<?=$reqJenis?>",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
        ...
        $('#btnAdd').on('click', function () {
        	newWindow = window.open("app/loadUrl/persuratan/surat_masuk_bkd_add?reqJenis=<?=$reqJenis?>", 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
        ...
        $('#btnEdit').on('click', function () {
        	if(anSelectedData == "")
        		return false;				
        	newWindow = window.open("app/loadUrl/persuratan/surat_masuk_bkd_add?reqJenis=<?=$reqJenis?>&reqId="+anSelectedId, 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
			tempindextab= parseInt(tempindextab) + 1;
		...
		$("#btnCari").on("click", function () {
			...
			oTable.fnReloadAjax("surat/surat_masuk_bkd_json/json?reqJenis=<?=$reqJenis?>&sSearch="+reqCariFilter);
		});
        		...
        		if (r){
        			$.getJSON("surat/surat_masuk_bkd_json/delete/?reqRowId="+anSelectedId,
        				function(data){
        					...
        					setCariInfo();
        					// oTable.fnReloadAjax("surat/surat_masuk_bkd_json/json?reqJenis=<?=$reqJenis?>");
        				});
...
function hapusdata(id)
{
	...
	$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
		if (r){
			$.getJSON("surat/surat_masuk_bkd_json/deletesurat/?reqId="+id,
				function(data){
					$.messager.alert('Info', data.PESAN, 'info');
					setCariInfo();
				});
