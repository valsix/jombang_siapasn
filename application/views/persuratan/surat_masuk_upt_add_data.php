...
/*(
CURRENT_DATE BETWEEN A.TANGGAL_BATAS_AWAL_USUL AND A.TANGGAL_BATAS_AKHIR_USUL_STRUKTURAL
)
OR*/
...
<head>
  ...
  <script type="text/javascript"> 
    $(function(){
    	...
    	function setinfopensiun(infomode)
    	{
    			   ...
    			   buttons: [
    				   {
    					   ...
    					   callback: function() {
    						   $.getJSON("surat/surat_masuk_upt_json/statuskirim/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqPerihal=<?=$tempPerihalInfo?>&reqStatusBerkas=2",
    							function(data){
    								mbox.alert(data.PESAN, {open_speed: 500}, interval = window.setInterval(function() 
    								{
    									...
    									parent.reloadparenttab();
    									document.location.href= "app/loadUrl/persuratan/surat_masuk_upt_add_data/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>";
    								}, 1000));
    								$(".mbox > .right-align").css({"display": "none"});
    								//$(".right-align").hide();
    							});
    ...
    $("#reqcetakpengantar").click(function() { 
      ...
      $.ajax({'url': s_url,'success': function(dataajax){
        ...
        requrlcss= element[3];
    		...
    		//requrl= "ijin_belajar_pengantar_uptd_lebih_1_org";
    		//requrllist= "ijin_belajar_pengantar_uptd_nominatif";
    		
    	  newWindow = window.open("app/loadUrl/persuratan/cetak_pdf?reqStatusBkdUptId=1&reqCss="+requrlcss+"&reqUrl="+requrl+"&reqUrlList="+requrllist+"&reqId=<?=$reqId?>", 'Cetak');
    	  newWindow.focus();
    	}});
	  });
...
getMonthDifference= getMonthDifference(
  new Date('2014-04-01'), new Date('2022-09-01')
);
