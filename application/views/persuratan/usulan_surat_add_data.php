...
$reqLogId= str_replace(" ", "_", strtolower($reqJenisNama)) ."/usulan_".$reqId;
// echo $reqLogId;exit;
              ...
              <form id="ff" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                  <div class="input-field col s12">
                    ...
                    <input required placeholder="" class="easyui-validatebox" type="text" name="reqIdSementara" id="reqIdSementara" <?=$read?> value="<?=$reqIdSementara?>"/>
                  </div>
                </div>
                ...
                <div class="row">
                  <div class="input-field col s12 m12">
                    ...
                    if($reqStatusKirim == "2")
                    {
                      ...
                      if($reqKirim == "1")
                        $infokirim= "BKN Pusat";
                    ?>
                      <button class="btn blue waves-effect waves-light" style="font-size:9pt" type="button" id="reqtujuankirim">Kirim ke <?=$infokirim?>
                      <i class="mdi-content-forward left hide-on-small-only"></i>
                      </button>
                    <?
                    }
                    ...
                    if($reqStatusKirim == "3")
                    {
                      ...
                      if($reqKirim == "1")
                        $infokirim= "BKN Pusat";
                    ?>
                      <button class="btn blue waves-effect waves-light" style="font-size:9pt" type="button" id="reqsudahkirim">Sudah di Kirim ke <?=$infokirim?>
                      <i class="mdi-content-forward left hide-on-small-only"></i>
                      </button>
                    <?
                    }
                  ...
                  <?
                  // if($reqStatusKirim >= "2" || ($reqJenis == "11" && $reqStatusKirim == "1"))
                  // if($reqStatusKirim >= "2" || ($reqJenis == "11" && $reqId !== ""))
                  // {

                  if(!empty($reqId))
                  {
                    $infobuttondownload= "Persiapan Download";
                    if($infostatus == "2")
                    {
                      $infobuttondownload= "Download File PDF";
                    }
                  ?>
                    <button class="btn blue waves-effect waves-light" style="font-size:9pt" type="button" id="reqdownloadfile"><span id="infodownloadfile"><?=$infobuttondownload?></span>
                      <i class="mdi-content-forward left hide-on-small-only"></i>
                    </button>
                  <?
                    if($infostatus == "2")
                    {
                  ?>
                    <button class="btn orange waves-effect waves-light" style="font-size:9pt" type="button" id="reqresetdownloadfile">Reset Download
                      <i class="mdi-content-forward left hide-on-small-only"></i>
                    </button>
                  <?
                    }
...
$(document).ready(function() {
  ...
  $("#reqsudahkirim,#reqtujuankirim,#reqkirim,#reqbatal").click(function() { 
    ...
    $.ajax({'url': s_url,'success': function(dataajax){
      ...
      else
      {
        ...
        {
         info= "Apakah anda yakin merubah status menjadi di kirim ke <?=$infokirim?>?";
         // info= "Apakah Anda Yakin, usul nomor surat Keluar di kirim ke Kanreg II BKN ?";
         reqStatusKirim= 3;
        }
        else if(id == "reqsudahkirim")
        {
         info= "Apakah anda yakin merubah status menjadi di sudah kirim ke <?=$infokirim?>?";
  ...
  $("#reqcetakpengantar").click(function() { 
    // parent.openModal('app/loadUrl/persuratan/tanda_tangan_pilih_badan?reqStatusBkdUptId=1&reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqSatuanKerjaId=<?=$reqSatuanKerjaBkdId?>')
  ...
  $("#reqdownloadfile,#reqresetdownloadfile").click(function() { 
    $(".preloader-wrapper").show();
    id= $(this).attr('id');

      <?
      // if($reqJenis == "11" && $reqStatusKirim == "")
      // {
      ?>
        // $(".preloader-wrapper").hide();
      ...
      s_url= "surat/download_json/syncdata?reqLogId=<?=$reqLogId?>";
      if(id == "reqresetdownloadfile")
      {
        s_url= "surat/download_json/syncdata?reqLogId=<?=$reqLogId?>&h=1";
        $("#reqresetdownloadfile").hide();
        $("#infodownloadfile").text("Persiapan Download");
      }
      ...
      $.ajax({'url': s_url,'success': function(dataajax){
        ...
        dataajax= String(dataajax);
        var element = dataajax.split('***'); 
        infosimpan= element[0];
        requrl= element[1];

        // console.log(requrl);return false;

        if(infosimpan == 'xxx')
        {
          mbox.alert('Tunggu 2 - 3 menit untuk mendownload file, silakan lakukan perkerjaan lain.', {open_speed: 0});
        }
        else
        {
          newWindow = window.open(requrl, 'Cetak');
          newWindow.focus();
        }
      }});
