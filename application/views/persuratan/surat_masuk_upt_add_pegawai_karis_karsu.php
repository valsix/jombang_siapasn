...
<head>
  ...
  <script type="text/javascript"> 
	...
	$(function(){
		...
		$('#ff').form({
			url:'surat/surat_masuk_pegawai_json/add_karsu',
			onSubmit:function(){
				var reqPegawaiId= "";
        	...
        	success:function(data){
				...
				else
				{
					mbox.alert(infodata, {open_speed: 500}, interval = window.setInterval(function() 
					{
					  ...
					  mbox.close();
					  document.location.href= "app/loadUrl/persuratan/surat_masuk_upt_add_pegawai_karis_karsu/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId="+rowid;
					}, 1000));
		...
		$('input[id^="reqNipBaru"]').autocomplete({
			source:function(request, response){
				...
				{
					urlAjax= "pendidikan_riwayat_json/cari_pegawai_karsu_usulan?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>";
				}
            ...
            <div class="row">
              ...
              <div class="row">
                ...
                <div class="input-field col s12 m3">
                  ...
                  <input placeholder="" type="text" class="easyui-validatebox" required id="reqSuamiIstriTerakhirPernikahanPertamaPasanganTanggal" name="reqSuamiIstriTerakhirPernikahanPertamaPasanganTanggal" maxlength="10" onKeyDown="return format_date(event,'reqSuamiIstriTerakhirPernikahanPertamaPasanganTanggal');" data-options="validType:'dateValidPicker'" <?=$read?> value="<?=$reqSuamiIstriTerakhirPernikahanPertamaPasanganTanggal?>" <?=$disabled?> />
                </div>
            	</div>
              </div>
				   ...
				   buttons: [
					   {
						   ...
						   callback: function() {
								...
								function(data){
									...
									{
										clearInterval(interval);
										document.location.href= "app/loadUrl/persuratan/surat_masuk_upt_add_pegawai_karis_karsu/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>";
									}, 1000));
									$(".mbox > .right-align").css({"display": "none"});
									//$(".right-align").hide();
								});
