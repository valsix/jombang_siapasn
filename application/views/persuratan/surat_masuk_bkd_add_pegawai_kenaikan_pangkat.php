...
// $statementfile= " AND A.PEGAWAI_ID = ".$reqId." AND A.RIWAYAT_TABLE = 'PENDIDIKAN_RIWAYAT' AND A.KATEGORI_FILE_ID = 14 ";
// $pegawaifile= new SuratMasukPegawai();
// $pegawaifile->selectByParamsLastRiwayatTable(array(),-1,-1, $statementfile);
// echo $pegawaifile->query;exit();
?>
...
<head>
	...
	<script type="text/javascript"> 
		...
		$(function(){
			...
			$('#ff').form({
				url:'surat/surat_masuk_pegawai_json/add_kenaikan_pangkat_dinas',
				onSubmit:function(){
					var reqPegawaiId= "";
				...
				success:function(data){
					...
					else
					{
						mbox.alert(infodata, {open_speed: 500}, interval = window.setInterval(function() 
						{
							...
							mbox.close();
							document.location.href= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_kenaikan_pangkat/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId="+rowid;
						}, 1000));
			...
			$('input[id^="reqNipBaru"]').autocomplete({
				source:function(request, response){
					...
					{
						urlAjax= "pendidikan_riwayat_json/cari_pegawai_kenaikan_pangkat_reguler?reqId=<?=$reqId?>&reqKpJenis=<?=$reqKpJenis?>&reqJenis=<?=$reqJenis?>&reqMode=1";
					}
				...
				{ 
					...
					if (id.indexOf('reqNipBaru') !== -1)
					{
						...
						// $.ajax({'url': "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_kenaikan_pangkat_detil/?reqId=<?=$reqId?>&reqKpJenis=<?=$reqKpJenis?>&reqPegawaiId="+ui.item.id,'success': function(datahtml) {
						// 	// console.log(datahtml);
						// 	$('#labeldetilinfo').html(datahtml);
						// 	// parent.iframeLoaded();
						// }});
						...
						// elseif($reqRowId == "" && ($reqKpJenis == "kppi" || $reqKpJenis == "kpstugas"))
						// {
						?>
						/*$("<option value=''></option>").appendTo("#reqKpSuratTandaLulusId");
						$.ajax({'url': "surat_tanda_lulus_json/combokenaikanpangkat/?reqJenis=ujiandinas&reqId="+pegawaiid,'success': function(dataJson) {
							var data= JSON.parse(dataJson);
						...
						vurl= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_kenaikan_pangkat_detil/?reqId=<?=$reqId?>&reqKpJenis=<?=$reqKpJenis?>&reqPegawaiId="+ui.item.id+"&reqKpTahun=<?=$reqTahunSurat?>";
						$.ajax({
							'url': vurl
							, beforeSend: function () {
								$(".preloader-wrapper").show();
	...
	function setJenisKenaikanPangkat()
    {
	    ...
	    document.location.href = "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_kenaikan_pangkat?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>&reqKpJenis="+reqKpJenis;
	}
								...
								<div id="labeldetilinfo" style="display: none;">
									...
									?>
										<div class="row">
											...
											{
											?>
											<div class="input-field col s12 m3 reqKpStatusStrukturalInfo">
									...
									?>
										...
										<div class="row">
											<div>
												<div class="input-field col s12 m8">
													...
													<label for="reqKpPendidikanRiwayatBelumDiakuiId">Pilih Pendidikan Yang Belum Diakui</label>
												</div>
											</div>
										</div>
										...
										<div class="row">
											<div>
												<div class="input-field col s12 m8">
													<input placeholder type="text" value="<?=$reqKpPendidikanRiwayatBelumDiakuiNama?>" disabled />
													<label for="reqKpPendidikanRiwayatBelumDiakuiNama">Pilih Pendidikan Yang Belum Diakui</label>
												</div>
											</div>
										</div>
										<!-- <div class="row">
...
$(document).ready(function() {
					...
					callback: function() {
						$.getJSON("surat/surat_masuk_pegawai_json/delete_pegawai/?reqId=<?=$reqRowId?>",
							function(data){
								...
								{
									clearInterval(interval);
									document.location.href= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_kenaikan_pangkat/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>";
								}, 1000));
								$(".mbox > .right-align").css({"display": "none"});
							});
						mbox.close();
	...
	$("#reqselanjutnya").click(function() { 
    	document.location.href= "app/loadUrl/persuratan/surat_masuk_bkd_add_pegawai_lookup_verfikasi_kenaikan_pangkat/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>";
    });
