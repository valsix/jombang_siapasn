...
// echo $sessionLoginLevel."--".$reqLastLevel."--".$reqJenisJabatanTugasId;exit();
...
<head>
  ...
  function settimetmt(info)
      ...
      success:function(data){
        ...
        else
        {
          mbox.alert(infodata, {open_speed: 500}, interval = window.setInterval(function() 
          {
           ...
           reloadparenttab();
           mbox.close();
           document.location.href= "app/loadUrl/persuratan/mutasi_usulan_add_fungsional/?reqId="+rowid;
         }, 1000));
    ...
    $(this).autocomplete({
      source:function(request, response){
        ...
        if (id.indexOf('reqNipBaru') !== -1)
        {
          ...
          urlAjax= "surat/mutasi_usulan_json/cari_pegawai_usulan?reqTipePegawaiId=12";
        }
   ...
   $('input[id^="reqPejabatPenetap"], input[id^="reqJabatanFu"], input[id^="reqSatker"], input[id^="reqNama"]').each(function(){
    $(this).autocomplete({
      source:function(request, response){
        var id= this.element.attr('id');
        var replaceAnakId= replaceAnak= urlAjax= "";
        ...
        else if (id.indexOf('reqPejabatPenetap') !== -1)
        {
            ...
            <form id="ff" method="post" enctype="multipart/form-data">
              ...
              <div class="row">
                <div class="input-field col s12">
                  ...
                  for($i_data=0; $i_data < count($arrFileBolehUpload); $i_data++)
                  {
                    $tempUrlFile= "uploadsurat/mutasi_fungsional/".$reqId.".".$arrFileBolehUpload[$i_data];
          ...
          var s_url= "surat/mutasi_usulan_json/valid?reqJenisJabatanTugasId=<?=$reqJenisJabatanTugasId?>&reqId=<?=$reqId?>";
           $.ajax({'url': s_url,'success': function(dataajax){
            ...
            mbox.alert(info, {open_speed: 500}, interval = window.setInterval(function() 
            {
              ...
              reloadparenttab();
              mbox.close();
              document.location.href= "app/loadUrl/persuratan/mutasi_usulan_add_fungsional/?reqId=<?=$reqId?>";
            }, 1000));
            $(".mbox > .right-align").css({"display": "none"});
          }});
           mbox.close();
