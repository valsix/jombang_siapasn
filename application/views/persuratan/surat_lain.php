...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/surat_keluar_lain_json/json_surat",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
        ...
        $('#btnAdd').on('click', function () {
        	newWindow = window.open("app/loadUrl/persuratan/surat_lain_teknis_add", 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
        ...
        $('#btnEdit').on('click', function () {
        	...
        	newWindow = window.open("app/loadUrl/persuratan/surat_lain_teknis_add?reqJenis="+anSelectedJenisId+"&reqBulan="+anSelectedBulan+"&reqTahun="+anSelectedTahun+"&reqId="+anSelectedId, 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
			tempindextab= parseInt(tempindextab) + 1;
		...
		$("#btnCari").on("click", function () {
			var reqSatuanKerjaId= reqTahun= reqJenis= reqStatusTerima= "";
			reqSatuanKerjaId= $("#reqSatuanKerjaId").val();
			reqTahun= $("#reqTahun").val();
			reqStatusTerima= $("#reqStatusTerima").val();
			oTable.fnReloadAjax("surat/surat_keluar_lain_json/json_surat?reqStatusTerima="+reqStatusTerima+"&reqJenis="+reqJenis+"&reqSatuanKerjaId="+reqSatuanKerjaId+"&reqTahun="+reqTahun);
		});
		...
		$('#btnDelete').on('click', function () {
        	...
        	$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
        		if (r){
        			$.getJSON("surat/surat_keluar_lain_json/delete/?reqRowId="+anSelectedId,
        				function(data){
        					$.messager.alert('Info', data.PESAN, 'info');
        					oTable.fnReloadAjax("surat/surat_keluar_lain_json/json_surat?reqJenis=<?=$reqJenis?>");
        				});
	   ...
	   buttons: [
		   {
			   ...
			   callback: function() {
				   $.ajax({'url': "surat/surat_keluar_lain_json/status_terima/?reqId="+id,'success': function(datahtml) {
					setCariInfo();
				   }});
				   mbox.close();
			   }
	   ...
	   buttons: [
		   {
			   ...
			   color: 'green darken-2',
			   callback: function() {
				   $.ajax({'url': "surat/surat_keluar_lain_json/delete/?reqId="+id,'success': function(datahtml) {
					setCariInfo();
				   }});
				   mbox.close();
			   }
