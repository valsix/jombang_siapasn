...
$statement= " AND A.STATUS_SEDIAAN = '1'";
$arrsatuankerjasediaan= [];
...
while($set->nextRow())
{
    ...
    array_push($arrsatuankerjasediaan, $arrdata);
}
...
else
{
   ...
   $reqIsSediaan= $set->getField("IS_SEDIAAN");
...
$disabled= "";
// kalau status sudah terkirim
if(!empty($reqStatusKirim))
{
	$disabled= "disabled";
}
// $disabled= "disabled";
...
<head>
  ...
  <script type="text/javascript"> 
            ...
            onSubmit:function(){
				...
				else
				{
					mbox.alert(infodata, {open_speed: 500}, interval = window.setInterval(function() 
					{
						...
						document.location.href= "app/loadUrl/persuratan/surat_keluar_teknis_add_surat_data/?reqId="+rowid;
					}, 1000));
				}
	...
	$("#reqIsManual").click(function () {
		$("#reqSatuanKerjaTujuanNama,#reqSatuanKerjaTujuanId").val("");

		$("#reqSatuanKerjaSelectTujuanNama").val("");
		$("#reqSatuanKerjaSelectTujuanNama").material_select();
	});

	$("#labeltujuanauto").show();
	$("#reqkirim, #labeltujuanselect").hide();
	reqIsSediaan= "<?=$reqIsSediaan?>";
	if(reqIsSediaan == "1")
	{
		$("#labeltujuanauto").hide();
		$("#reqkirim, #labeltujuanselect").show();
	}

	$("#reqSatuanKerjaSelectTujuanNama").change(function() {
		reqSatuanKerjaTujuanId= $(this).find("option:selected").val();
		reqSatuanKerjaTujuanNama= $(this).find("option:selected").text();

		$("#reqSatuanKerjaTujuanId").val(reqSatuanKerjaTujuanId);
		$("#reqSatuanKerjaTujuanNama").val(reqSatuanKerjaTujuanNama);
	});

	$("#reqIsSediaan").click(function () {
		$("#reqSatuanKerjaSelectTujuanNama").val("");
		$("#reqSatuanKerjaSelectTujuanNama").material_select();

		if (!this.checked)
		{
			$("#reqIsManual").prop('checked', false).removeAttr("disabled", "disabled");
		...
		{
			$("#reqSatuanKerjaTujuanNama,#reqSatuanKerjaTujuanId").val("");
			$("#reqIsManual").prop('checked', false).attr("disabled", "disabled");
	...
	$('input[id^="reqSatuanKerjaTujuanNama"],input[id^="reqKlasifikasi"]').each(function(){
		$(this).autocomplete({
			source:function(request, response){
				var id= this.element.attr('id');
				var replaceAnakId= replaceAnak= urlAjax= "";

				  if (id.indexOf('reqSatuanKerjaTujuanNama') !== -1)
				  {
				  	if($("#reqIsManual").prop('checked') || $("#reqIsSediaan").prop('checked'))
				  	{
				  		return false;
				  	}
				  }
			
				  if (id.indexOf('reqKlasifikasi') !== -1)
				  {
					var element= id.split('reqKlasifikasi');
					var indexId= "reqKlasifikasiId"+element[1];
					urlAjax= "surat/klasifikasi_json/combo?reqId=";
				  }
				  else if (id.indexOf('reqSatuanKerjaTujuanNama') !== -1)
				  {
					var element= id.split('reqSatuanKerjaTujuanNama');
					var indexId= "reqSatuanKerjaTujuanId"+element[1];
				...
				$.ajax({
					...
					success: function(responseData){
						if(responseData == null || responseData == "")
						{
							...
							if (id.indexOf('reqKlasifikasi') !== -1)
				  			{
								$("#reqNomorSuratKodeKlasifikasi").val("");
								setNomorSurat();
							}
			...
			{ 
				var id= $(this).attr('id');
				  if (id.indexOf('reqKlasifikasi') !== -1)
				  {
					var element= id.split('reqKlasifikasi');
					var indexId= "reqKlasifikasiId"+element[1];
					$("#reqNomorSuratKodeKlasifikasi").val(ui.item.label);
					setNomorSurat();
				  }
				  else if (id.indexOf('reqSatuanKerjaTujuanNama') !== -1)
				  {
...
function setNomorSurat()
{
	...
	if(panjang == 10)
	{
		reqTanggal= String($("#reqTanggal").val());
		reqNomorSuratTahun= reqTanggal.split('-');
		reqNomorSuratTahun= reqNomorSuratTahun[2]
                    ...
                    <form id="ff" method="post" enctype="multipart/form-data">
                    	<div class="row">
                    		<div class="input-field col s12 m3">
                    			...
                    			<input required class="easyui-validatebox" data-options="validType:'dateValidPicker'" type="text" name="reqTanggal" id="reqTanggal"  value="<?=$reqTanggal?>" maxlength="10" onKeyDown="return format_date(event,'reqTanggal');" <?=$disabled?> />
                    		</div>
                    		<?
                    		if(empty($disabled))
                    		{
	                    ...
	                    <div class="row">
	                    	<div class="input-field col s12 m3">
	                    		...
	                    		<input placeholder="" class="easyui-validatebox" required type="text" name="reqKlasifikasi" id="reqKlasifikasi" value="<?=$reqKlasifikasi?>" <?=$disabled?> />
	                    	</div>
	                    	<div class="input-field col s12 m2">
	                    		<label for="reqNomor">Nomor Agenda</label>
	                    		<input type="text" name="reqNomorAwal" id="reqNomorAwal" value="<?=$reqNomorAwal?>" class="easyui-validatebox" required <?=$disabled?> />
	                    	</div>
	                    	<div class="input-field col s12 m2">
	                    		<label for="reqNomorUrut">Nomor Sub Agenda</label>
	                    		<input placeholder="" type="text" name="reqNomorUrut" id="reqNomorUrut" value="<?=$reqNomorUrut?>" class="easyui-validatebox" <?=$disabled?> />
	                    	</div>
	                    	<div class="input-field col s12 m2">
	                    	...
	                    	<div class="input-field col s12 m2">
	                    		...
	                    		<input type="text" readonly id="reqNomorSuratTahun" value="<?=$reqNomorSuratTahun?>" <?=$disabled?> />
	                    	</div>
	                    </div>
	                    ...
	                    <div class="row">
	                    	<div class="input-field col s12 m5">
	                    		...
	                    		<input type="text" id="reqNomor" style="font-size:20px; font-weight:bold" name="reqNomor"  value="<?=$reqNomor?>" class="easyui-validatebox" readonly required <?=$disabled?> />
	                    	</div>
	                    </div>
	                    ...
	                    <div class="row">
	                    	<div class="input-field col s12 m6">
	                    		...
	                    		<input placeholder="" type="text" name="reqPerihal" id="reqPerihal"  value="<?=$reqPerihal?>" <?=$disabled?> />
	                    	</div>
	                    </div>

                        <div class="row">
                        	<div class="input-field col s6">
                        		<input type="checkbox" id="reqIsSediaan" name="reqIsSediaan" value="1" <? if($reqIsSediaan == 1) echo 'checked'?> <?=$disabled?> />
                        		<label for="reqIsSediaan"></label>
                        		*centang jika sediaan surat keluar sekda/bupati
                        	</div>
                        	<div class="input-field col s6">
                        		<input type="checkbox" id="reqIsManual" name="reqIsManual" value="1" <? if($reqIsManual == 1) echo 'checked'?> <?=$disabled?> />
                        		<label for="reqIsManual"></label>
                        		*centang jika satker di luar kab jombang
                        	</div>
                        </div>

						<div class="row" id="labeltujuanauto">
							<div class="input-field col s12 m12">
								<label for="reqSatuanKerjaTujuanNama">Tujuan Satuan Kerja</label>
								<input type="hidden" name="reqSatuanKerjaTujuanId" id="reqSatuanKerjaTujuanId" value="<?=$reqSatuanKerjaTujuanId?>" />
								<input placeholder="" type="text" id="reqSatuanKerjaTujuanNama" name="reqSatuanKerjaTujuanNama" <?=$read?> value="<?=$reqSatuanKerjaTujuanNama?>" class="easyui-validatebox" required <?=$disabled?> />
							</div>
						</div>

						<div class="row" id="labeltujuanselect">
							<div class="input-field col s12 m12">
								<select id="reqSatuanKerjaSelectTujuanNama" <?=$disabled?> >
									<option value="" <? if($reqSatuanKerjaTujuanId=="") echo 'selected';?>>Pilih salah satu</option>
									<?
									foreach($arrsatuankerjasediaan as $valkey => $valitem) 
                                    {
                                    	$vid= $valitem["ID"];
                                    	$vtext= $valitem["TEXT"];
									?>
									<option value="<?=$vid?>" <? if($reqSatuanKerjaTujuanId == $vid) echo 'selected';?>><?=$vtext?></option>
									<?
									}
									?>
								</select>
								<label for="reqSatuanKerjaSelectTujuanNama">Tujuan Surat</label>
							</div>
						</div>

						<div class="row">
							<div class="input-field col s12 m12">
								<label for="reqKeterangan">Catatan / Keterangan</label>
								<input placeholder="" type="text" id="reqKeterangan" name="reqKeterangan" <?=$read?> value="<?=$reqKeterangan?>" <?=$disabled?> />
							</div>
						</div>
						...
						?>
						<div class="row">
							<div class="input-field col s12 m12" style="margin-top: -25px; margin-bottom: 10px;">
                        ...
                        <div class="row">
                            <div class="input-field col s12 m12">
								...
								if(!empty($reqId))
								{
									...
									{
                                ?>
                                	<button class="btn red waves-effect waves-light" style="font-size:9pt" type="button" id="reqkirim">Kirim <i class="mdi-content-forward left hide-on-small-only"></i>
                                	</button>
                                <?
                                	}
								}
								...
								{
	                            ?>
	                            <button class="btn waves-effect waves-light purple" style="font-size:9pt" type="button" onClick="parent.setload('surat_keluar_add_data_agenda_file?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqBulan=<?=$reqBulan?>&reqTahun=<?=$reqTahun?>&reqUrlFile=<?=$tempUrlFile?>')">Lihat File
...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#tambah").click(function() { 
		document.location.href= "app/loadUrl/persuratan/surat_keluar_teknis_add_surat_data/?reqId=&reqJenis=<?=$reqJenis?>&reqMode=<?=$reqMode?>";
	});
	...
	$("#reqkirim").click(function() { 
		...
		mbox.custom({
		   message: "Apakah anda yakin kirim, surat keluar sediaan. Pastikan tujuan surat sudah sesuai ?",
		   options: {close_speed: 100},
		   buttons: [
			   {
				   label: 'Ya',
				   color: 'green darken-2',
				   callback: function() {
					   $.getJSON("surat/surat_keluar_bkd_json/statussediaankirim/?reqId=<?=$reqId?>&reqStatusBerkas=1",
						function(data){
							// console.log(data);return false;
							mbox.alert(data.PESAN, {open_speed: 500}, interval = window.setInterval(function() 
							{
								...
								parent.reloadparenttab();
								document.location.href= "app/loadUrl/persuratan/surat_keluar_teknis_add_surat_data/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>";
							}, 1000));
