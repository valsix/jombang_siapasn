...
else
{
    ...
    $set= new Menu();
    $set->selectByParamsUsulanSurat(array(), -1, -1, $statement);
    $set->firstRow();
    ...
    $tempInfoTab= "Usulan ".$reqJenisNama." - ".$reqIdSementara;
}

$index_set=0;
$arrMenu=setmenuusulansurat($reqId);
$jumlah_menu= count($arrMenu);
// echo $jumlah_menu;exit;
// print_r($arrMenu);exit;

$infourlawal= "app/loadUrl/persuratan/".$arrMenu[0]["LINK_FILE"]."/?reqId=".$reqId."&reqJenis=".$reqJenis."&reqKirim=".$reqKirim;
?>
<!DOCTYPE html>
<html>
<head>
        ...
        function executeOnClick(varItem){
            ...
            for($index_loop=0; $index_loop < $jumlah_menu; $index_loop++)
            {
                ...
                ?>
                    ...
                    {
                        ?>
                        mainFrame.location.href='app/loadUrl/persuratan/<?=$tempLinkFile?>/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqKirim=<?=$reqKirim?>';
						iframeLoaded();
...
<body id="layouts-horizontal">
  ...
  <script type="text/javascript">
	...
	function reloadinfosurat(id)
	{
		...
		{
			$('#info-surat').empty();
			$.ajax({'url': "surat/usulan_surat_json/set_info_pegawai_data/?reqId="+id,'success': function(datahtml) {
    ...
    <script type="text/javascript">
		...
		function cetakpengantartipe(id, tipeid)
		{
			...
			$.ajax({'url': s_url,'success': function(dataajax){
			  ...
			  newWindow = window.open("app/loadUrl/persuratan/cetak_pdf?reqCss=surat_pengantar&reqUrl="+requrl+"&reqUrlList="+requrllist+"&reqId=<?=$reqId?>&reqPegawaiPilihKepalaId="+id+"&reqTipeId="+tipeid, 'Cetak');
			  newWindow.focus();
			}});
		}

        // function cetakpengantartipejabatan(id, tipeid, jabatanpilihan, jabatanmanual)
        function cetakpengantartipejabatan(reqGantiBaris, id, tipeid, jabatanpilihan, jabatanmanual)
        {
            // alert(jabatanpilihan);return false;
            ...
            $.ajax({'url': s_url,'success': function(dataajax){
              ...
              newWindow = window.open("app/loadUrl/persuratan/cetak_pdf?reqGantiBaris="+reqGantiBaris+"&reqStatusBkdUptId=3&reqCss="+requrlcss+"&reqUrl="+requrl+"&reqUrlList="+requrllist+"&reqId=<?=$reqId?>&reqPegawaiPilihKepalaId="+id+"&reqTipeId="+tipeid+"&reqJabatanPilihan="+jabatanpilihan+"&reqJabatanManual="+reqJabatanManual, 'Cetak');
              newWindow.focus();

              // newWindow = window.open("app/loadUrl/persuratan/cetak_pdf?reqStatusBkdUptId=3&reqCss="+requrlcss+"&reqUrl="+requrl+"&reqUrlList="+requrllist+"&reqId=<?=$reqId?>&reqPegawaiPilihKepalaId="+id+"&reqTipeId="+tipeid+"&reqJabatanPilihan="+jabatanpilihan+"&reqJabatanManual="+reqJabatanManual, 'Cetak');
              newWindow.focus();
            }});
        }
