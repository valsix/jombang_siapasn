...
<head>
  ...
  function settimetmt(info)
    ...
    onSubmit:function(){
        ...
        reqSatkerReadOnly= $('#reqSatker').attr('readonly');
        // console.log(reqSatkerReadOnly);
        if(reqSatkerReadOnly == undefined)
        {
          var reqTugasTambahanId= $("#reqTugasTambahanId").val();
          if(reqTugasTambahanId == "")
          {
            mbox.alert("Lengkapi data Nama Tugas terlebih dahulu", {open_speed: 0});
            return false;
          } 
        }

        var reqSatkerId= $("#reqSatkerId").val();
        if(reqSatkerId == "")
        {
          mbox.alert("Lengkapi data Satuan Kerja terlebih dahulu", {open_speed: 0});
          return false;
        } 
        ...
        success:function(data){
          ...
          else
          {
            mbox.alert(infodata, {open_speed: 500}, interval = window.setInterval(function() 
            {
              ...
              reloadparenttab();
              mbox.close();
              document.location.href= "app/loadUrl/persuratan/mutasi_usulan_add_jft_kesehatan/?reqId="+rowid;
            }, 1000));
              ...
              <form id="ff" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                  <div class="input-field col s12">
                    ...
                    <?
                    $tempUrlFile= "uploadsurat/mutasi_tugas_kesehatan/".$reqId.".pdf";
          ...
          var s_url= "surat/mutasi_usulan_json/valid?reqJenisJabatanTugasId=<?=$reqJenisJabatanTugasId?>&reqId=<?=$reqId?>";
           $.ajax({'url': s_url,'success': function(dataajax){
            ...
            mbox.alert(info, {open_speed: 500}, interval = window.setInterval(function() 
            {
              ...
              reloadparenttab();
              mbox.close();
              document.location.href= "app/loadUrl/persuratan/mutasi_usulan_add_jft_kesehatan/?reqId=<?=$reqId?>";
            }, 1000));
            $(".mbox > .right-align").css({"display": "none"});
          }});
           mbox.close();
