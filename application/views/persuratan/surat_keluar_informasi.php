...
while($set->nextRow())
{
	$arrJenisPelayanan[$index_data]["JENIS_PELAYANAN_ID"] = $set->getField("JENIS_PELAYANAN_ID");
	$arrJenisPelayanan[$index_data]["NAMA"] = $set->getField("NAMA");
	$index_data++;
}
$jumlah_pelayanan= $index_data;
//print_r($arrJenisPelayanan);exit;
...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/surat_keluar_bkd_json/json_surat_keluar",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
        ...
        $('#btnEdit').on('click', function () {
        	if(anSelectedData == "")
        		return false;				
        	newWindow = window.open("app/loadUrl/persuratan/surat_keluar_teknis_add?reqJenis="+anSelectedJenisId+"&reqId="+anSelectedId, 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
			tempindextab= parseInt(tempindextab) + 1;
		...
		$("#reqHari,#reqBulan,#reqTahun,#reqJenis").change(function() {
			setCariInfo();
		});
		
		$("#btnCari").on("click", function () {
			var reqSatuanKerjaId= reqHari= reqBulan= reqTahun= reqJenis= reqCariFilter= "";
			reqHari= $("#reqHari").val();
			reqBulan= $("#reqBulan").val();
			reqTahun= $("#reqTahun").val();
			reqCariFilter= $("#reqCariFilter").val();
			oTable.fnReloadAjax("surat/surat_keluar_bkd_json/json_surat_keluar?reqJenis="+reqJenis+"&reqHari="+reqHari+"&reqBulan="+reqBulan+"&reqTahun="+reqTahun+"&sSearch="+reqCariFilter);
		});
...
<style>
#example td:nth-child(3),td:nth-child(4) {
    text-align : center;
    *font-weight: bold;
	*color:#F00 !important
}
			...
			<section id="content-full">
                ...
                <div class="area-parameter">
					<div class="kiri">
						...
						<span>entries</span>
						<span style="padding-left:50px">Hari</span>
                        <select name="reqHari" id="reqHari">
                            ...
                            {
                            ?>
                            <option value="<?=generateZero($hari,2)?>" <? if($tempBulanAktif == $hari) echo "selected";?>><?=generateZero($hari,2)?></option>
                            <?
                            }
                            ?>
                        </select>
                        <span style="padding-left:5px">Bulan</span>
                        ...
                        <span style="padding-left:5px">Jenis Surat</span>
                        <select id="reqJenis">
							<option value="">Semua</option>
                            <?
							for($index=0; $index < $jumlah_pelayanan; $index++)
							{
								$tempJenisPelayananId= $arrJenisPelayanan[$index]["JENIS_PELAYANAN_ID"];
								$tempJenisPelayananNama= $arrJenisPelayanan[$index]["NAMA"];
                            ?>
                            <option value="<?=$tempJenisPelayananId?>"><?=$tempJenisPelayananNama?></option>
                            <?
							}
                            ?>
						</select>
