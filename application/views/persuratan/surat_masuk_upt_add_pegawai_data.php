...
<tr>
    ...
    <th class="material-font" style=" <?=$stlePernahTurun?>text-align:center"><?=$set->getField("SATUAN_KERJA_NAMA_UPT")?></th>
    ...
    {
    ?>
    <th class="material-font" style=" <?=$stlePernahTurun?>text-align:center"><?=$set->getField("INFO_E_FILE_UPT")?></th>
    <?
    }
    ?>
    ...
    <th class="material-font" style=" <?=$stlePernahTurun?>text-align:center">
    	<a href="javascript:void(0)" class="round waves-effect waves-light blue white-text" title="Ubah" onClick="parent.setload('<?=seturllink($reqJenis)?>?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$set->getField("SURAT_MASUK_PEGAWAI_ID")?>')"><i class="mdi-editor-mode-edit"></i></a>
        
        <a href="javascript:void(0)" class="round waves-effect waves-light red white-text" title="Kelengkapan" onClick="parent.setload('<?=setlinkuptverifikasilookup($reqJenis)?>?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$set->getField("SURAT_MASUK_PEGAWAI_ID")?>')"><i class="mdi-editor-attach-file"></i></a>

        <?
		if($set->getField("STATUS_KEMBALI") == "1")
		{
        ?>
        <a href="javascript:void(0)" class="round waves-effect waves-light red white-text" title="Status Kembali" onClick="parent.setload('<?=setlinkuptverifikasi($reqJenis)?>?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$set->getField("SURAT_MASUK_PEGAWAI_ID")?>')"><i class="mdi-editor-mode-edit"></i></a>
        <?
		}
        ?>
        <?
