...
while($set->nextRow())
{
    ...
    if($set->getField("STATUS_BERKAS") == "99")
    {
        $stlePernahTurun= "background-color:#FB99E1;";
    }
?>
<tr>
    ...
    <th class="material-font" style=" <?=$stlePernahTurun?>text-align:center">
        <a href="javascript:void(0)" class="round waves-effect waves-light blue white-text" title="Ubah" onClick="parent.setload('<?=seturllink($reqJenis)?>?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$set->getField("SURAT_MASUK_PEGAWAI_ID")?>&kembali=surat_masuk_dinas_upt_add_pegawai')"><i class="mdi-editor-mode-edit"></i></a>

        <a href="javascript:void(0)" class="round waves-effect waves-light red white-text" title="Kelengkapan" onClick="parent.setload('<?=setlinkuptverifikasilookup($reqJenis)?>?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$set->getField("SURAT_MASUK_PEGAWAI_ID")?>&kembali=surat_masuk_dinas_upt_add_pegawai')"><i class="mdi-editor-attach-file"></i></a>
    </th>
    <?
    }
    ?>
