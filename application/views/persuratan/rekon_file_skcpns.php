...
while($set->nextRow())
{
  ...
  $reqPegawaiJenisKelamin= $set->getField("PEGAWAI_JENIS_KELAMIN");
  $arrInfo[$index_data]["PEGAWAI_JENIS_KELAMIN"] = $reqPegawaiJenisKelamin;
  if($reqPegawaiJenisKelamin == "L")
    $arrInfo[$index_data]["FORMAT_JENKEL"] = "SK_SUKET_DUDA";
  else
    $arrInfo[$index_data]["FORMAT_JENKEL"] = "SK_SUKET_JANDA";
...
for($index=0; $index < $jumlah_info; $index++)
{
  ...
  $tempFormatJenkel= $arrInfo[$index]["FORMAT_JENKEL"];
  ...
  else
  {
    ...
    // if($tempNamaCheck == "Surat Ket Tdk Pernah Dijatuhi Hukuman Disiplin dan Tidak Sedang/Pernah Hukuman Pidana")
    // if($tempNamaCheck == "Surat Keterangan Kematian")
    if($tempNamaCheck == "KARPEG")
    {
    // echo $set_detil->query;exit;
    }
    ...
    elseif($tempNamaCheck == "Surat Keterangan Janda/Duda")
    {
      $tempFormatBkn= $tempFormatJenkel."_".$tempNipBaru;
    }
    // echo $tempPath."<br/>";
...
for($index_file=0; $index_file < $indexZip; $index_file++)
{
  ...
  // echo $filelokasiformatbaru."<br/>";exit();
