...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/surat_masuk_pegawai_json/json_dinas/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
		...
		$("#btnCari").on("click", function () {
			...
			oTable.fnReloadAjax("surat/surat_masuk_pegawai_json/json_dinas/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>");
		});
		...
		$('#btnPilih').on('click', function () {
			  ...
			  {*/
				  ...
				  $.messager.confirm('Konfirmasi', "Apakah Anda Yakin, pilih pegawai ?",function(r){
						if (r){
							$.ajax({'url': 'surat/surat_masuk_pegawai_json/add_dinas/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqPegawaiId='+data,'success': function(data) {
								//$("#reqCheckAll").prop('checked', false);
								setCariInfo();
								parent.setCariInfo();
								parent.iframeLoaded();
