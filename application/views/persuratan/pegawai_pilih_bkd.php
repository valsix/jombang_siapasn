...
$reqJenisUsulan= setjenisusulan($reqJenis);
$reqKategori= $this->input->get("reqKategori");
$reqKategoriNama= setkategorinama($reqKategori);
...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/surat_masuk_pegawai_json/json_bkd/?reqStatusUsulan=1&reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqJenisUsulan=<?=$reqJenisUsulan?>&reqKategori=<?=$reqKategori?>",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
		...
		$("#btnCari").on("click", function () {
			...
			oTable.fnReloadAjax("surat/surat_masuk_pegawai_json/json_bkd/?reqStatusUsulan=1&reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqJenisUsulan=<?=$reqJenisUsulan?>&reqKategori=<?=$reqKategori?>");
		});
		...
		$('#btnPilih').on('click', function () {
			  ...
			  {*/
				  ...
				  if(data == "")
				  {
					mbox.alert("Checked data salah satu data terlebih dahulu.", {open_speed: 0});
					   ...
					   buttons: [
						   {
							   ...
							   callback: function() {
								   $.ajax({'url': 'surat/surat_masuk_pegawai_json/add_dinas_bkd/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqJenisUsulan=<?=$reqJenisUsulan?>&reqKategori=<?=$reqKategori?>&reqPegawaiId='+data,'success': function(data) {
										//$("#reqCheckAll").prop('checked', false);
										setCariInfo();
										parent.setCariInfo();
										parent.iframeLoaded();
				  ...
				  /*$.messager.confirm('Konfirmasi', "Apakah Anda Yakin, pilih pegawai ?",function(r){
						if (r){
							$.ajax({'url': 'surat/surat_masuk_pegawai_json/add_dinas_upt/?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqPegawaiId='+data,'success': function(data) {
								//$("#reqCheckAll").prop('checked', false);
								setCariInfo();
								parent.setCariInfo();
								parent.iframeLoaded();
				...
				<div id="breadcrumbs-wrapper">
					<div class="container">
						<div class="row">
							...
							<h5 class="breadcrumbs-title">List Pegawai Usulan Pelayanan <?=$reqJenisNama?> <?=$reqKategoriNama?> yang terverifikasi</h5>
							</div>
						</div>
					</div>
				</div>
