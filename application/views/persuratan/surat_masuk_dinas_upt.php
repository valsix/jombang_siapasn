...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "surat/surat_masuk_upt_json/json_surat/?reqJenis=<?=$reqJenis?>",
        	"sScrollX": "100%",								  
        	"sScrollXInner": "100%",
        ...
        /* RIGHT CLICK EVENT */
        var anSelectedData = '';
        var anSelectedId = anSelectedJenisId= '';
        ...
        $('#btnEdit').on('click', function () {
			...
			newWindow = window.open("app/loadUrl/persuratan/surat_masuk_dinas_upt_add?reqJenis="+anSelectedJenisId+"&reqId="+anSelectedId, 'Cetak'+Math.floor(Math.random()*999999));
        	newWindow.focus();
			tempindextab= parseInt(tempindextab) + 1;
			
			/*$.messager.confirm('Konfirmasi', "Apakah Anda Yakin, menerima surat ini. Pastikan berkas usulan pegawai sudah di diterima ?",function(r){
				if (r){
					var s_url= "surat/surat_masuk_upt_json/baca_surat?reqId="+anSelectedId;
		...
		$("#btnCari").on("click", function () {
			var reqSatuanKerjaId= reqTerbaca= reqCariFilter= "";
			reqSatuanKerjaId= $("#reqSatuanKerjaId").val();
			reqTerbaca= $("#reqTerbaca").val();
			reqCariFilter= $("#reqCariFilter").val();

			oTable.fnReloadAjax("surat/surat_masuk_upt_json/json_surat/?reqJenis=<?=$reqJenis?>&reqTerbaca="+reqTerbaca+"&sSearch="+reqCariFilter);
		});
		
		$("#reqTerbaca").change(function() { 
			setCariInfo();
		});
		...
		$('#btnDelete').on('click', function () {
        	...
        	$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
        		if (r){
        			$.getJSON("surat/surat_masuk_upt_json/delete/?reqRowId="+anSelectedId,
        				function(data){
        					$.messager.alert('Info', data.PESAN, 'info');
        					oTable.fnReloadAjax("surat/surat_masuk_upt_json/json_surat/?reqJenis=<?=$reqJenis?>");
        				});
			...
			<section id="content-full">
				...
				<div class="area-parameter">
					<div class="kiri">
						...
						<span>entries</span>
						<span style="padding-left:5px">Status Surat</span>
						<select id='reqTerbaca'>
