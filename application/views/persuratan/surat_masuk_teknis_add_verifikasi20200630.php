...
$arrInfo="";
$index_data= 0;
//$statement= " AND A.JENIS_PELAYANAN_ID = ".$reqJenis." AND A.TIPE = '0' AND B.PEGAWAI_ID = ".$reqId;
		...
		$("#reqCheckAllAtas, #reqCheckAllBawah").change(function() {
			if($(this).prop('checked')) {
			   ...
			   $('input[id^="reqStatusCheckBoxFix"]').each(function(){
					...
					//reqInfoDownload= $("#reqInfoDownload"+id).val();
					//if(reqInfoDownload == "")
					//{
      ...
      <ul class="collection card">
        ...
        <li class="collection-item">
          ...
          <table class="bordered striped md-text">
          	<thead> 
            	<tr>
                	<th class="grey-text material-font" style="width:20%">Nama</th>
                    <th class="grey-text material-font" style="width:1%">:</th>
                    <th colspan="3" class="deep-purple-text material-font"><?=$reqPegawaiNama?></th>
                </tr>
                <tr>
                	<th class="grey-text material-font">Jabatan</th>
                    <th class="grey-text material-font">:</th>
                    <th colspan="3" class="deep-purple-text material-font"><?=$reqPegawaiJabatan?></th>
                </tr>
                <tr>
                	<th class="grey-text material-font">Pendidikan Diusulkan</th>
                    <th class="grey-text material-font">:</th>
                    <th class="deep-purple-text material-font" style="width:28%"><?=$reqPendidikanNamaUsulan?></th>
                    <th class="grey-text material-font hide-on-small-only" style="width:10%">Jurusan Diusulkan</th>
                    <th class="grey-text material-font hide-on-small-only" style="width:1%">:</th>
                    <th class="deep-purple-text material-font hide-on-small-only"><?=$reqPendidikanJurusanUsulan?></th>
                </tr>
                <tr class="hide-on-med-and-up">
                  <th class="grey-text material-font" style="width:10%">Jurusan</th>
                  <th class="grey-text material-font" style="width:1%">:</th>
                  <th class="deep-purple-text material-font"><?=$reqPendidikanJurusanUsulan?></th>
                </tr>
                <tr>
                	<th class="grey-text material-font">Keterangan Petugas Teknis</th>
                    <th class="grey-textmaterial-font">:</th>
                    <th colspan="4" class="deep-purple-text material-font">
          ...
          <div id="resize">
            ...
            <div id="swipe-1" class="col s12" style="height:auto !important">
                ...
                <thead> 
                  <tr class="ubah-color-warna">
                    ...
                    <th class="white-text material-font" style="width:5%">
            ...
            $tempPath= "";
                ...
                ?>
                      ...
                      <input type="hidden" name="reqKategoriFileId[]" id="reqKategoriFileId<?=$index?>" value="<?=$tempKategoriFileId?>" />
			...
			else
			{
				...
				{
			?>
            <button class="btn waves-effect waves-light green" style="font-size:9pt" type="submit" name="action">
            Simpan
            <i class="mdi-content-save left hide-on-small-only"></i>
            </button>
            <button class="btn pink waves-effect waves-light" style="font-size:9pt" type="button" id="reqcetaksuratkeluar">Cetak Ijin Belajar
            ...
            <?
				...
				{
			?>
            <button class="btn blue waves-effect waves-light" style="font-size:9pt" type="button" id="reqbisadiambil">Bisa di ambil
                <i class="mdi-content-save left hide-on-small-only"></i>
            </button>
            <button class="btn pink waves-effect waves-light" style="font-size:9pt" type="button" id="reqcetaksuratkeluar">Cetak Ijin Belajar
                <i class="mdi-content-inbox left hide-on-small-only"></i>
            ...
            <?
				...
				{
			?>
            <button class="btn blue waves-effect waves-light" style="font-size:9pt" type="button" id="reqrevisiusulnomor">Revisi
                <i class="mdi-content-save left hide-on-small-only"></i>
            </button>
            ...
            <button class="btn blue waves-effect waves-light" style="font-size:9pt" type="button" id="requsulnomor">Usul No IB
                <i class="mdi-content-save left hide-on-small-only"></i>
            </button>
            <?
					}
				}
			//if($tempJumlahChecked == $jumlah_info){}
			if($reqStatusVerifikasi == "1"){}
			else
			{
            ?>
			...
			{
            ?>
            <button class="btn pink waves-effect waves-light" style="font-size:9pt" type="button" id="reqturunstatus">Turun Status
                <i class="mdi-content-inbox left hide-on-small-only"></i>
            </button>
            <input type="hidden" id="reqStatusTms" value="1" />
            <button class="btn pink waves-effect waves-light" style="font-size:9pt" type="button" id="reqtms">TMS
                <i class="mdi-content-inbox left hide-on-small-only"></i>
            </button>
            <?
			}
            ...
            <button class="btn pink waves-effect waves-light" style="font-size:9pt" type="button" id="reqtms">BATAL TMS
                <i class="mdi-content-inbox left hide-on-small-only"></i>
            </button>
            <?
			}
            ...
            <?
			 if($jumlah_revisi_log > 0)
			 {
             ?>
             <table class="bordered striped md-text table_list tabel-responsif" style="margin-top:20px">
             <tr class="ubah-color-warna">
                <th colspan="2" class="white-text material-font">Log Cetak</th>
             </tr>
             <?
			 for($index=0; $index < $jumlah_revisi_log; $index++)
			 {
				$tempCetakIjinBelajar= $arrRevisiLog[$index]["CETAK_IJIN_BELAJAR_ID"];
				$arrRevisiLog[$index]["LAST_USER"];
				$tempTanggal= datetimeToPage($arrRevisiLog[$index]["LAST_DATE"], "datetime");
             ?>
              <tr>
              	<td class="material-font" style="width:20%"><?=$tempTanggal?></td>
                <td class="material-font">
                <button class="btn pink waves-effect waves-light" style="font-size:9pt" type="button" id="reqlogcetaksuratkeluar<?=$tempCetakIjinBelajar?>">Cetak Ijin Belajar
                    <i class="mdi-content-inbox left hide-on-small-only"></i>
                </button>
                </td>
             </tr>
...
$(document).ready( function () {
	...
	$('button[id^="reqlogcetaksuratkeluar"]').click(function() { 
	  var id= $(this).attr('id');
	  var element= id.split('reqlogcetaksuratkeluar');
      var reqrowid= element[1];
	  //alert(id);return false;
	  requrl= "cetak_ijin_belajar_surat_keluar";
	  newWindow = window.open("app/loadUrl/persuratan/cetak_pdf?reqCss=surat_pengantar&reqUrl="+requrl+"&reqUrlList=&reqId="+reqrowid, 'Cetak');
	  newWindow.focus();
	});
	
	$("#reqcetaksuratkeluar").click(function() { 
	  requrl= "ijin_belajar_surat_keluar";
	  newWindow = window.open("app/loadUrl/persuratan/cetak_pdf?reqCss=surat_pengantar&reqUrl="+requrl+"&reqUrlList=&reqId=<?=$reqRowId?>", 'Cetak');
	  newWindow.focus();
	});
	
	$("#reqbisadiambil").click(function() { 
		info= "Apakah yakin untuk ambil surat keluar untuk an. <?=$reqPegawaiNama?>";
		mbox.custom({
		   message: info,
		   ...
		   buttons: [
			   {
				   ...
				   callback: function() {
					   $.ajax({'url': "surat/cetak_ijin_belajar_json/add/?reqRowId=<?=$reqRowId?>",'success': function(datahtml) {
					   	document.location.href= "app/loadUrl/persuratan/surat_masuk_teknis_add_verifikasi?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>";
					   }});
					   mbox.close();
				   }
	...
	$.ajax({'url': s_url,'success': function(dataajax){
		if(dataajax == '2' || dataajax == '3')
		{
			...
			//$.messager.alert('Info', "Surat tidak bisa di batalkan", 'info');
			document.location.href= "app/loadUrl/persuratan/surat_masuk_teknis_add_verifikasi?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>";
		}
		else
		{
			info= "Apakah yakin untuk membatalkan nomor usul surat untuk an. <?=$reqPegawaiNama?>";
			mbox.custom({
			   message: info,
			   options: {close_speed: 100},
			   buttons: [
				   {
					   ...
					   color: 'green darken-2',
					   callback: function() {
						   $.ajax({'url': "surat/surat_masuk_pegawai_check_json/status_surat_keluar/?reqRowId=<?=$reqRowId?>&reqStatusSuratKeluar=2",'success': function(datahtml) {
							document.location.href= "app/loadUrl/persuratan/surat_masuk_teknis_add_verifikasi?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>";
						   }});
						   mbox.close();
					   }
	...
	$("#requsulnomor").click(function() { 
		info= "Apakah yakin untuk buat nomor usul surat untuk an. <?=$reqPegawaiNama?>";
		mbox.custom({
		   message: info,
		   options: {close_speed: 100},
		   buttons: [
			   {
				   ...
				   color: 'green darken-2',
				   callback: function() {
					   $.ajax({'url': "surat/surat_masuk_pegawai_check_json/status_surat_keluar/?reqRowId=<?=$reqRowId?>&reqStatusSuratKeluar=1",'success': function(datahtml) {
						document.location.href= "app/loadUrl/persuratan/surat_masuk_teknis_add_verifikasi?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>";
					   }});
					   mbox.close();
				   }
	...
	$("#reqturunstatus").click(function() { 
		parent.openModal('app/loadUrl/persuratan/surat_masuk_teknis_add_turun_status?reqId=<?=$reqId?>&reqJenis=<?=$reqJenis?>&reqRowId=<?=$reqRowId?>')
	});
