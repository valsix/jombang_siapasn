...
$arrtabledata= array(
  ...
  , array("label"=>"Tingkat Hukuman", "field"=> "TINGKAT_HUKUMAN_NAMA", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
  , array("label"=>"Jenis Hukuman", "field"=> "JENIS_HUKUMAN_NAMA", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
  , array("label"=>"TMT Hukdis", "field"=> "TMT_SK", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
  , array("label"=>"Tanggal Akhir", "field"=> "TANGGAL_AKHIR", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
  ...
  , array("label"=>"fieldrowid", "field"=> "HUKUMAN_ID", "display"=>"1", "width"=>"")
    ...
    <div class="wrapper">
        <section id="content-full">
                ...
                <div class="container">
                    <div class="row">
                        <div class="col s12 m12 l12">
                            ...
                            <h5 class="breadcrumbs-title">Sinkron Hukuman Disiplin</h5>
            ...
            <div class="area-parameter no-marginbottom">
              ...
              <div id="settoggle">
                <div class="row">
                  <div class="col s3 m2">Dasar Aturan</div>
                  <div class="col s2 m1 select-semicolon">:</div>
                  <div class="col s3 m2">
                    <select id="reqPeraturanId">
                    <option value="" <? if("" == $reqPeraturanId) echo "selected";?>>Semua</option>
	                  <?
                    foreach($arrperaturan as $item) 
                    {
                      $selectvalid= $item["PERATURAN_ID"];
                      $selectvaltext= $item["NAMA"];
	                  ?>
	                    <option value="<?=$selectvalid?>" <? if($selectvalid == $reqPeraturanId) echo "selected";?> ><?=$selectvaltext?></option>
	                  <?
	                  }
	                  ?>
	                  </select>
                  ...
                  <div class="col s3 m2">Tahun TMT</div>
                  <div class="col s2 m1 select-semicolon">:</div>
                  <div class="col s3 m1">
                ...
                <div class="row">
                  <div class="col s3 m2">Tingkat Hukuman</div>
                  <div class="col s2 m1 select-semicolon">:</div>
                  <div class="col s3 m2">
                    <select id="reqTingkatHukumanId">
                      <option value="" <? if("" == $reqJenisHukumanId) echo "selected";?>>Semua</option>
                    </select>
                  </div>

                  <div class="col s3 m2">Jenis Hukuman</div>
                  <div class="col s2 m1 select-semicolon">:</div>
                  <div class="col s3 m2">
                    <select id="reqJenisHukumanId">
                      <option value="" <? if("" == $reqJenisHukumanId) echo "selected";?>>Semua</option>
                    </select>
                  </div>
...
<script type="text/javascript">
    ...
    $("#btncetak").on("click", function () {
      var reqGlobalValidasiCheck= reqCariFilter= reqSatuanKerjaId= reqPeraturanId= reqPeraturanId= reqPeraturanId= "";
      // reqPencarian= $('#example_filter input').val();
      reqSatuanKerjaId= $("#reqSatuanKerjaId").val();
      reqCariFilter= $("#reqCariFilter").val();
      reqPeraturanId= $("#reqPeraturanId").val();
      reqTipePegawaiId= $("#reqTipePegawaiId").val();
      reqEselonGroupId= $("#reqEselonGroupId").val();

      newWindow = window.open("app/loadUrl/app/talenta_rekam_jejak_excel.php?reqBulan=<?=date('m')?>&reqTahun=<?=date('Y')?>&reqPencarian="+reqCariFilter+"&reqSatuanKerjaId="+reqSatuanKerjaId+"&reqPeraturanId="+reqPeraturanId+"&reqTipePegawaiId="+reqTipePegawaiId+"&reqEselonGroupId="+reqEselonGroupId, 'Cetak');
      newWindow.focus();
    });
    ...
    $("#btnEdit").on("click", function () {
      ...
      ajaxurl= "json/sepadan_json/sessdetil?m=hukuman&reqId="+valinfoid+"&reqRowId="+valinforowid;
    ...
    $('#btnCari').on('click', function () {
      var reqGlobalValidasiCheck= reqCariFilter= reqSatuanKerjaId= reqPeraturanId= reqTingkatHukumanId= reqJenisHukumanId= reqTahun= "";
      // reqPencarian= $('#example_filter input').val();
      reqSatuanKerjaId= $("#reqSatuanKerjaId").val();
      reqCariFilter= $("#reqCariFilter").val();
      reqPeraturanId= $("#reqPeraturanId").val();
      ...
      jsonurl= "json/sepadan_json/jsonhukuman?reqPencarian="+reqCariFilter+"&reqSatuanKerjaId="+reqSatuanKerjaId+"&reqPeraturanId="+reqPeraturanId+"&reqTingkatHukumanId="+reqTingkatHukumanId+"&reqJenisHukumanId="+reqJenisHukumanId+"&reqTahun="+reqTahun;
      datanewtable.DataTable().ajax.url(jsonurl).load();
    });

    $("#reqPeraturanId,#reqTingkatHukumanId,#reqJenisHukumanId").change(function() {
        setCariInfo();
        // reloadglobalklikcheck();
    });
    ...
    $("#reqPeraturanId").change(function() { 
      ...
      var items = "<option value=''>Semua</option>";
      infoid= $(this).val();
    ...
    jQuery(document).ready(function() {
      var jsonurl= "json/sepadan_json/jsonhukuman";
