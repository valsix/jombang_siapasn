...
<head>
  ...
  <style type="text/css">
    ...
    (min-device-width: 768px) and (max-device-width: 1024px)  {
      ...
      */
      <?
      $arrData= array("Status Pendidikan", "TK Pendidikan", "Jurusan", "Tgl. STTB", "Status", "Aksi");
        ...
        $.ajax({'url': s_url,'success': function(msg){
          if(msg == ''){}
            else
            {
              ...
              document.location.href= "app/loadUrl/app/pegawai_add_pendidikan_monitoring/?reqId="+pegawai_id;
            }
          }});
      }
    });  
  ...
  <div class="row">
    ...
    ?>
      <ul class="collection card">
        <li class="collection-item ubah-color-warna">RIWAYAT PENDIDIKAN</li>
        <li class="collection-item">
          <div class="row">
            ...
            ?>
              ...
              {
              ?>
              onClick="parent.setappload('pegawai_add_pendidikan_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>&pelayananid=<?=$pelayananid?>&pelayananjenis=<?=$pelayananjenis?>&pelayananrowid=<?=$pelayananrowid?>&pelayanankembali=<?=$pelayanankembali?>')"
              <?
              }
              else
              {
              ?>
              onClick="parent.setload('pegawai_add_pendidikan_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')"
              <?
              }
              ?>
            ...
            {
            ?>
              <a style="cursor:pointer; margin-left: 20px" onClick="parent.setload('pegawai_add_pendidikan_monitoring_bkn?reqId=<?=$reqId?>')"> <i class="mdi-action-view-list"> <span class=" material-font">BKN</span></i></a>
            <?
            }
            ?>
          </div>
          <table class="bordered striped md-text table_list tabel-responsif responsive-table" id="link-table">
            ...
            <tbody>
              ...
              while($set->nextRow()){
                ?>
                <tr>
                  <td><?=$set->getField('STATUS_PENDIDIKAN_NAMA')?></td>
                  <td><?=$set->getField('PENDIDIKAN_NAMA')?></td>
                  <td><?=$set->getField('JURUSAN')?></td>
                  <td><?=dateToPageCheck($set->getField('TANGGAL_STTB'))?></td>
                  <td>
                    <?
                  ...
                  <td width="90px">
                    ...
                    <span>
                      ...
                      <span>
                        <a href="javascript:void(0)" class="round waves-effect waves-light purple white-text" title="Log"  onclick="parent.openModal('app/loadUrl/app/informasi_data_log?reqjson=pendidikan_riwayat_json/log/<?=$reqRowId?>&reqjudul=Data Pendidikan Riwayat Log')">
