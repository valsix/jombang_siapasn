...
<script type="text/javascript" charset="utf-8">
  ...
  $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
         ...
         "bProcessing": true,
         "bServerSide": true,
         "sAjaxSource": "hari_libur_json/json?reqTahun=<?=$reqTahun?>",
         "sScrollX": "100%",								  
         "sScrollXInner": "100%",
   ...
   $("#btnCari").on("click", function () {
		tempJsonParsing= reqTahun= "";
		reqTahun= $("#reqTahun").val();
		//setJsonFilter();
		//alert(tempJsonParsing);
		oTable.fnReloadAjax("hari_libur_json/json?"+tempJsonParsing+"&reqTahun="+reqTahun);
	});

   $('#btnAdd').on('click', function () {
       window.parent.openPopup("app/loadUrl/app/hari_libur_add");
   ...
   $('#btnEdit').on('click', function () {
       if(anSelectedData == "")
          return false;				
      window.parent.openPopup("app/loadUrl/app/hari_libur_add/?reqId="+anSelectedId);
   ...
   $('#btnDelete').on('click', function () {
      ...
      $.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
          if (r){
             $.getJSON("hari_libur_json/delete/?reqId="+anSelectedId,
                function(data){
                   $.messager.alert('Info', data.PESAN, 'info');
                   oTable.fnReloadAjax("hari_libur_json/json");
               });
   ...
   $('#btnLog').on('click', function () {
       window.parent.openPopup("app/loadUrl/app/informasi_data_log?reqjson=hari_libur_json/log&reqjudul=Bank Log");
...
function hapusData(id, statusaktif)
{
    ...
    $.messager.confirm('Konfirmasi', infoMode+" ?",function(r){
        if (r){
            var s_url= "hari_libur_json/delete/?reqMode="+reqmode+"&reqId="+id;
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col s12 m12 l12">
                                ...
                                <h5 class="breadcrumbs-title">Hari Libur</h5>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="container" style="clear:both;">
                    <div class="section">
                        <table id="example" class="display mdl-data-table dt-responsive" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                	<th width="110px">Tanggal Awal</th> 
                                    <th width="120px">Tanggal Akhir</th>
