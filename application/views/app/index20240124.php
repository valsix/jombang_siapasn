...
/*if($tempUserLoginId == "1")
{

  $arrMenu[$index_set]["MENU_ID"]= "28";
  $arrMenu[$index_set]["MENU_PARENT_ID"]= "0";
  $arrMenu[$index_set]["NAMA"]= "Layanan Kanreg II BKN";
  $arrMenu[$index_set]["LINK_FILE"]= "soap_agenda_layanan_kanreg";
  $arrMenu[$index_set]["AKSES"]= "A";
  $arrMenu[$index_set]["JUMLAH_CHILD"]= "0";
  $arrMenu[$index_set]["JUMLAH_MENU"]= "1";
  $arrMenu[$index_set]["JUMLAH_DISABLE"]= "0";
  $index_set++;
}*/

//echo $this->AKSES_APP_SIMPEG_ID;exit;
//selectByParamsMenu(1, $akses_id, "AKSES_APP_ZIS", " AND A.MENU_PARENT_ID = '".$id_induk."'");
$set->selectByParamsMenu(1, $this->AKSES_APP_SIMPEG_ID, "AKSES_APP_SIMPEG", " AND A.STATUS IS NULL", "ORDER BY COALESCE(A.URUT, 999), A.MENU_ID");
// echo $set->query;exit;
while($set->nextRow())
{
	$arrMenu[$index_set]["MENU_ID"]= $set->getField("MENU_ID");
  $arrMenu[$index_set]["MENU_PARENT_ID"]= $set->getField("MENU_PARENT_ID");
	$arrMenu[$index_set]["MENU_ICON_ID"]= $set->getField("MENU_ICON_ID");
...
<div class="wrapper">
  ...
  <header class="main-header">
    ...
    <a href="app/index" class="logo">
      ...
      <span class="logo-mini"><img src="images/s_logo.png"></span>
    ...
    <nav class="navbar navbar-static-top">
      ...
      <div class="navbar-custom-menu">
        <ul class="nav navbar-nav">
		  ...
		  <?
          if($tempUserLoginId == "1" || $tempUserLoginId == "376")
          {
          ?>
          <!-- kalau admin muncul -->
          <li class="dropdown messages-menu">
            <a href="sidak/index" class="dropdown-toggle">
          ...
          <li class="dropdown user user-menu">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
              ...
              <img src="<?=$tempUserLoginInfoPath?>" width="160" height="160" class="user-image" alt="User Image" />
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <?
                for($index_detil=0; $index_detil < count($arrayKey); $index_detil++)
                {
                  ...
                  $infoiconid= $arrMenu[$index_row]["MENU_ICON_ID"];
        ...
        <?
            ...
            elseif(isStrContain($tempLinkFile, "soap_agenda_layanan_kanreg") == true)
            $tempUrl= "app/loadUrl/app/".$tempLinkFile."&reqAkses=".$tempAkses."&reqBreadCrum=".$tempInfoNama;
            elseif(isStrContain($tempLinkFile, "validasi") == true)
            $tempUrl= "app/loadUrl/verifikasi/".$tempLinkFile."&reqAkses=".$tempAkses."&reqBreadCrum=".$tempInfoNama;
						else
						$tempUrl= "app/loadUrl/persuratan/".$tempLinkFile."&reqAkses=".$tempAkses."&reqBreadCrum=".$tempInfoNama;
		?>
                <li>
                  <a href="<?=$tempUrl?>" <?php /*?>onClick="setBreacrum('<?=$tempInfoNama?>');"<?php */?> target="mainFrame"><i class="<?=$infoiconid?>"></i> <?=$tempNama?></a>
                </li>
        <?
					}
                }
        ...
        <?
                  ...
                  else
                  {
        						...
        						{
        ?>
        		          <li><a href="nopage" target="mainFrame"><i class="<?=$infoiconid?>"></i> <?=$tempNama?></a></li>
        <?
						        }
						        else
						        {
        							$tempUrl= "";
                      if(isStrContain($tempLinkFile, "validasi") == true)
                      $tempUrl= "app/loadUrl/verifikasi/".$tempLinkFile."?reqAkses=".$tempAkses."&reqBreadCrum=".$tempInfoNama;
                      elseif(isStrContain($tempLinkFile, "?") == false)
                      $tempUrl= "app/loadUrl/app/".$tempLinkFile."?reqAkses=".$tempAkses."&reqBreadCrum=".$tempInfoNama;
        							else
        							$tempUrl= "app/loadUrl/persuratan/".$tempLinkFile."&reqAkses=".$tempAkses."&reqBreadCrum=".$tempInfoNama;
		?>
        		        <li>
                      <a href="<?=$tempUrl?>" target="mainFrame"><i class="<?=$infoiconid?>"></i> <?=$tempNama?></a>
                    </li>
        <?
						        }
                  }
  ...
  <div class="content-wrapper">
    ...
    <section class="content area-main">
        ...
        <div class="konten-utama">
            ...
            <!-- <iframe src="<?=site_url('app/loadUrl/persuratan/surat_masuk?1=1&reqAkses=A&reqBreadCrum=Administrasi Persuratan;Surat Masuk')?>" name="mainFrame" id="mainFrame" onload="iframeIndexLoaded()" style="height: 100% !important; position: relative !important;" ></iframe> -->
