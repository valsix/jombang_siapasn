...
<script type="text/javascript" charset="utf-8">
  ...
  $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
         ...
         "bProcessing": true,
         "bServerSide": true,
         "sAjaxSource": "tanda_tangan_json/json",
         "sScrollX": "100%",								  
         "sScrollXInner": "100%",
   ...
   $("#btnCari").on("click", function () {
    tempJsonParsing= "";
              ...
              oTable.fnReloadAjax("tanda_tangan_json/json?"+tempJsonParsing);
          });

   $('#btnAdd').on('click', function () {
       window.parent.openPopup("app/loadUrl/app/tanda_tangan_bkd_add");
   ...
   $('#btnEdit').on('click', function () {
       if(anSelectedData == "")
          return false;				
      window.parent.openPopup("app/loadUrl/app/tanda_tangan_bkd_add/?reqId="+anSelectedId);
   ...
   $('#btnDelete').on('click', function () {
      ...
      $.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
          if (r){
             $.getJSON("tanda_tangan_json/delete/?reqId="+anSelectedId,
                function(data){
                   $.messager.alert('Info', data.PESAN, 'info');
                   oTable.fnReloadAjax("tanda_tangan_json/json");
               });
   ...
   $('#btnLog').on('click', function () {
       window.parent.openPopup("app/loadUrl/app/informasi_data_log?reqjson=tanda_tangan_json/log&reqjudul=Tanda Tangan Log");
...
function hapusData(id, statusaktif)
{
    ...
    $.messager.confirm('Konfirmasi', infoMode+" ?",function(r){
        if (r){
            var s_url= "tanda_tangan_json/delete/?reqMode="+reqmode+"&reqId="+id;
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col s12 m12 l12">
                                ...
                                <h5 class="breadcrumbs-title">Tanda Tangan</h5>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table id="example" class="display mdl-data-table dt-responsive" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    <th>Mulai Berlaku</th>   
                                    <th>Akhir Berlaku</th>   
                                    <th>No Nomenklatur Kab</th>   
                                    <th>No Nomenklatur BKD</th>   
                                    <th>Nama</th>   
                                    <th>PLT Jabatan</th>   
                                    <th>Nama Pejabat</th>   
                                    <th>Kode Pangkat</th>   
