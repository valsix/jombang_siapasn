     ...
     if($reqBreadCrum == ""){}
        ...
        $("#btnCari").on("click", function () {
            tempJsonParsing= "";
              ...
              oTable.fnReloadAjax("eselon_json/json?"+tempJsonParsing);
          });

        $("#setIconCari").hide();

        $('#btnAdd').on('click', function () {
        	window.parent.openPopup("app/loadUrl/app/eselon_add");
        ...
        $('#btnDelete').on('click', function () {
        	...
        	$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
        		if (r){
        			$.getJSON("eselon_json/delete/?reqId="+anSelectedId,
        				function(data){
        					$.messager.alert('Info', data.PESAN, 'info');
        					oTable.fnReloadAjax("eselon_json/json");
        				});
        ...
        $('#btnLog').on('click', function () {
            window.parent.openPopup("app/loadUrl/app/informasi_data_log?reqjson=eselon_json/log&reqjudul=Eselon Log");
...
function hapusData(id, statusaktif)
{
    ...
    if(statusaktif == "")
    {
        ...
        $.messager.confirm('Konfirmasi', infoMode+" ?",function(r){
            if (r){
                var s_url= "eselon_json/delete/?reqMode="+reqmode+"&reqId="+id;
                ...
                <div class="container">
                    <div class="row">
                        <div class="col s12 m12 l12">
                            ...
                            <h5 class="breadcrumbs-title">Eselon</h5>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="section">
                    <table id="example" class="display mdl-data-table dt-responsive" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>Nama</th>                                       
                                <th>Pangkat Minimal</th>                                       
                                <th>Pangkat Maksimal</th>                                       
                                <th>Aksi</th>  
                            </tr>
                        </thead>
                    </table>
