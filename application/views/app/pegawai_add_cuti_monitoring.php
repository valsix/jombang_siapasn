  ...
  <div class="row">
    <div class="col s12 m10 offset-m1 ">
      <ul class="collection card">
        <li class="collection-item ubah-color-warna">RIWAYAT CUTI</li>
        <li class="collection-item">
          <div class="row">
            ...
            {
            ?>
            <a style="cursor:pointer" onClick="parent.setload('pegawai_add_cuti_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')"> <i class="mdi-content-add-circle-outline"> <span class=" material-font">Tambah</span></i></a>
            <?
            }
            ?>
          </div>
          <table class="bordered striped md-text table_list tabel-responsif responsive-table" id="link-table">
            <thead> 
              <tr>
                <th class="green-text material-font">Tahun</th>
                <th class="green-text material-font">Jenis Cuti</th>
                <th class="green-text material-font">No Surat</th>
                <th class="green-text material-font">Tgl. Mulai</th>                                                          
                <th class="green-text material-font">Lama</th>                             
            ...
            <tbody>
              <? while($set->nextRow()){?>
              <tr>
                <!-- <td><a href="cuti_detil.php?reqPegawaiId=<?=$set->getField('PEGAWAI_ID')?>&reqCUTI_ID=<?=$set->getField('CUTI_ID')?>&reqMode=view">link data</a></td> -->
                <td><?=getYear($set->getField('TANGGAL_MULAI'))?></td>
                <td><?=$set->getField('JENIS_CUTI_NAMA')?></td>
                <td><?=$set->getField('NO_SURAT')?></td>
                <td><?=dateToPageCheck($set->getField('TANGGAL_MULAI'))?></td>
               
                <td><?=$set->getField('CUTI_KETERANGAN')?></td>
                <td>
                  <?
                ...
                <td width="90px">
                  ...
                  <span>
                        <a href="javascript:void(0)" class="round waves-effect waves-light purple white-text" title="Log"  onclick="parent.openModal('app/loadUrl/app/informasi_data_log?reqjson=cuti_json/log/<?=$reqRowId?>&reqjudul=Data Cuti Log')">
                      <i class="mdi-action-query-builder"></i>
                    </a>
                  </span>
                  <span>
                    <?if($set->getField('STATUS')!=1):?>
                    <a href="javascript:void(0)" class="round waves-effect waves-light blue white-text" title="Ubah" onClick="parent.setload('pegawai_add_cuti_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')">
                  ...
                  <!-- <a href="javascript:void(0)" class="rounded material-bold green white-text" onclick="addData('')">TAMBAH</a> -->
                  <!-- <span>
                      <a href="javascript:void(0)" class="rounded material-bold blue white-text" onclick="parent.setload('pegawai_add_cuti_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')">UBAH</a>
                    </span>
                    <span>
                     <?if($set->getField('STATUS')==1):?>
