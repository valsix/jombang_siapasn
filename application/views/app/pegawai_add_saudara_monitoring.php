...
<head>
  ...
  <style type="text/css">
    ...
    (min-device-width: 768px) and (max-device-width: 1024px)  {
      ...
      */
      <?
      $arrData= array("Nama", "Tempat Lahir", "Tanggal Lahir", "Jenis Kelamin", "Status Saudara", "Satatus Aktif", "Last Create/ Update", "Last User Access", "Status", "Aksi");
  ...
  <script type="text/javascript">
  	function calllog()
   {
    window.parent.openPopup("app/loadUrl/app/informasi_data_log?reqjson=bank_json/log&reqjudul=Bank Log");
  }
  ...
  function hapusdata(id, statusaktif, pegawai_id)
  {
    ...
    $.messager.confirm('Konfirmasi', infoMode+" ?",function(r){
     if (r){
      var s_url= "saudara_json/delete/?reqMode="+reqmode+"&reqId="+id;
        ...
        $.ajax({'url': s_url,'success': function(msg){
          if(msg == ''){}
            else
            {
              ...
              document.location.href= "app/loadUrl/app/pegawai_add_saudara_monitoring/?reqId="+pegawai_id;
            }
          }});
      }
    });  
  ...
  <div class="row">
    <div class="col s12 m10 offset-m1 ">
      <ul class="collection card">
        <li class="collection-item ubah-color-warna">SAUDARA</a></li>
        <li class="collection-item">
          <div class="row">
            ...
            {
            ?>
            <a style="cursor:pointer" onClick="parent.setload('pegawai_add_saudara_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')"> <i class="mdi-content-add-circle-outline"> <span class=" material-font">Tambah</span></i></a>
            <?
            }
            ?>
          </div>
          <table class="bordered striped md-text table_list tabel-responsif responsive-table" id="link-table">
            <thead> 
              <tr>
                ...
                <th class="green-text material-font">Status Saudara</th>                         
                <th class="green-text material-font">Satatus Aktif</th> 
            ...
            <tbody>
              ...
              <tr class="md-text">
                ...
                <td><?=$set->getField('TANGGAL_LAHIR')?></td>
                <td><?=$set->getField('JENIS_KELAMIN')?></td>
                <td><?=$set->getField('STATUS_SDR_NAMA')?></td>
                <td><?=$set->getField('STATUS_HIDUP_NAMA')?></td>
                ...
                <td width="90px">
                  <?$reqRowId = $set->getField('SAUDARA_ID');
                  ...
                  <span>
                    <a href="javascript:void(0)" class="round waves-effect waves-light purple white-text" title="Log"  onclick="parent.openModal('app/loadUrl/app/informasi_data_log?reqjson=saudara_json/log/<?=$reqRowId?>&reqjudul=Data Saudara Log')">
                  ...
                  <span>
                    <?if($set->getField('STATUS')!=1):?>
                    <a href="javascript:void(0)" class="round waves-effect waves-light blue white-text" title="Ubah" onClick="parent.setload('pegawai_add_saudara_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')">
                ...
                <!--  <span>
                   <a href="javascript:void(0)" class="rounded material-bold blue white-text" onclick="parent.setload('pegawai_add_saudara_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')">UBAH</a>
                 </span>
                 <span>
                   <?if($set->getField('STATUS')==1):?>
