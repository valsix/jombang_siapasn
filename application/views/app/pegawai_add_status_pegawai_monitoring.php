...
$arrData= array("TMT", "Status Pegawai", "Status Kedudukan", "Status", "Aksi");
        ...
        $.ajax({'url': s_url,'success': function(msg){
          if(msg == ''){}
            else
            {
              ...
              document.location.href= "app/loadUrl/app/pegawai_add_status_pegawai_monitoring/?reqId="+pegawai_id;
            }
          }});
      }
    });  
  ...
  <div class="row">
    <div class="col s12 m10 offset-m1 ">
      ...
      <ul class="collection card">
        <li class="collection-item ubah-color-warna">RIWAYAT STATUS PEGAWAI</li>
        <li class="collection-item">
          <div class="row">
            ...
            {
            ?>
            <a style="cursor:pointer" onClick="parent.setload('pegawai_add_status_pegawai_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')"> <i class="mdi-content-add-circle-outline"> <span class=" material-font">Tambah</span></i></a>
            <?
            }
            ?>
          </div>
          <table class="bordered striped md-text table_list tabel-responsif responsive-table" id="link-table">
            ...
            <tbody>
              ...
              while($set->nextRow())
              {
                ...
                ?>
                <tr>
                  <td><?=dateToPageCheck($set->getField('TMT'))?></td>
                  <td><?=$set->getField('STATUS_PEGAWAI_INFO')?></td>
                  <td><?=$set->getField('KEDUDUKAN_INFO')?></td>
                  <td>
                    <?
                  ...
                  <td width="90px">
                    ...
                    <span>
                      <?if($set->getField('STATUS')!=1):?>
                      <a href="javascript:void(0)" class="round waves-effect waves-light blue white-text" title="Ubah" onClick="parent.setload('pegawai_add_status_pegawai_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')">
