...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
        	...
        	"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "user_login_json/json",			  
        ...
        $("#btnCari").on("click", function () {
			...
			reqCariFilter= $("#reqCariFilter").val();
				  ...
				  oTable.fnReloadAjax("user_login_json/json?sSearch="+reqCariFilter);
				});
        ...
        $('#btnAdd,#btnEdit').on('click', function () {
	        ...
	        if(anSelectedId == "" && btnid == "btnEdit")
	        {
	            $.messager.alert('Info', "Pilih salah satu data terlebih dahulu.", 'warning');
	            return false;
	        }

	        varurl= "app/loadUrl/app/user_login_add/?reqId="+anSelectedId;
	        window.parent.openPopup(varurl);
	        // document.location.href = varurl;
	        $('div.flexmenumobile').hide()
        	$('div.flexoverlay').css('display', 'none')
        });

        $('#btnLoginFile').on('click', function () {
        	if(anSelectedData == "")
        		return false;

        	window.parent.openPopup("app/loadUrl/app/user_login_file/?reqId="+anSelectedId);
        	$('div.flexmenumobile').hide()
        	$('div.flexoverlay').css('display', 'none')
        });
        ...
        $('#btnReset').on('click', function () {
        	if(anSelectedData == "")
        		return false;				
        	window.parent.openPopup("app/loadUrl/app/user_login_reset/?reqId="+anSelectedId);
        ...
        $('#btnDelete').on('click', function () {
        	...
        	$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
        		if (r){
        			$.getJSON("user_login_json/delete/?reqId="+anSelectedId,
        				function(data){
        ...
        $('#btnLog').on('click', function () {
        	window.parent.openPopup("app/loadUrl/app/informasi_data_log?reqjson=user_login_json/log&reqjudul=User Login Log");
...
function hapusData(id, statusaktif)
{
	...
	reqmode= "user_login_1";
	infoMode= "Apakah anda yakin menonaktifkan data terpilih" //selain di user login, seharusnya 1 = non aktif
	if(statusaktif == "")
	{
		reqmode= "user_login_0";
		infoMode= "Apakah anda yakin mengaktifkan data terpilih" //selain di user login, 0 = aktif
	}

	$.messager.confirm('Konfirmasi', infoMode+" ?",function(r){
		if (r){
			var s_url= "user_login_json/delete/?reqMode="+reqmode+"&reqId="+id;
					...
					<div class="container">
						<div class="row">
							<div class="col s12 m12 l12">
								...
								<h5 class="breadcrumbs-title">User Login</h5>
							</div>
						</div>
					</div>
				</div>
				...
				<div id="bluemenu" class="bluetabs">
					<ul>
						<li>
							...
							<a id="btnReset" title="Reset"><img src="images/icon-edit.png" /> Reset</a>
							<a id="btnLoginFile" title="File Kondisi"><img src="images/icon-edit.png" /> File Kondisi</a>
