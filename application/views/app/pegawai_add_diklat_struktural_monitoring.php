...
$arrData= array("Jenjang Diklat", "No. STTPP", "Angkatan", "Tahun", "Penyelenggara", "Status", "Aksi");
        ...
        $.ajax({'url': s_url,'success': function(msg){
          if(msg == ''){}
            else
            {
              ...
              document.location.href= "app/loadUrl/app/pegawai_add_diklat_struktural_monitoring/?reqId="+pegawai_id;
            }
          }});
      }
    });  
  ...
  <div class="row">
    ...
    ?>
      <ul class="collection card">
        <li class="collection-item ubah-color-warna">RIWAYAT DIKLAT STRUKTURAL</li>
        <li class="collection-item">
          <div class="row">
            ...
            ?>
              ...
              {
              ?>
              onClick="parent.setappload('pegawai_add_diklat_struktural_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>&pelayananid=<?=$pelayananid?>&pelayananjenis=<?=$pelayananjenis?>&pelayananrowid=<?=$pelayananrowid?>&pelayanankembali=<?=$pelayanankembali?>')"
              <?
              }
              else
              {
              ?>
              onClick="parent.setload('pegawai_add_diklat_struktural_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')"
              <?
              }
              ?>
            ...
            {
            ?>
              <a style="cursor:pointer; margin-left: 20px" onClick="parent.setload('pegawai_add_diklat_struktural_monitoring_bkn?reqId=<?=$reqId?>')"> <i class="mdi-action-view-list"> <span class=" material-font">BKN</span></i></a>
            <?
            }
            ?>
          </div>
          <table class="bordered striped md-text table_list tabel-responsif responsive-table" id="link-table">
            ...
            <tbody>
              ...
              while($set->nextRow())
              {
                $reqRowId = $set->getField('DIKLAT_STRUKTURAL_ID');
                $reqId = $set->getField('PEGAWAI_ID');
                ?>
                <tr>
                  <td><?=$set->getField('NAMA')?></td>
                  <td><?=$set->getField('NO_STTPP')?></td>
                  <td><?=$set->getField('ANGKATAN')?></td>
                  <td><?=$set->getField('TAHUN')?></td>
                  <td><?=$set->getField('PENYELENGGARA')?></td>
                  <td>
                    <?
                  ...
                  <td width="90px">
                    ...
                    <span>
                        <a href="javascript:void(0)" class="round waves-effect waves-light purple white-text" title="Log"  onclick="parent.openModal('app/loadUrl/app/informasi_data_log?reqjson=diklat_struktural_json/log/<?=$reqRowId?>&reqjudul=Data Diklat Struktural Log')">
                  ...
                  <!--   <span>
                      <a href="javascript:void(0)" class="rounded material-bold blue white-text" onClick="parent.setload('pegawai_add_diklat_struktural_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')">UBAH</a>
                    </span>
                    <span>
                      <?if($set->getField('STATUS')==1):?>
