...
<head>
  ...
  <style type="text/css">
    ...
    (min-device-width: 768px) and (max-device-width: 1024px)  {
      ...
      */
      <?
      $arrData= array("Nama", "Tempat Lahir", "Jenis Kelamin", "Pekerjaan", "Alamat", "Last Create/Update", "Last User Access", "Status", "Aksi");
        ...
        $.ajax({'url': s_url,'success': function(msg){
          if(msg == ''){}
            else
            {
              ...
              document.location.href= "app/loadUrl/app/pegawai_add_suami_istri_monitoring/?reqId="+pegawai_id;
            }
          }});
      }
    });  
  ...
  <div class="row">
    ...
    ?>
      <ul class="collection card">
        <li class="collection-item ubah-color-warna">SUAMI / ISTRI</li>
        <li class="collection-item">
          ...
          <div class="row">
            ...
            ?>
              ...
              {
              ?>
              onClick="parent.setappload('pegawai_add_suami_istri_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>&pelayananid=<?=$pelayananid?>&pelayananjenis=<?=$pelayananjenis?>&pelayananrowid=<?=$pelayananrowid?>&pelayanankembali=<?=$pelayanankembali?>')"
              <?
              }
              else
              {
              ?>
              onClick="parent.setload('pegawai_add_suami_istri_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')"
              <?
              }
              ?>
            ...
            {
            ?>
              <a style="cursor:pointer; margin-left: 20px" onClick="parent.setload('pegawai_add_suami_istri_monitoring_bkn?reqId=<?=$reqId?>')"> <i class="mdi-action-view-list"> <span class=" material-font">BKN</span></i></a>
            <?
            }
            ?>
          </div>
          ...
          <table class="bordered striped md-text table_list tabel-responsif responsive-table" id="link-table">
            <thead> 
              <tr>
                <th class="green-text material-font">Nama Suami/Istri</th>
                <th class="green-text material-font">Tanggal Lahir</th>
                <th class="green-text material-font">Tempat Lahir</th>
                <th class="green-text material-font">Karis/Karsu</th>
                <th class="green-text material-font">Tgl nikah</th>
                <th class="green-text material-font">Nip</th>
                <!-- <th style="display:none">Pendidikan</th> -->
                <th class="green-text material-font">Pekerjaan</th>
                <th class="green-text material-font">Status Nikah</th>
            ...
            <tbody>
              ...
              <tr class="md-text">
                ...
                <td><?=dateToPageCheck($set->getField('TANGGAL_LAHIR'))?></td>
                <td><?=$set->getField('TEMPAT_LAHIR')?></td>
                <td><?=$set->getField('KARTU')?></td>
                <td><?=dateToPageCheck($set->getField('TANGGAL_KAWIN'))?></td>
                <!-- <td><?=$set->getField('NO_AKTA_NIKAH')?></td> -->
                <td><?=$set->getField('NIP_PNS')?></td>
                <td><?=$set->getField('PEKERJAAN')?></td>
                <td><?=$set->getField('STATUS_S_I_NAMA')?></td>
                <td>
                  <?
                ...
                <td width="90px">
                  ...
                  <span>
                        <a href="javascript:void(0)" class="round waves-effect waves-light purple white-text" title="Log"  onclick="parent.openModal('app/loadUrl/app/informasi_data_log?reqjson=suami_istri_json/log/<?=$reqRowId?>&reqjudul=Data Suami Istri Log')">
               ...
               <!--   <span>
                   <a href="javascript:void(0)" class="rounded material-bold blue white-text" onclick="parent.setload('pegawai_add_suami_istri_data?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>')">UBAH</a>
                 </span>
                 <span>
                   <?if($set->getField('STATUS')==1):?>
