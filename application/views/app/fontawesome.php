...
// $set= new Menu();
// $set->selectparam(array(), -1,-1, " AND A.MENU_ID = '".$reqId."' AND A.MENU_GROUP_ID = ".$reqMenuGroupId)
// $set->firstRow();
...
$arriconmenu= [];
foreach ($infoicon as $vicon) 
{
	$arrdata= [];
	$arrdata["id"]= "fa ".$vicon;
	$arrdata["text"]= $vicon;
	array_push($arriconmenu, $arrdata);
}
// print_r($arriconmenu);exit;
?>
		...
		<div class="row" style="display: flex; display: -webkit-flex; flex-wrap: wrap;">
			...
			foreach($arriconmenu as $item) 
			{
				...
				<div class="fa-item col-md-3 col-sm-4">
					<a href="javascript:void(0)" onclick="parent.seticon('<?=$reqId?>', '<?=$selectvalid?>')">
						<i class="<?=$selectvalid?>" aria-hidden="true"></i> 
						<span class="sr-only">Example of </span><?=$selectvaltext?>
					</a>
				</div>
			<?
			}
