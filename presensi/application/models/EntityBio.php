...
***************************************************************************************************** */
  ...
  ***/
  ...
  class EntityBio extends CI_Model{ 
    ...
    **/
	...
	public function getNextId($idName,$tableName,$filter=""){
		$str = "SELECT MAX($idName) AS maxvalue FROM $tableName ";
			$query = $this->load->database("bio", TRUE)->query($str);
	...
	function selectLimit($sql,$numrow=-1,$offset=-1){
		...
		$sql = $this->load->database("bio", TRUE)->_limit($sql, $numrow, $offset);
		$rs = $this->load->database("bio", TRUE)->query($sql);

		if(!$rs){
			$this->errorMsg = "PESAN ERROR : ".$this->load->database("bio", TRUE)->_error_number();
		}
		else
		{
			$i=0;
	...
	function select($sql){
		...
		$rs = $this->load->database("bio", TRUE)->query($sql, "server");
		if(!$rs){
			$this->errorMsg = "PESAN ERROR : ".$this->load->database("bio", TRUE)->_error_number();
		}
		else
	...
	function execQuery($sql){
		return $this->load->database("bio", TRUE)->query($sql);
    }
