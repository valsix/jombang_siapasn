...
class rekapitulasi_jam_kerja_json extends CI_Controller {
	...
	function json() 
	{
		...
		$allRecord = $set->getCountByParamsRekapJamKerja(array(), $reqPeriode, $statement);
		//echo $allRecord;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter =  $set->getCountByParamsRekapJamKerja(array(), $reqPeriode, $searchJson.$statement);
		
		$sOrder= " ORDER BY A.ESELON_ID ASC, A.PANGKAT_ID DESC, A.TMT_PANGKAT ASC";
		$set->selectByParamsRekapJamKerja(array(), $dsplyRange, $dsplyStart, $reqPeriode, $searchJson.$statement, $sOrder);
		  // echo $set->query;exit;
		$output = array(
	...
	function json_ranking() 
	{
		...
		$aColumns = array("NIP_BARU_NAMA_LENGKAP", "SATUAN_KERJA_NAMA", "TOTAL_JUMLAH_JAM");
		...
		$searchJson = " AND (UPPER(A.NAMA_LENGKAP) LIKE '%".strtoupper($_GET['sSearch'])."%')";
		$allRecord = $set->getCountByParamsRekapRankingJamKerja(array(), $reqPeriode, $statement);
		//echo $allRecord;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter =  $set->getCountByParamsRekapRankingJamKerja(array(), $reqPeriode, $searchJson.$statement);
		
		$sOrder= " ORDER BY TOTAL_JUMLAH_JAM DESC";
		$set->selectByParamsRekapRankingJamKerja(array(), $dsplyRange, $dsplyStart, $reqPeriode, $searchJson.$statement, $sOrder);
		  // echo $set->query;exit;
		$output = array(
			"sEcho" => intval($_GET['sEcho']),
			"iTotalRecords" => $allRecord,
	...
	function json_pertahun_ranking() 
	{
		$this->load->model('main/AbsensiRekap');
		$this->load->model('main/SatuanKerja');
		...
		for($i=1; $i <= 12; $i++) 
		{
			array_push($aColumns, "BLN_".generateZero($i,2));
		}
		array_push($aColumns, "JUMLAH");
		array_push($aColumns, "PEGAWAI_ID");
		// print_r($aColumns);exit();
		$aColumnsAlias = $aColumns;
		...
		$searchJson = " AND (UPPER(A.NAMA_LENGKAP) LIKE '%".strtoupper($_GET['sSearch'])."%')";
		$allRecord = $set->getCountByParamsTotalJamKerjaTahun(array(), $reqTahun, $statement);
		//echo $allRecord;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter =  $set->getCountByParamsTotalJamKerjaTahun(array(), $reqTahun, $searchJson.$statement);
		
		// $sOrder= " ORDER BY TOTAL_JUMLAH_JAM DESC";
		$set->selectByParamsTotalJamKerjaTahun(array(), $dsplyRange, $dsplyStart, $reqTahun, $searchJson.$statement);
		  // echo $set->query;exit;
		$output = array(
			"sEcho" => intval($_GET['sEcho']),
			"iTotalRecords" => $allRecord,
	...
	function json_terlambat_pulang_cepat() 
	{
		$this->load->model('main/AbsensiRekap');
		$this->load->model('main/SatuanKerja');
		...
		$aColumns = array("NIP_BARU_NAMA_LENGKAP", "SATUAN_KERJA_NAMA", "KELOMPOK", "JUMLAH_H", "JUMLAH_HT", "HT", "JUMLAH_HPC", "PC", "JUMLAH_TM", "TM", "PEGAWAI_ID");
		// print_r($aColumns);exit();
		$aColumnsAlias = $aColumns;
		...
		$searchJson = " AND (UPPER(A.NAMA_LENGKAP) LIKE '%".strtoupper($_GET['sSearch'])."%')";
		$allRecord = $set->getCountByParamsRekapTerlambatPulangKoreksi(array(), $reqPeriode, $statement);
		//echo $allRecord;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter =  $set->getCountByParamsRekapTerlambatPulangKoreksi(array(), $reqPeriode, $searchJson.$statement);
		
		// $sOrder= " ORDER BY TOTAL_JUMLAH_JAM DESC";
		$set->selectByParamsRekapTerlambatPulangKoreksi(array(), $dsplyRange, $dsplyStart, $reqPeriode, $searchJson.$statement);
		  // echo $set->query;exit;
		$output = array(
			"sEcho" => intval($_GET['sEcho']),
			"iTotalRecords" => $allRecord,
