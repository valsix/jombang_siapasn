...
class rekapitulasi_review_absensi_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("NIP_BARU", "NAMA_LENGKAP", "SATUAN_KERJA_NAMA", "PEGAWAI_ID");
		...
		// echo $statement;exit();
		// echo $_GET['sSearch']."==".$_POST['sSearch'];exit();

		$searchJson = " AND (UPPER(A.NAMA_LENGKAP) LIKE '%".strtoupper($_GET['sSearch'])."%')";
		$allRecord = $set->getCountByParamsPegawai(array(), $statement);
		//echo $allRecord;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter =  $set->getCountByParamsPegawai(array(), $searchJson.$statement);
		
		$sOrder= " ORDER BY A.ESELON_ID ASC, A.PANGKAT_ID DESC, A.TMT_PANGKAT ASC";
		$set->selectByParamsPegawai(array(), $dsplyRange, $dsplyStart, $searchJson.$statement, $sOrder);
		  // echo $set->query;exit;
		$output = array(
