...
class departemen_json extends CI_Controller {
	...
	function json() 
	{
		...
		$aColumns = array("DEPT_NAME", "STATUS_INTEGRASI_NAMA", "DEPT_CODE");
		...
		//$statement_privacy = " AND A.CABANG_ID = '".$this->KODE_CABANG."' ";
		
		if (trim($reqSatkerId) !== '')
			$statement .= " AND A.DEPT_CODE = '".$reqSatkerId."'";

		$searchJson = " AND (UPPER(A.DEPT_NAME) LIKE '%".strtoupper($_GET['sSearch'])."%')";
