...
class satuan_kerja_json extends CI_Controller {
	...
	function auto() 
	{
		...
		//$statement= " AND A.STATUS IS NULL";
		//$statement.= " AND UPPER(A.NAMA) LIKE '%".strtoupper($search_term)."%' ";
		$statement.= " AND ( UPPER(A.NAMA) LIKE '".strtoupper($search_term)."%' OR UPPER(A.NAMA_SINGKAT) LIKE '".strtoupper($search_term)."%' )";
		...
		$statementAktif.= " AND A.STATUS_MESIN_POSISI = 1";
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id'] = $set->getField("SATUAN_KERJA_ID");
			//$arr_json[$i]['label'] = $set->getField("NAMA");
			$arr_json[$i]['label'] = $set->getField("NAMA_SINGKAT_MESIN");
			$arr_json[$i]['desc'] = $set->getField("NAMA_SINGKAT_MESIN")."<br/><label style='font-size:12px'>".$set->getField("SATUAN_KERJA_NAMA_DETIL")."</label>";
			$i++;
		}
		echo json_encode($arr_json);		
	...
	function automesin() 
	{
		...
		$reqTanggalBatas= $this->input->get("reqTanggalBatas");
		$reqSatuanKerjaIndukId= $this->input->get("reqSatuanKerjaIndukId");
		$reqPakaiId= $this->input->get("reqPakaiId");
		$reqPakaiLainId= $this->input->get("reqPakaiLainId");
		$reqDefault= $this->input->get("reqDefault");
		$batasdata= 70;
		...
		if(!empty($reqPakaiId))
		{
			$statement.= " AND A.SATUAN_KERJA_ID NOT IN (".$reqPakaiId.")";
		}

		$set->selectByParamsMesin(array(), $batasdata, 0, $statementAktif.$statement);
		// echo $set->query;exit;
		$arr_json = array();
		$i = 0;
		while($set->nextRow())
		...
		if($reqDefault == "2")
		{
			$statement= " AND ( UPPER(IPAR.AREA_NAME) LIKE '".strtoupper($search_term)."%' )";

			if(!empty($reqPakaiLainId))
			{
				// echo str_replace(",", "','", $reqPakaiLainId);
				// echo count(explode(",", $reqPakaiLainId));
				// echo $reqPakaiLainId;

				$reqPakaiLainId= str_replace(",", "','", $reqPakaiLainId);
				$reqPakaiLainId= "'".$reqPakaiLainId."'";
				// if(count(explode(",", $reqPakaiLainId)) > 1)
				// {
				...
				$statement.= " AND IPAR.AREA_CODE NOT IN (".$reqPakaiLainId.")";
			}

			$set->selectByParamsMesinLain(array(), $batasdata, 0, $statement);
			//echo $set->query;exit;
	...
	function combotree() 
	{
		...
		$reqStatusMesinPosisi= $this->input->get("reqStatusMesinPosisi");

		$set = new SatuanKerja();
		
		$statementdetil= "";
		if(!empty($reqStatusMesinPosisi))
		{
			$statementdetil.= " AND A.STATUS_MESIN_POSISI = ".$reqStatusMesinPosisi;
		}
		...
		while($set->nextRow())
		{
			$arr_json[$i]['id'] = $set->getField("SATUAN_KERJA_ID");
			$arr_json[$i]['text'] = $set->getField("NAMA");
			$arr_json[$i]['children'] = $this->combotreegetchild($set->getField("SATUAN_KERJA_ID"), $satuankerjakondisi);
			$i++;
		}
		echo json_encode($arr_json);
	}
	
	function combotreegetchild($id, $statementdetil)
	{
		$this->load->model('siap/SatuanKerja');
		...
		while($set->nextRow())
		{
			...
			if($record > 0)
				$arr_json[$j]['children'] = $this->combotreegetchild($set->getField("SATUAN_KERJA_ID"), $statementdetil);
	...
	function treesync() 
	{
		...
		$reqId =  $this->input->get('reqId');
		$reqStatusSync =  $this->input->get('reqStatusSync');
		...
		if($reqStatusSync == "1")
		{
			$statement.= " AND SYNC.SATUAN_KERJA_ID IS NULL";
		}
		elseif($reqStatusSync == "2")
		{
			$statement.= " AND SYNC.SATUAN_KERJA_ID IS NOT NULL";
		}
		// echo $statement;exit();
		// echo $id;exit();
		...
		if ($id == 0)
		{
			// $statement= " AND A.SATUAN_KERJA_ID IN (62,66)";
			$result["total"] = 0;
			$i=0;

			$set->selectByParamsArea(array("IPAR.AREA_PARENT_CODE" => "xxx"), -1, -1);
			// echo $set->query;exit;
			//echo $set->errorMsg;exit;
			while($set->nextRow())
			{
				$reqStatusIntegrasi= $set->getField("STATUS_INTEGRASI");
				$reqStatusIntegrasiInfo= $this->kondisi($reqStatusIntegrasi, "1");
				$reqStatusDefault= $set->getField("STATUS_DEFAULT");
				$reqStatusDefaultInfo= $this->kondisi($reqStatusDefault, "1");

				$items[$i]['ID'] = $set->getField("SATUAN_KERJA_ID");
				$items[$i]['AREA_CODE'] = $set->getField("SATUAN_KERJA_ID");
				$items[$i]['AREA_NAMA'] = $set->getField("AREA_NAMA");
				$items[$i]['NAMA'] = $set->getField("NAMA");
				$items[$i]['JUMLAH_PEGAWAI_AREA'] = $set->getField("JUMLAH_PEGAWAI_AREA");
				$items[$i]['JUMLAH_DEVICE'] = $set->getField("JUMLAH_DEVICE");
				$items[$i]['STATUS_INTEGRASI_INFO'] = $reqStatusIntegrasiInfo;
				$items[$i]['STATUS_DEFAULT_INFO'] = $reqStatusDefaultInfo;
				$items[$i]['LINK_URL_INFO'] = $set->getField("LINK_URL_INFO");
				$items[$i]['state'] = $this->haschild($set->getField("SATUAN_KERJA_ID"), $statementAktif) ? 'closed' : 'open';
				$i++;
			}
			
			$set->selectByParamsSatuanKerjaTree(array("A.SATUAN_KERJA_PARENT_ID" => 0), -1, -1, $statementAktif.$statement);
			//echo $set->query;exit;
			//echo $set->errorMsg;exit;
			while($set->nextRow())
			{
		...
		{
			$i=0;
			$set->selectByParamsSatuanKerjaArea(array("IPAR.AREA_PARENT_CODE" => $id), -1, -1);
			// echo $set->query;exit;
			...
			while($set->nextRow())
			{
				...
				$result[$i]['ID'] = $set->getField("SATUAN_KERJA_ID");
				$result[$i]['AREA_CODE'] = $set->getField("SATUAN_KERJA_ID");
				$result[$i]['AREA_NAMA'] = $set->getField("AREA_NAMA");
				$result[$i]['NAMA'] = $set->getField("NAMA");
				$result[$i]['JUMLAH_PEGAWAI_AREA'] = $set->getField("JUMLAH_PEGAWAI_AREA");
				$result[$i]['JUMLAH_DEVICE'] = $set->getField("JUMLAH_DEVICE");
				$result[$i]['STATUS_INTEGRASI_INFO'] = $reqStatusIntegrasiInfo;
				$result[$i]['STATUS_DEFAULT_INFO'] = $reqStatusDefaultInfo;
				$result[$i]['LINK_URL_INFO'] = $set->getField("LINK_URL_INFO");
				$result[$i]['state'] = 'open';
				$i++;
			}

			$set->selectByParamsSatuanKerjaTree(array("A.SATUAN_KERJA_PARENT_ID" => $id), -1, -1, $statementAktif.$statement);
			// echo $set->query;exit;
			while($set->nextRow())
			{
				$reqStatusIntegrasi= $set->getField("STATUS_INTEGRASI");
				...
				$result[$i]['STATUS_DEFAULT_INFO'] = $reqStatusDefaultInfo;
				$result[$i]['LINK_URL_INFO'] = $set->getField("LINK_URL_INFO");
				$result[$i]['state'] = $this->haschild($set->getField("SATUAN_KERJA_ID"), $statementAktif) ? 'closed' : 'open';
				$i++;
			}
		}
	...
	function haschild($id, $stat)
	{
		...
		$child = new SatuanKerja();
		$child->selectByParamsCheckSatuanKerjaTree(array("SATUAN_KERJA_PARENT_ID" => $id), -1,-1, $stat);
		// echo $child->query;exit;
		$child->firstRow();
		$tempId= $child->getField("SATUAN_KERJA_ID");
		//echo $child->errorMsg;exit;
		...
		{
			$child = new SatuanKerja();
			$child->selectByParamsCheckSatuanKerjaArea(array("IPAR.AREA_PARENT_CODE" => $id), -1,-1);
			// echo $child->query;exit;
			$child->firstRow();
			$tempId= $child->getField("SATUAN_KERJA_ID");
	...
	function kondisi($value, $valpembanding, $rno="x", $ryes= "√")
	{
		if($value == $valpembanding)
			$retval= $ryes;
		else
			$retval= $rno;
