...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "bio/area_json/json/?reqSatuanKerjaId=<?=$reqSatuanKerjaId?>",
			...
			/* Click event handler */
			  ...
			  $('#btnSinkronisasi').on('click', function () {
				...
				if (r){
						...
						var jqxhr = $.get( "bio/area_json/proses/", function() {
						})
						.done(function() {
			  ...
			  $("#btnCari").on("click", function () {
			  	...
			  	oTable.fnReloadAjax("bio/area_json/json/?reqSatuanKerjaId="+reqSatuanKerjaId+"&reqStatusIntegrasi="+reqStatusIntegrasi+"&sSearch="+reqCariFilter);
			  });
		...
		function integrasi()
		{
			$.getJSON("bio/bio_integrasi_json/areaadd/",
				function(data){
					// console.log(data);
					if(parseInt(data) > 0)
					{
...
<div id="wadah">
    <div class="judul-halaman"><span>Area</span></div>
