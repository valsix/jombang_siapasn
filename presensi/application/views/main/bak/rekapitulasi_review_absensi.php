...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bServerSide": true,		
			  "ajax": {	
	            "url": "main/rekapitulasi_review_absensi_json/json/?reqSatuanKerjaId=<?=$reqSatuanKerjaId?>&reqBulan=<?=$reqBulan?>&reqTahun=<?=$reqTahun?>",
        	  	"type": "POST"
        	  },
			  // "sAjaxSource": "main/rekapitulasi_review_absensi_json/json/?reqSatuanKerjaId=<?=$reqSatuanKerjaId?>&reqBulan=<?=$reqBulan?>&reqTahun=<?=$reqTahun?>",
			...
			/* Click event handler */
			  ...
			  $('#btnLihat').on('click', function () {
				  ...
				  top.openPopup("app/loadUrl/main/rekapitulasi_review_absensi_add/?reqPegawaiId="+anSelectedId+"&reqMode=view");
			  ...
			  $("#btnCari").on("click", function () {
			  	...
			  	oTable.fnReloadAjax("main/rekapitulasi_review_absensi_json/json/?reqSatuanKerjaId="+reqSatuanKerjaId+"&reqBulan="+reqBulan+"&reqTahun="+reqTahun+"&sSearch="+reqCariFilter);
			  });
			  ...
			  $("#reqSatuanKerjaId, #reqBulan, #reqTahun").change(function() { 
			  	...
			  	document.location.href= "app/loadUrl/main/rekapitulasi_review_absensi/?reqSatuanKerjaId="+reqSatuanKerjaId+"&reqBulan="+reqBulan+"&reqTahun="+reqTahun;
			  	// setCariInfo();
			  });
...
<div id="wadah">
    <div class="judul-halaman"><span>Rekapitulasi Review Absensi</span></div>
    <div id="bluemenu" class="bluetabs" style="background:url(css/media/bluetab.gif)">    
        <ul>
			<li>
				...
				<span><a id="btnLihat" title="Lihat"><img src="images/icon-edit.png" /> Lihat</a> </span>
            </li>
        </ul>
    </div>
   <div id="parameter-tambahan" style="width: 40%; margin-right: 0px !important;">
	   	<span style="float: right;">
	   		Cari :
	   		<input type="text" id="reqCariFilter" />
	   	</span>

	   	<input type="hidden" id="reqBulan" />
	   	<input type="hidden" id="reqTahun" />
	   	<input type="hidden" id="reqSatuanKerjaId" />
   		<? /* ?>Bulan :
