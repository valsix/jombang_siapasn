...
<title>Penjadwalan Jam Kerja</title>
<base href="<?=base_url()?>">
...
<script type="text/javascript" charset="utf-8">
	var oTable;
	var json= "jadwal_json/jamkerjapegawai"; var form= "jadwal_json/jamkerjapegawaiadd";
    $(document).ready( function () {
		...
		oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			...
			"bServerSide": true,
			// "FixedColumns": true,
			"sAjaxSource": "main/"+json+"/?reqSatuanKerjaId=<?=$reqSatuanKerjaId?>&reqStatus=<?=$reqStatus?>",
		...
		$("#btnCari").on("click", function () {
			var reqSatuanKerjaId= reqJenis= reqStatus= reqCariFilter= "";
			...
			oTable.fnReloadAjax("main/"+json+"/?reqSatuanKerjaId="+reqSatuanKerjaId+"&reqJenis="+reqJenis+"&reqStatus="+reqStatus+"&reqPencarian="+reqCariFilter);
		});

		$("#reqSatuanKerjaId,#reqJenis,#reqStatus").change(function() {
			setCariInfo();
		});
		...
		$('.vmenu .first_li').on('click',function() {
			if( $(this).children().size() == 1 ) {
				infolabelvmenu= $(this).children().text();
            	...
            	?>
	            	if(infolabelvmenu == "<?=$infonama?>")
	            	{
	            		$("#reqJenisJamKerja").val("<?=$infoid?>");
	            		$("#reqJenisJamKerjaNama").val("<?=$infonama?>");
	            		$("#btnPilih").click();
	            	}
            	<?
            	}
            	...
            	if(infolabelvmenu == "Reset")
            	{
            		$("#reqJenisJamKerja").val("Reset");
            		$("#reqJenisJamKerjaNama").val("Reset");
            		$("#btnPilih").click();
            	}
			}
		});
...
<div id="wadah">
    <div class="judul-halaman">
    	<span>Penjadwalan Jam Kerja Pegawai</span>
    	<input type="hidden" id="reqSatuanKerjaId" value="<?=$reqSatuanKerjaId?>" />
    	<label id="reqLabelSatuanKerjaNama"><?=$reqSatuanKerjaNama?></label>
    	<input type="hidden" id="reqJenisJamKerja" />
    	<input type="hidden" id="reqJenisJamKerjaNama" />
    </div>
    	...
    	<ul>
            ...
            <li>
            	...
            	<select name="reqStatus" id="reqStatus">
            		...
            		<option value="xxx" <? if($reqStatus == "xxx") echo "selected";?>>Aktif</option>
	...
	<div class="vmenu">
    	...
    	{ 
    		...
    		$infonama= $arrjenisjamkerja[$i]["nama"];
    	?>
    		<div class="first_li"><span><?=$infonama?></span></div>
    	<?
    	}
    	?>
    	<div class="first_li"><span>Reset</span></div>
	</div>
</div>
