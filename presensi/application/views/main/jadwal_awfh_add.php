...
$arrstatusjadwalawfh= statusjadwalawfh();
$jumlahstatusjadwalawfh= count($arrstatusjadwalawfh);
// print_r($arrstatusjadwalawfh);exit();

$arr_json = array();
$statement = " AND A.PEGAWAI_ID IN (".$reqId.")";
$set= new KerjaJam();
$set->selectByParamsAwfhJamPegawaiView(array(), -1, -1, $reqPeriode, $statement);
...
<body class="bg-permohonan">
	<div class="area-permohonan">
		<div class="judul-monitoring"><span>Form Jadwal Jam Kerja AWFH</span></div>
        <form id="ff" method="post" novalidate enctype="multipart/form-data">
            ...
            <div class="tab-content">
                                                ...
                                                else if ($firstDayHasCome==false) 
                                                {
                                                    if ($dayCounter < $daysInMonth) 
                                                    {
                                                        ...
                                                        <div class="date_topright"><strong><?=$reqTanggal?></strong>
                                                            <?
                                                            $infodatanama= "";
                                                            $infosql= "select b.nama_jam_kerja nama from presensi.kerja_jam_shift_pegawai a inner join presensi.kerja_jam_shift b on b.kerja_jam_shift_id = a.status where pegawai_id = '".$reqId."' and hari = '".generateZero($reqTanggal,2)."' and periode = '".$reqPeriode."'";
                                                            ...
                                                            <table>
                                                                <tr>
                                                                    ...
                                                                    <select id="reqHari<?=$reqTanggal?>-<?=$reqPegawaiId?>">
                                                                        <?
                                                                        for ($stats=0; $stats < $jumlahstatusjadwalawfh; $stats++) 
                                                                        { 
                                                                            $infoid= $arrstatusjadwalawfh[$stats]["id"];
                                                                            $infonama= $arrstatusjadwalawfh[$stats]["nama"];
                                                                        ?>
                ...
                <div class="area-tombol-bawah" style="width: 805px; margin: 0 auto;" >
                    <input type="button" onclick="document.location.href='app/loadUrl/main/jadwal_awfh'" class="btn btn-primary" value="Kembali" />
...
$(function(){
    $('#ff').form({
        url:'main/jadwal_json/jamkerjaawfhpegawaiadd',
        onSubmit:function(){
        ...
        success:function(data) {
            ...
            if(rowid == "xxx")
            {
                $.messager.alert('Error',infodata,'error', function()
                {
            ...
            else
            {
                ...
                {
                    document.location.href = "app/loadUrl/main/jadwal_awfh";
                });
            }
        }
    });
