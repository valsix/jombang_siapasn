...
$set->selectByParamsMPresensiAbsen(array(), -1, -1, $reqPeriode, $searchJson.$statement, $sOrder);
// echo $set->query;exit;
$arrinfodata=[];
while($set->nextRow())
{
  ...
  $arrdata["NAMA_LENGKAP"]= $set->getField("NAMA_LENGKAP");
  $arrdata["NIP_BARU"]= $set->getField("NIP_BARU");
  $arrdata["TANGGAL"]= $set->getField("TANGGAL");
  $arrdata["WAKTU"]= $set->getField("WAKTU");
  $arrdata["TIPE_PRESENSI"]= $set->getField("TIPE_PRESENSI");
  $arrdata["TIPE_LOG"]= $set->getField("TIPE_LOG");
  $arrdata["MESIN_PRESENSI"]= $set->getField("MESIN_PRESENSI");
  $arrdata["TANGGAL_DATA_MASUK"]= $set->getField("TANGGAL_DATA_MASUK");
  array_push($arrinfodata, $arrdata);
}
$jumlahdata= count($arrinfodata);
