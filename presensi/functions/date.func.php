...
//		$_date = $arrDate[0]."-".$arrDate[1]."-".$arrDate[2];
	...
	function gettanggalperiode($_date) {
		$arrDate = explode("-", $_date);
		
		return $arrDate[2].$arrDate[1];
	}

	function getperiodetanggal($_date) {
		$arrDate = explode("-", $_date);
	...
	function getNameDayPostgre($number) {
		$arrMonth = array("1"=>"Senin", "2"=>"Selasa", "3"=>"Rabu", "4"=>"Kamis", 
	...
	function getFormattedExtDateTimeCheck($_date, $showTime=true)
	{
		...
		$arrDate = explode("-", $explodedDate);
		$_month = intval($arrDate[1]);
		$arrMonth = getExtMonth($_month);
		
		$date = $arrDate[2].' '.$arrMonth.' '.$arrDate[0];
		$time = $explodedTime;
	...
	function dateToPeriode($_date){
		
		if($_date == "")
			return "";
		
		$arrDate = explode("-", $_date);
		$_date = $arrDate[1].$arrDate[2];
		return $_date;
	}
	...
	function dateTimeToDBCheckEncrypt($_date){
		if($_date == "")
		{
			return "NULL";	
		}
		return "TO_TIMESTAMP('".$_date."', 'DD-MM-YYYY-HH24:MI:SS')";
	}

	function datediffhari($awal, $akhir){
		$start= strtotime(dateToPageCheck($awal));
		$end= strtotime(dateToPageCheck($akhir));
