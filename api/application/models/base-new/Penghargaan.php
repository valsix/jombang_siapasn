...
class Penghargaan extends Entity{ 
  ...
  **/
	...
	function insert()
	{
		...
		$str = "
			...
			(
				...
				, REF_PENGHARGAAN_ID, NAMA_DETIL, JENJANG_PERINGKAT_ID
    ...
    function selectByParamsPersonal($paramsArray=array(),$limit=-1,$from=-1,$pegawaiid, $id="", $rowid="",$statement='', $order='ORDER BY A.PENGHARGAAN_ID ASC')
	{
		...
		CASE A.NAMA WHEN '1' THEN 'Satya Lencana Karya Satya X (Perunggu)'
		WHEN '2' THEN 'Satya Lencana Karya Satya XX (Perak)' WHEN '3' THEN 'Satya Lencana Karya Satya XXX (Emas)' ELSE 'Belum di tentukan' END NAMA_NAMA_LWS,
		A.NO_SK, A.TANGGAL_SK, A.TAHUN,B.NAMA PEJABAT_PENETAP_NAMA
		, A.REF_PENGHARGAAN_ID, A.NAMA_DETIL, A.JENJANG_PERINGKAT_ID
		, RP.INFO_DETIL, RP.NAMA NAMA_NAMA
