...
***************************************************************************************************** */
  ...
  ***/
  ...
  class UlasanAsn extends Entity{ 
    ...
    **/
    ...
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='', $order='ORDER BY A.ULASAN_ASN_ID ASC')
	{
		$str = "
						SELECT 
							A.ULASAN_ASN_ID, A.PEGAWAI_ID, A.JUDUL,
							A.KETERANGAN, A.TANGGAL
						FROM ULASAN_ASN A
						WHERE 1 = 1
					"; 
	...
	{
		$str = "SELECT COUNT(A.ULASAN_ASN_ID) AS ROWCOUNT 
				FROM ULASAN_ASN A
				WHERE 1 = 1 ".$statement; 
		while(list($key,$val)=each($paramsArray))
		{
    ...
    function selectByPersonal($paramsArray=array(),$limit=-1,$from=-1, $pegawaiid, $id="", $rowid="", $statement='', $order='ORDER BY A.ULASAN_ASN_ID ASC')
	{
		$str = "
		SELECT
			A.ULASAN_ASN_ID, A.PEGAWAI_ID, A.JENIS
