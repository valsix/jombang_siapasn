    ...
    **/
    ...
    function selectByPersonal($paramsArray=array(),$limit=-1,$from=-1,$pegawaiid, $id="", $rowid="", $statement='', $order='ORDER BY A.TMT_JABATAN DESC')
	{
		...
		SELECT
			...
			, COALESCE(HK.HUKUMAN_ID,0) DATA_HUKUMAN
			, A.TMT_ESELON
			, TEMP_VALIDASI_ID, TEMP_VALIDASI_HAPUS_ID, VALIDASI, VALIDATOR, PERUBAHAN_DATA, TIPE_PERUBAHAN_DATA, TANGGAL_VALIDASI,TMT_SELESAI_JABATAN,LAMA_JABATAN_HITUNG,
	CASE WHEN LAMA_JABATAN_HITUNG >= 5 THEN 100
		...
		ELSE 0 END NILAI_REKAM_JEJAK_HITUNG,STATUS_SK_DASAR_JABATAN
		...
		LEFT JOIN (
			...
			FROM JABATAN_RIWAYAT A
	) CC ON A.JABATAN_RIWAYAT_ID = CC.JABATAN_RIWAYAT_ID
		WHERE 1 = 1 
		"; 
