  ...
  class DiklatStruktural extends Entity{ 
    ...
    **/
    ...
    function insert()
	{
		...
		$str = "
			INSERT INTO validasi.DIKLAT_STRUKTURAL (
				DIKLAT_STRUKTURAL_ID, DIKLAT_ID, PEGAWAI_ID, TEMPAT, PENYELENGGARA, ANGKATAN, TAHUN, TANGGAL_MULAI, TANGGAL_SELESAI, NO_STTPP, TANGGAL_STTPP, JUMLAH_JAM, LAST_USER, LAST_DATE, LAST_LEVEL, USER_LOGIN_ID, USER_LOGIN_PEGAWAI_ID, TEMP_VALIDASI_ID,DS_JABATAN_RIWAYAT_ID,NILAI_REKAM_JEJAK,RUMPUN_ID
			) 
			VALUES (
    ...
    function selectByPersonal($paramsArray=array(),$limit=-1,$from=-1, $pegawaiid, $id="", $rowid="", $statement='', $order='ORDER BY A.DIKLAT_STRUKTURAL_ID ASC')
	{
		...
		SELECT 
		A.DIKLAT_STRUKTURAL_ID, A.DIKLAT_ID, A.PEGAWAI_ID, A.TEMPAT, A.PENYELENGGARA, A.ANGKATAN, A.TAHUN, A.TANGGAL_MULAI, A.TANGGAL_SELESAI, 
		A.NO_STTPP, A.TANGGAL_STTPP, A.JUMLAH_JAM, B.NAMA, TEMP_VALIDASI_ID, TEMP_VALIDASI_HAPUS_ID, VALIDASI, VALIDATOR, PERUBAHAN_DATA, TIPE_PERUBAHAN_DATA, TANGGAL_VALIDASI,JR.*,nilai_rekam_jejak
